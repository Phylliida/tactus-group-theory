...
//  PREDICATE-BASE PORT (FA-8) of `normal_form_afp_textbook.rs`: the
//  `AmalgamatedData`/`Presentation`/`equiv_in_presentation` machinery is
//  type-swapped to `PredAmalgamatedData`/`PredPresentation`/
//  `equiv_in_pred_presentation`.  Presentation-agnostic helpers (word_lex_rank,
//  shortlex, shift_word, unshift_sym, benign) are reused verbatim.  The
//  relator-classification (`lemma_action_well_defined_proof`), the derivation
//  bridge (`lemma_act_word_deriv`), and `action_well_defined` are hand-rewritten
//  to the predicate disjunction form (relators are `spec_fn(Word)->bool`, not a
//  finite Seq).
...
use crate::pred_normal_form_amalgamated::{
    in_left_subgroup_pred, in_right_subgroup_pred,
    same_left_coset_pred, same_right_coset_pred,
    identifications_isomorphic_pred,
    in_generated_subgroup_pred,
};
use crate::normal_form_amalgamated::unshift_sym;
use crate::benign::*;
use crate::shortlex::*;
...
pub open spec fn a_words(data: PredAmalgamatedData) -> Seq<Word> {
    Seq::new(data.identifications.len(), |i: int| data.identifications[i].0)
}

///  The K-word generators for the right factor (v_i words).
pub open spec fn b_words(data: PredAmalgamatedData) -> Seq<Word> {
    Seq::new(data.identifications.len(), |i: int| data.identifications[i].1)
}

///  Number of identification generators.
pub open spec fn k_size(data: PredAmalgamatedData) -> nat {
    data.identifications.len() as nat
}

///  Predicate analog of `lemma_add_relators_num_generators`: adding relators to a
///  predicate presentation preserves the generator count.  Definitional — the
///  `add_relators_pred` struct literal sets `num_generators: p.num_generators`.
pub proof fn lemma_afp_num_gens_pred(p: PredPresentation, rs: Seq<Word>)
    ensures
        add_relators_pred(p, rs).num_generators == p.num_generators,
{
}

///  Predicate analog of `benign::lemma_identity_in_generated_subgroup`: ε lies in
///  every generated subgroup (witness = empty factor list).  Ported verbatim.
pub proof fn lemma_identity_in_generated_subgroup_pred(p: PredPresentation, gens: Seq<Word>)
    ensures
        in_generated_subgroup_pred(p, gens, empty_word()),
{
...
///  Predicate analog of `benign::lemma_generator_in_generated_subgroup`: each
///  generator lies in the generated subgroup (witness = singleton).  Ported verbatim.
pub proof fn lemma_generator_in_generated_subgroup_pred(
    p: PredPresentation, gens: Seq<Word>, i: int,
)
    requires
        0 <= i < gens.len(),
    ensures
        in_generated_subgroup_pred(p, gens, gens[i]),
{
    ...
    crate::benign::lemma_concat_all_singleton(gens[i]);
    lemma_pred_equiv_refl(p, gens[i]);
}
...
pub open spec fn has_left_coset_word_of_len(
    data: PredAmalgamatedData, g: Word, l: nat,
) -> bool {
    exists|w: Word| word_valid(w, data.p1.num_generators)
        && same_left_coset_pred(data, g, w) && w.len() == l
}

///  The lex base for a given alphabet: 2*n + 1 ensures injectivity.
pub open spec fn lex_base(data: PredAmalgamatedData) -> nat {
...
pub open spec fn has_left_coset_word_of_len_rank(
    data: PredAmalgamatedData, g: Word, l: nat, r: nat,
) -> bool {
    exists|w: Word| word_valid(w, data.p1.num_generators)
        && same_left_coset_pred(data, g, w) && w.len() == l
...
pub open spec fn left_min_coset_len(data: PredAmalgamatedData, g: Word) -> nat {
...
pub open spec fn left_min_coset_lex(data: PredAmalgamatedData, g: Word) -> nat {
...
pub open spec fn left_canonical_rep(data: PredAmalgamatedData, g: Word) -> Word {
    let l = left_min_coset_len(data, g);
    let r = left_min_coset_lex(data, g);
    choose|rep: Word|
        word_valid(rep, data.p1.num_generators)
        && same_left_coset_pred(data, g, rep)
        && rep.len() == l
...
pub open spec fn has_left_h_witness_of_len(
    data: PredAmalgamatedData, target: Word, l: nat,
) -> bool {
    exists|h: Word| word_valid(h, k_size(data)) && h.len() == l
        && equiv_in_pred_presentation(data.p1,
            apply_embedding(a_words(data), h), target)
}

///  Min-length K-word witnessing the subgroup decomposition.
pub open spec fn left_h_min_len(data: PredAmalgamatedData, g: Word) -> nat {
...
pub open spec fn left_h_min_lex(data: PredAmalgamatedData, g: Word) -> nat {
    let rep = left_canonical_rep(data, g);
    let target = concat(inverse_word(rep), g);
...
pub open spec fn left_h_part(data: PredAmalgamatedData, g: Word) -> Word {
    let rep = left_canonical_rep(data, g);
    let target = concat(inverse_word(rep), g);
    let l = left_h_min_len(data, g);
...
pub open spec fn has_right_coset_word_of_len(
    ...
    exists|w: Word| word_valid(w, data.p2.num_generators)
        && same_right_coset_pred(data, g, w) && w.len() == l
}

///  Minimum length of any valid word in g's right B-coset.
pub open spec fn right_min_coset_len(data: PredAmalgamatedData, g: Word) -> nat {
...
pub open spec fn right_canonical_rep(data: PredAmalgamatedData, g: Word) -> Word {
    let l = right_min_coset_len(data, g);
    choose|rep: Word|
        word_valid(rep, data.p2.num_generators)
        && same_right_coset_pred(data, g, rep)
        && rep.len() == l
}
...
pub open spec fn has_right_h_witness_of_len(
    ...
    exists|h: Word| word_valid(h, k_size(data)) && h.len() == l
        && equiv_in_pred_presentation(data.p2,
            apply_embedding(b_words(data), h), target)
}

///  Min-length K-word for right decomposition.
pub open spec fn right_h_min_len(data: PredAmalgamatedData, g: Word) -> nat {
...
pub open spec fn right_h_part(data: PredAmalgamatedData, g: Word) -> Word {
    let rep = right_canonical_rep(data, g);
    let target = concat(inverse_word(rep), g);
...
pub open spec fn same_a_rcoset(data: PredAmalgamatedData, w1: Word, w2: Word) -> bool {
    in_left_subgroup_pred(data, concat(w1, inverse_word(w2)))
}
...
pub open spec fn a_rcoset_min_len(data: PredAmalgamatedData, g: Word) -> nat {
...
pub open spec fn a_rcoset_min_lex(data: PredAmalgamatedData, g: Word) -> nat {
...
pub open spec fn a_rcoset_rep(data: PredAmalgamatedData, g: Word) -> Word {
    let l = a_rcoset_min_len(data, g);
...
pub open spec fn a_rcoset_h_min_len(data: PredAmalgamatedData, g: Word) -> nat {
...
pub open spec fn a_rcoset_h_min_lex(data: PredAmalgamatedData, g: Word) -> nat {
    let rep = a_rcoset_rep(data, g);
    let target = concat(g, inverse_word(rep));
...
pub open spec fn a_rcoset_h(data: PredAmalgamatedData, g: Word) -> Word {
    let rep = a_rcoset_rep(data, g);
    let target = concat(g, inverse_word(rep));
    let l = a_rcoset_h_min_len(data, g);
...
pub open spec fn same_b_rcoset(data: PredAmalgamatedData, w1: Word, w2: Word) -> bool {
    in_right_subgroup_pred(data, concat(w1, inverse_word(w2)))
}
...
pub open spec fn is_min_b_rcoset_len(data: PredAmalgamatedData, g: Word, l: nat) -> bool {
    has_b_rcoset_word_of_len(data, g, l) && no_shorter_b_rcoset_word(data, g, l)
}

pub open spec fn b_rcoset_min_len(data: PredAmalgamatedData, g: Word) -> nat {
...
pub open spec fn is_min_b_rcoset_lex(data: PredAmalgamatedData, g: Word, l: nat, r: nat) -> bool {
    has_b_rcoset_word_of_len_rank(data, g, l, r)
    && no_smaller_b_rcoset_lex(data, g, l, r)
}

pub open spec fn b_rcoset_min_lex(data: PredAmalgamatedData, g: Word) -> nat {
...
pub open spec fn b_rcoset_rep(data: PredAmalgamatedData, g: Word) -> Word {
    let l = b_rcoset_min_len(data, g);
...
proof fn lemma_scan_min_h_lex_g2(
    data: PredAmalgamatedData, target: Word, l: nat, current: nat, bound: nat,
)
    requires
...
pub open spec fn b_rcoset_h_min_len(data: PredAmalgamatedData, g: Word) -> nat {
...
pub open spec fn b_rcoset_h_min_lex(data: PredAmalgamatedData, g: Word) -> nat {
    let rep = b_rcoset_rep(data, g);
    let target = concat(g, inverse_word(rep));
...
pub open spec fn b_rcoset_h(data: PredAmalgamatedData, g: Word) -> Word {
    let rep = b_rcoset_rep(data, g);
    let target = concat(g, inverse_word(rep));
    let l = b_rcoset_h_min_len(data, g);
...
pub proof fn lemma_in_subgroup_equiv(
    p: PredPresentation, gens: Seq<Word>, w1: Word, w2: Word,
)
    requires
        in_generated_subgroup_pred(p, gens, w1),
        equiv_in_pred_presentation(p, w1, w2),
    ensures
        in_generated_subgroup_pred(p, gens, w2),
{
    ...
    let factors: Seq<Word> = choose|factors: Seq<Word>|
        ...
        && equiv_in_pred_presentation(p, concat_all(factors), w1);
    crate::pred_presentation::lemma_pred_equiv_transitive(
        p, concat_all(factors), w1, w2);
}

///  The left coset of g contains g itself (reflexivity).
proof fn lemma_same_left_coset_reflexive(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
    ensures
        same_left_coset_pred(data, g, g),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(p1, g);
    //  ε is in the generated subgroup
    lemma_identity_in_generated_subgroup_pred(p1, a_words(data));
    //  We need: in_generated_subgroup_pred(p1, a_words, product)
    //  = in_generated_subgroup_pred(p1, a_words, concat(inv(g), g))
    ...
    //  Actually equiv(product, ε) is what lemma_pred_word_inverse_left gives.
    //  For subgroup closure we need: in_subgroup(ε) && equiv(ε, product) => in_subgroup(product)
    //  lemma_pred_equiv_symmetric gives equiv(ε, product) from equiv(product, ε)
    crate::pred_presentation::lemma_pred_equiv_symmetric(p1, product, empty_word());
...
pub open spec fn state_valid(data: PredAmalgamatedData, h: Word, syllables: Seq<Syllable>) -> bool {
    let k = k_size(data);
    ...
    &&& (forall|i: int| 0 <= i < syllables.len() ==> ({
        ...
        if syl.is_left {
            ...
            && !in_left_subgroup_pred(data, syl.rep)
        } else {
            word_valid(syl.rep, n2) && !(syl.rep =~= empty_word())
            && !in_right_subgroup_pred(data, syl.rep)
        }
    }))
}
...
///
///  Proof chain:
///    equiv_in_pred_presentation(AFP, w1, w2)
...
pub open spec fn same_action(data: PredAmalgamatedData, w1: Word, w2: Word) -> bool {
...
pub proof fn lemma_g1_decompose_identity(data: PredAmalgamatedData)
    requires
        amalgamated_data_pred_valid(data),
    ensures
            ...
            == (empty_word(), Seq::<Syllable>::empty()),
{
    ...
    lemma_identity_in_generated_subgroup_pred(data.p1, a_words(data));
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(data.p1, e);
...
proof fn lemma_inv_equiv_eps(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
        equiv_in_pred_presentation(data.p1, g, empty_word()),
    ensures
        equiv_in_pred_presentation(data.p1, inverse_word(g), empty_word()),
{
    reveal(pred_presentation_valid);
    let p1 = data.p1;
    let inv_g = inverse_word(g);
    //  g * inv(g) ≡ ε
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p1, g);
    //  g ≡ ε, so concat(g, inv(g)) ≡ concat(ε, inv(g)) by left-congruence
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p1, g, empty_word(), inv_g);
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(p1, concat(g, inv_g), empty_word());
    crate::pred_presentation::lemma_pred_equiv_transitive(
    ...
    crate::word::lemma_inverse_word_valid(g, p1.num_generators);
    crate::pred_presentation::lemma_pred_equiv_symmetric(p1, empty_word(), inv_g);
}

///  If g ≡ ε in G₁, then g is in the left subgroup.
proof fn lemma_equiv_eps_in_subgroup(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
        equiv_in_pred_presentation(data.p1, g, empty_word()),
    ensures
        in_left_subgroup_pred(data, g),
{
    reveal(pred_presentation_valid);
    lemma_identity_in_generated_subgroup_pred(data.p1, a_words(data));
    crate::pred_presentation::lemma_pred_equiv_symmetric(data.p1, g, empty_word());
    lemma_in_subgroup_equiv(data.p1, a_words(data), empty_word(), g);
}

///  If g ≡ ε in G₁, then same_left_coset_pred(g, ε).
proof fn lemma_same_coset_equiv_eps(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
        equiv_in_pred_presentation(data.p1, g, empty_word()),
    ensures
        same_left_coset_pred(data, g, empty_word()),
{
    reveal(pred_presentation_valid);
    let inv_g = inverse_word(g);
    ...
    //  in_left_subgroup_pred(data, inv(g))

    //  same_left_coset_pred(g, ε) = in_left_subgroup_pred(concat(inv(g), ε))
...
proof fn lemma_left_min_coset_len_equiv_eps(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
        equiv_in_pred_presentation(data.p1, g, empty_word()),
        ...
        left_min_coset_len(data, g) == 0,
{
    ...
    //  ε is in g's coset (since g ≡ ε → same_left_coset_pred(g, ε))
...
proof fn lemma_left_rep_equiv_eps(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
        equiv_in_pred_presentation(data.p1, g, empty_word()),
...
proof fn lemma_left_h_min_len_equiv_eps(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
        equiv_in_pred_presentation(data.p1, g, empty_word()),
        ...
        left_h_min_len(data, g) == 0,
{
    ...
    assert(apply_embedding(a_words(data), e) =~= e);
    crate::pred_presentation::lemma_pred_equiv_symmetric(p1, g, e);
...
proof fn lemma_left_h_equiv_eps(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
        equiv_in_pred_presentation(data.p1, g, empty_word()),
        ...
        left_h_part(data, g) =~= empty_word(),
{
    ...
    reveal(pred_presentation_valid);
    crate::pred_presentation::lemma_pred_equiv_symmetric(data.p1, g, e);
...
pub proof fn lemma_g1_decompose_trivial(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
        equiv_in_pred_presentation(data.p1, g, empty_word()),
...
proof fn lemma_scan_min_coset_len(
    data: PredAmalgamatedData, g: Word, current: nat, bound: nat,
)
    requires
        amalgamated_data_pred_valid(data),
...
proof fn lemma_left_min_coset_len_satisfiable(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
    ensures
...
proof fn lemma_left_rep_props(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
    ensures
        same_left_coset_pred(data, g, left_canonical_rep(data, g)),
...
///  Converse: if same_left_coset_pred(g, ε) and left_h_part(g) = ε, then g ≡ ε.
///
...
///
///  Proving satisfiability requires: in_generated_subgroup_pred → exists K-word
...
///  TODO: prove satisfiability from in_left_subgroup_pred.
pub proof fn lemma_g1_decompose_converse(
    ...
    requires
        ...
        equiv_in_pred_presentation(data.p1, apply_embedding(a_words(data), h_witness), g),
    ensures
        equiv_in_pred_presentation(data.p1, g, empty_word()),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(data.p1, empty_word(), g);
}
...
proof fn lemma_no_smaller_h_lex_transfer(
    data: PredAmalgamatedData, target_a: Word, target_b: Word, l: nat, r: nat,
)
    requires
...
proof fn lemma_b_rcoset_h_full_props(
    data: PredAmalgamatedData, g: Word, h_witness: Word,
)
    requires
        amalgamated_data_pred_valid(data),
    ...
    }),
{
    ...
    let w_at_l: Word = choose|w: Word| word_valid(w, k_size(data)) && w.len() == l
        && equiv_in_pred_presentation(data.p2, apply_embedding(b_words(data), w), target);
...
proof fn lemma_no_smaller_g2_implies_ge(
    data: PredAmalgamatedData, target: Word, l: nat, m: nat, k: nat,
)
    requires
...
proof fn lemma_left_min_coset_len_identity(data: PredAmalgamatedData)
    requires
        amalgamated_data_pred_valid(data),
    ensures
...
pub proof fn lemma_left_rep_identity(data: PredAmalgamatedData)
    requires
        amalgamated_data_pred_valid(data),
    ensures
...
proof fn lemma_left_h_min_len_identity(data: PredAmalgamatedData)
    requires
        amalgamated_data_pred_valid(data),
    ensures
        left_h_min_len(data, empty_word()) == 0,
{
    ...
    assert(word_valid(e, k)) by { assert(e.len() == 0); }
    assert(apply_embedding(a_words(data), e) =~= e);
    crate::pred_presentation::lemma_pred_equiv_refl(p1, e);
    assert(has_left_h_witness_of_len(data, target, 0nat));
...
pub proof fn lemma_left_h_identity(data: PredAmalgamatedData)
    requires
        amalgamated_data_pred_valid(data),
    ensures
...
proof fn lemma_b_rcoset_h_min_len_identity(data: PredAmalgamatedData)
    requires
        amalgamated_data_pred_valid(data),
    ensures
        b_rcoset_h_min_len(data, empty_word()) == 0,
{
    ...
    //  b_rcoset_rep(ε) = ε
    lemma_identity_in_generated_subgroup_pred(p2, b_words(data));
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p2, e);
...
pub proof fn lemma_b_rcoset_h_satisfiable(data: PredAmalgamatedData, g: Word, h_witness: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p2.num_generators),
        word_valid(h_witness, k_size(data)),
...
pub proof fn lemma_b_rcoset_h_identity(data: PredAmalgamatedData)
    requires
        amalgamated_data_pred_valid(data),
    ensures
...
pub proof fn lemma_identity_state_canonical(data: PredAmalgamatedData)
    requires
        amalgamated_data_pred_valid(data),
    ensures
...
pub open spec fn relator_acts_trivially(
    data: PredAmalgamatedData, r: Word, h: Word, syls: Seq<Syllable>,
) -> bool {
...
pub open spec fn is_canonical_state(data: PredAmalgamatedData, h: Word, syls: Seq<Syllable>) -> bool {
    word_valid(h, k_size(data))
...
pub open spec fn action_preserves_canonical(data: PredAmalgamatedData) -> bool {
...
pub open spec fn action_well_defined(data: PredAmalgamatedData) -> bool {
    let afp = amalgamated_free_product_pred(data);
    //  Every AFP relator acts trivially on canonical states.  In the predicate
    //  base the relators are a `spec_fn(Word)->bool`, so we quantify over the
    //  WORD `r` satisfying `(afp.relators)(r)` (rather than an index into a Seq).
    &&& (forall|r: Word, inverted: bool, h: Word, syls: Seq<Syllable>|
        (afp.relators)(r) && is_canonical_state(data, h, syls) ==>
        #[trigger] relator_acts_trivially(data, get_relator_pred(r, inverted), h, syls))
...
///  PredDerivation-level well-definedness: a full derivation preserves the action.
pub proof fn lemma_act_word_deriv(
    ...
    requires
        ...
        pred_derivation_produces(amalgamated_free_product_pred(data), steps, w1) == Some(w2),
        word_valid(w1, amalgamated_free_product_pred(data).num_generators),
    ensures
        act_word(data, w1, h, syllables) == act_word(data, w2, h, syllables),
    decreases steps.len(),
{
    if steps.len() == 0 {
        ...
        let w_mid = apply_step_pred(afp, w1, step).unwrap();

        //  Connect afp.num_generators == n1 + n2 for action_preserves_canonical trigger
        lemma_afp_num_gens_pred(
            crate::pred_free_product::free_product_pred(data.p1, data.p2),
            crate::pred_amalgamated_free_product::amalgamation_relators_pred(data));
        ...
        match step {
            ...
            PredDerivationStep::RelatorInsert { position, relator, inverted } => {
                //  apply_step_pred succeeded (derivation produced Some), so the
                //  RelatorInsert arm's guard `(afp.relators)(relator)` holds.
                assert((afp.relators)(relator));
                ...
                assert(relator_acts_trivially(data,
                    get_relator_pred(relator, inverted), sh, ss));
            ...
            PredDerivationStep::RelatorDelete { position, relator, inverted } => {
                //  apply_step_pred succeeded, so `(afp.relators)(relator)` holds.
                assert((afp.relators)(relator));
                let r = get_relator_pred(relator, inverted);
                let rlen = r.len();
                let prefix = w1.subrange(0, position);
        ...
        crate::pred_amalgamated_free_product::lemma_amalgamated_pred_valid(data);
        crate::pred_presentation::lemma_pred_step_preserves_word_valid(
            afp, w1, step, w_mid);
...
proof fn lemma_a_rcoset_h_min_len_equiv(
    data: PredAmalgamatedData, g1: Word, g2: Word,
    ...
    requires
        ...
        equiv_in_pred_presentation(data.p1, g1, g2),
        ...
        a_rcoset_h_min_len(data, g1) == a_rcoset_h_min_len(data, g2),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p1, g1, g2, inverse_word(rep1));
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(p1, target1, target2);
        ...
        a_rcoset_h(data, g1) =~= a_rcoset_h(data, g2),
{
    ...
    let w1: Word = choose|w: Word| word_valid(w, k_size(data)) && w.len() == l
        && equiv_in_pred_presentation(p1, apply_embedding(a_words(data), w), target1);
    let w2: Word = choose|w: Word| word_valid(w, k_size(data)) && w.len() == l
        && equiv_in_pred_presentation(p1, apply_embedding(a_words(data), w), target2);
    ...
    crate::pred_presentation::lemma_pred_equiv_transitive(p1,
...
proof fn lemma_one_shot_identity(
    data: PredAmalgamatedData, h: Word, syls: Seq<Syllable>,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        ...
        g1_one_shot_action(data, apply_embedding(a_words(data), h), syls) == (h, syls),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(data.p1, embed_h);
...
proof fn lemma_one_shot_subgroup_restore(
    data: PredAmalgamatedData, g: Word, h: Word, syls: Seq<Syllable>,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        word_valid(g, data.p1.num_generators),
        is_canonical_state(data, h, syls),
        equiv_in_pred_presentation(data.p1, g, apply_embedding(a_words(data), h)),
    ensures
        g1_one_shot_action(data, g, syls) == (h, syls),
{
    ...
    lemma_apply_embedding_in_subgroup(data.p1, a_words(data), h);
    crate::pred_presentation::lemma_pred_equiv_symmetric(data.p1, g, embed_h);
...
proof fn lemma_one_shot_prepend_cancel(
    data: PredAmalgamatedData, p: Word, c: Word, syls: Seq<Syllable>, new_syls: Seq<Syllable>,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
            ...
            == g1_one_shot_action(data, p, syls),
{
    ...
    //  inv(c) · c ≡ ε
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(p1, c);
    //  concat(p, concat(inv(c), c)) ≡ concat(p, ε) =~= p
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p1, p, concat(inverse_word(c), c), empty_word());
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p1, concat(p, empty_word()));
    ...
    let hw1: Word = choose|hw: Word| word_valid(hw, k_size(data))
        && equiv_in_pred_presentation(p1, apply_embedding(a_words(data), hw),
...
proof fn lemma_one_shot_subgroup_prepend_setup(
    data: PredAmalgamatedData, q: Word, c: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
    ...
    ensures ({
        ...
        &&& target_q =~= q
        &&& equiv_in_pred_presentation(data.p1, target_p, target_q)
    ...
    }),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p1, q);
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(p1,
...
proof fn lemma_one_shot_subgroup_prepend_h_eq(
    data: PredAmalgamatedData, q: Word, p: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        ...
        a_rcoset_h(data, q) =~= a_rcoset_h(data, p),
{
    ...
    let hw1: Word = choose|hw: Word| word_valid(hw, k_size(data))
        && equiv_in_pred_presentation(p1, apply_embedding(a_words(data), hw), target_q);
    let hw2: Word = choose|hw: Word| word_valid(hw, k_size(data))
        && equiv_in_pred_presentation(p1, apply_embedding(a_words(data), hw), target_p);
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(p1, target_p, target_q);
    ...
    let w_q: Word = choose|w: Word| word_valid(w, k_size(data)) && w.len() == l_q
        && equiv_in_pred_presentation(p1, apply_embedding(a_words(data), w), target_q);
    let w_p: Word = choose|w: Word| word_valid(w, k_size(data)) && w.len() == l_q
        && equiv_in_pred_presentation(p1, apply_embedding(a_words(data), w), target_p);
...
proof fn lemma_one_shot_subgroup_prepend(
    data: PredAmalgamatedData, q: Word, c: Word, syls: Seq<Syllable>, new_syls: Seq<Syllable>,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
...
proof fn lemma_one_shot_g1_invariant(
    data: PredAmalgamatedData, g: Word, g_prime: Word, syls: Seq<Syllable>,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        ...
        equiv_in_pred_presentation(data.p1, g, g_prime),
        ...
        g1_one_shot_action(data, g, syls) == g1_one_shot_action(data, g_prime, syls),
{
    ...
    if !(a_rcoset_rep(data, g) =~= empty_word()) && syls.len() > 0 && syls.first().is_left {
        ...
        crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p1, g, g_prime, c1);
...
proof fn lemma_embed_hs_equiv_chain(
    data: PredAmalgamatedData, w: Word, g_s: Word, h_s: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
    ...
    ensures ({
        ...
        &&& equiv_in_pred_presentation(data.p1, concat(w, embed_hs),
    ...
    }),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p1,
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p1, w, embed_hs,
        concat(g_s, inverse_word(rep_s)));
            ...
            == g1_one_shot_action(data, concat(w, apply_embedding(a_words(data), h_s)), syls_s),
{
    ...
    if a_rcoset_rep(data, peeled) =~= empty_word() {
        ...
        crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(data.p1, rep_s);
        crate::word::lemma_concat_word_valid(inverse_word(rep_s), rep_s, n1);
        crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(data.p1, concat(w, g_s),
        ...
        crate::pred_presentation::lemma_pred_equiv_refl(data.p1, concat(concat(w, g_s), empty_word()));
...
proof fn lemma_peeled_concat_bridge(
    data: PredAmalgamatedData, w_full: Word, rep: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
            ...
            concat(concat(w_full, inverse_word(rep)), rep), w_full),
{
    ...
    //  inv(rep) · rep ≡ ε
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(p1, rep);
    crate::word::lemma_concat_word_valid(inverse_word(rep), rep, n1);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p1, w_full,
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p1, concat(w_full, empty_word()));
}
            ...
            == g1_one_shot_action(data, concat(w, apply_embedding(a_words(data), h_s)), syls_s),
{
    ...
    if merged_rep =~= empty_word() {
        ...
        crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p1, w, embed_hs, full);
        crate::pred_presentation::lemma_pred_equiv_symmetric(p1, concat(w, embed_hs), concat(w, full));
    ...
    if rep_s =~= empty_word() {
        ...
        let hw_s: Word = choose|hw: Word| word_valid(hw, k_size(data))
            && equiv_in_pred_presentation(p1, apply_embedding(a_words(data), hw),
                concat(g_s, inverse_word(rep_s)));
        lemma_rcoset_decomposition(data, g_s, hw_s);
        crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p1, w,
...
pub proof fn lemma_g1_relator_acts_trivially(
    ...
    requires
        ...
        is_canonical_state(data, h, syls),
        word_valid(w, data.p1.num_generators),
        equiv_in_pred_presentation(data.p1, w, empty_word()),
        action_preserves_canonical(data),
    ensures
        act_word(data, w, h, syls) == (h, syls),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(data.p1, w, empty_word(), embed_h);
...
proof fn lemma_g2_one_shot_subgroup_restore(
    ...
    requires
        ...
        word_valid(g, data.p2.num_generators),
        is_canonical_state(data, h, syls),
        equiv_in_pred_presentation(data.p2, g, apply_embedding(b_words(data), h)),
    ensures
        g2_one_shot_action(data, g, syls) == (h, syls),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(data.p2, g, embed_h);
...
proof fn lemma_b_rcoset_h_min_len_equiv_general(
    ...
    requires
        ...
        equiv_in_pred_presentation(data.p2, g1, g2),
        ...
        b_rcoset_h_min_len(data, g1) == b_rcoset_h_min_len(data, g2),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p2, g1, g2, inverse_word(rep1));
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(p2, target1, target2);
        ...
        b_rcoset_h(data, g1) =~= b_rcoset_h(data, g2),
{
    ...
    crate::word::lemma_concat_word_valid(g2, inverse_word(b_rcoset_rep(data, g2)), n2);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p2, g1, g2,
    ...
    //  Lex scan
    let w1: Word = choose|w: Word| word_valid(w, k_size(data)) && w.len() == l
        && equiv_in_pred_presentation(p2, apply_embedding(b_words(data), w), target1);
    let w2: Word = choose|w: Word| word_valid(w, k_size(data)) && w.len() == l
        && equiv_in_pred_presentation(p2, apply_embedding(b_words(data), w), target2);
    let wr1 = word_lex_rank_base(w1, h_lex_base(data));
    let wr2 = word_lex_rank_base(w2, h_lex_base(data));
    ...
    crate::pred_presentation::lemma_pred_equiv_transitive(p2,
...
proof fn lemma_g2_one_shot_g2_invariant(
    ...
    requires
        ...
        equiv_in_pred_presentation(data.p2, g, g_prime),
        (syls.len() > 0 && !syls.first().is_left
        ...
        g2_one_shot_action(data, g, syls) == g2_one_shot_action(data, g_prime, syls),
{
    ...
    let hw1: Word = choose|hw: Word| word_valid(hw, k_size(data))
        && equiv_in_pred_presentation(p2, apply_embedding(b_words(data), hw),
            concat(g, inverse_word(b_rcoset_rep(data, g))));
    let hw2: Word = choose|hw: Word| word_valid(hw, k_size(data))
        && equiv_in_pred_presentation(p2, apply_embedding(b_words(data), hw),
            concat(g_prime, inverse_word(b_rcoset_rep(data, g_prime))));
    ...
    if !(b_rcoset_rep(data, g) =~= empty_word()) && syls.len() > 0 && !syls.first().is_left {
        ...
        crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p2, g, g_prime, c1);
            ...
            == g2_one_shot_action(data, p, syls),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(p2, c);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p2, p, concat(inverse_word(c), c), empty_word());
    assert(concat(p, empty_word()) =~= p) by {
        assert(concat(p, empty_word()).len() == p.len());
        assert forall|k: int| 0 <= k < p.len() implies concat(p, empty_word())[k] == p[k] by {}
    }
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p2, concat(p, empty_word()));
    //  concat(q, c) ≡ p
...
proof fn lemma_g2_subgroup_prepend_setup(
    ...
    ensures ({
        ...
        &&& target_q =~= q
        &&& equiv_in_pred_presentation(data.p2, target_p, target_q)
    ...
    }),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p2, q);
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(p2,
        concat(concat(q, c), inverse_word(c)), q);
        ...
        b_rcoset_h(data, q) =~= b_rcoset_h(data, p),
{
    ...
    //  Min-len equality
    let hw1: Word = choose|hw: Word| word_valid(hw, k_size(data))
        && equiv_in_pred_presentation(p2, apply_embedding(b_words(data), hw), target_q);
    let hw2: Word = choose|hw: Word| word_valid(hw, k_size(data))
        && equiv_in_pred_presentation(p2, apply_embedding(b_words(data), hw), target_p);
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(p2, target_p, target_q);
    ...
    let wq: Word = choose|w: Word| word_valid(w, k_size(data)) && w.len() == l_q
        && equiv_in_pred_presentation(p2, apply_embedding(b_words(data), w), target_q);
    let wp: Word = choose|w: Word| word_valid(w, k_size(data)) && w.len() == l_q
        && equiv_in_pred_presentation(p2, apply_embedding(b_words(data), w), target_p);
...
proof fn lemma_g2_embed_hs_equiv_chain(
    ...
    ensures ({
        ...
        &&& equiv_in_pred_presentation(data.p2, concat(w, embed_hs),
                concat(concat(w, g_s), inverse_word(rep_s)))
    ...
    }),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p2,
        concat(embed_hs, rep_s), g_s, inverse_word(rep_s));
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(p2,
        concat(g_s, inverse_word(rep_s)), embed_hs);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p2, w, embed_hs,
        concat(g_s, inverse_word(rep_s)));
            ...
            concat(concat(w_full, inverse_word(rep)), rep), w_full),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(p2, rep);
    crate::word::lemma_concat_word_valid(inverse_word(rep), rep, n2);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p2, w_full,
        concat(inverse_word(rep), rep), empty_word());
    assert(concat(w_full, empty_word()) =~= w_full) by {
        assert(concat(w_full, empty_word()).len() == w_full.len());
        assert forall|k: int| 0 <= k < w_full.len()
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p2, concat(w_full, empty_word()));
}
    ...
    if rep_s =~= empty_word() {
        ...
        let hw_s: Word = choose|hw: Word| word_valid(hw, k_size(data))
            && equiv_in_pred_presentation(p2, apply_embedding(b_words(data), hw),
                concat(g_s, inverse_word(rep_s)));
        lemma_b_rcoset_decomposition(data, g_s, hw_s);
        crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p2, w,
            ...
            == g2_one_shot_action(data, concat(w, apply_embedding(b_words(data), h_s)), syls_s),
{
    ...
    if merged_rep =~= empty_word() {
        crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p2, w, embed_hs, full);
        crate::pred_presentation::lemma_pred_equiv_symmetric(p2, concat(w, embed_hs), concat(w, full));
    ...
    decreases w.len(),
{
    ...
    if w.len() == 0 {
        ...
        crate::pred_presentation::lemma_pred_equiv_refl(data.p2, embed_h);
...
pub proof fn lemma_g2_relator_acts_trivially(
    ...
    requires
        ...
        is_canonical_state(data, h, syls),
        word_valid(w, data.p2.num_generators),
        equiv_in_pred_presentation(data.p2, w, empty_word()),
        action_preserves_canonical(data),
    ensures
        act_word(data, shift_word(w, data.p1.num_generators), h, syls) == (h, syls),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(data.p2, w, empty_word(), embed_h);
    assert(concat(empty_word(), embed_h) =~= embed_h) by {
        assert(concat(empty_word(), embed_h).len() == embed_h.len());
        assert forall|k: int| 0 <= k < embed_h.len()
            implies concat(empty_word(), embed_h)[k] == embed_h[k] by {}
...
proof fn lemma_diff_trivial_implies_equiv(
    p: PredPresentation, a: Word, b: Word,
)
    requires
        ...
        equiv_in_pred_presentation(p, a, b),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p,
        concat(a, inv_b), empty_word(), b);
    //  Chain 2: inv(b) · b ≡ ε → a · (inv(b) · b) ≡ a · ε = a
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(p, b);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p,
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(p,
        concat(a, concat(inv_b, b)), a);
    crate::pred_presentation::lemma_pred_equiv_transitive(p,
...
proof fn lemma_ident_g2_diff_trivial(
    data: PredAmalgamatedData, h_prime: Word, k_combined: Word, k_diff: Word, g2_product: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p2),
            ...
            apply_embedding(b_words(data), k_diff), empty_word()),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p2, apply_embedding(b_words(data), k_combined));
    ...
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p2, embed_b_kcomb);
    crate::pred_presentation::lemma_pred_equiv_transitive(p2,
...
proof fn lemma_ident_g1_product_equiv(
    data: PredAmalgamatedData, i: int, h: Word, h_prime: Word,
    ...
    requires
        ...
        identifications_isomorphic_pred(data),
        k_diff =~= concat(h_prime, inverse_word(k_combined)),
    ...
    }),
{
    ...
    //  identifications_isomorphic_pred gives the biconditional for any K-word
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p1,
        u_i, embed_a_hprime, embed_a_kcomb);

    //  u_i · inv(u_i) ≡ ε → (u_i · inv(u_i)) · embed_a(h) ≡ ε · embed_a(h) = embed_a(h)
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p1, u_i);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p1,
...
proof fn lemma_ident_isomorphism_transfer(
    data: PredAmalgamatedData, i: int, h: Word, h_prime: Word, g2_product: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
    ...
    ensures ({
        ...
        let embed_a_h = apply_embedding(a_words(data), h);
        let embed_a_hprime = apply_embedding(a_words(data), h_prime);
        equiv_in_pred_presentation(data.p1, concat(u_i, embed_a_hprime), embed_a_h)
    }),
{
    let n1 = data.p1.num_generators;
    let n2 = data.p2.num_generators;
...
///  This is the mathematical heart — uses identifications_isomorphic_pred.
///
///  Proof sketch:
...
pub proof fn lemma_identification_relator_acts_trivially(
    data: PredAmalgamatedData, i: int, h: Word, syls: Seq<Syllable>,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        ...
        action_preserves_canonical(data),
    ensures
        act_word(data, amalgamation_relator_pred(data, i), h, syls) == (h, syls),
{
    let n1 = data.p1.num_generators;
    let n2 = data.p2.num_generators;
    let p1 = data.p1;
    ...
    let relator = amalgamation_relator_pred(data, i);
    ...
    lemma_generator_in_generated_subgroup_pred(p2, b_words(data), i);
...
proof fn lemma_inv_r_concat_r_trivial(
    ...
    requires
        ...
        is_canonical_state(data, h, syls),
        action_preserves_canonical(data),
        word_valid(r, amalgamated_free_product_pred(data).num_generators),
    ensures
    ...
    decreases r.len(),
{
    let n1 = data.p1.num_generators;
    let n2 = data.p2.num_generators;
    let fp = crate::pred_free_product::free_product_pred(data.p1, data.p2);
    lemma_afp_num_gens_pred(fp,
        crate::pred_amalgamated_free_product::amalgamation_relators_pred(data));
    assert(amalgamated_free_product_pred(data).num_generators == n1 + n2);
    ...
    }),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p1, embed_h);
...
///  Uses identifications_isomorphic_pred to transfer between G₁ and G₂ equiv classes.
proof fn lemma_a_witness_to_b_witness(
    data: PredAmalgamatedData, h0: Word, l: nat,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        ...
        has_right_h_witness_of_len(data, apply_embedding(b_words(data), h0), l),
{
    ...
    let h_prime: Word = choose|h: Word| word_valid(h, k) && h.len() == l
        && equiv_in_pred_presentation(p1, apply_embedding(a_words(data), h),
    ...
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p1,
        apply_embedding(a_words(data), h0));
    crate::pred_presentation::lemma_pred_equiv_transitive(p1,
    ...
    //  By identifications_isomorphic_pred: equiv(embed_b(diff), ε) in G₂
    //  (identifications_isomorphic_pred quantifies over word_valid(w, k))
        ...
        has_left_h_witness_of_len(data, apply_embedding(a_words(data), h0), l),
{
    ...
    let h_prime: Word = choose|h: Word| word_valid(h, k) && h.len() == l
        && equiv_in_pred_presentation(p2, apply_embedding(b_words(data), h),
            apply_embedding(b_words(data), h0));
    ...
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p2,
        apply_embedding(b_words(data), h0));
    crate::pred_presentation::lemma_pred_equiv_transitive(p2,
    ...
    //  By identifications_isomorphic_pred (reverse direction): equiv(embed_a(diff), ε) in G₁
    crate::benign::lemma_apply_embedding_concat(a_words(data), h_prime, inverse_word(h0));
    crate::benign::lemma_apply_embedding_inverse(a_words(data), h0);
    crate::benign::lemma_apply_embedding_valid(a_words(data), h_prime, n1);
    crate::benign::lemma_apply_embedding_valid(a_words(data), h0, n1);
...
proof fn lemma_b_min_len_eq(
    data: PredAmalgamatedData, h: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        ...
        b_rcoset_h_min_len(data, apply_embedding(b_words(data), h)) == h.len(),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p2, embed_b_h);
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p1, embed_a_h);
    ...
    }),
{
    ...
    assert forall|r: nat| pred_b_rank(r) implies #[trigger] pred_a_rank(r) by {
        ...
        let h_r: Word = choose|hw: Word| word_valid(hw, k) && hw.len() == hl
            ...
            && equiv_in_pred_presentation(p2, apply_embedding(b_words(data), hw), target_bh);
        ...
        crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p2, apply_embedding(b_words(data), h));
        crate::pred_presentation::lemma_pred_equiv_transitive(p2,
        ...
        //  identifications_isomorphic_pred: embed_b(diff) ≡ ε → embed_a(diff) ≡ ε
    ...
    }),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p2, embed_h);
...
///  Mirrors lemma_a_rcoset_h_b_canonical (uses identifications_isomorphic_pred for A↔B transfer).
#[verifier::rlimit(200)]
    ...
    }),
{
    ...
    by {
        let h_r: Word = choose|hw: Word| word_valid(hw, k) && hw.len() == al
            && word_lex_rank_base(hw, h_lex_base(data)) == r2
            && equiv_in_pred_presentation(p1, apply_embedding(a_words(data), hw), target_lh);
        crate::benign::lemma_apply_embedding_valid(a_words(data), h_r, n1);
        crate::word::lemma_inverse_word_valid(embed_a_h, n1);
        crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p1,
            apply_embedding(a_words(data), h_r), embed_a_h, inverse_word(embed_a_h));
        crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p1, embed_a_h);
        crate::pred_presentation::lemma_pred_equiv_transitive(p1,
...
///  Prove action_preserves_canonical from identifications_isomorphic_pred.
...
///  identifications_isomorphic_pred implies action_preserves_canonical (universal statement).
pub proof fn lemma_iso_implies_apc(data: PredAmalgamatedData)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        pred_presentation_valid(data.p2),
        ...
        action_well_defined(data),
{
    ...
    let afp = amalgamated_free_product_pred(data);
    let fp = crate::pred_free_product::free_product_pred(data.p1, data.p2);
    let amalg_rs = crate::pred_amalgamated_free_product::amalgamation_relators_pred(data);
    let k = data.identifications.len();
    reveal(pred_presentation_valid);

    //  afp = add_relators_pred(fp, amalg_rs), so (afp.relators)(r) reduces to
    //  added_relators_pred(fp, amalg_rs, r) = (fp.relators)(r) || (∃j. r = amalg_rs[j]).
    //  And (fp.relators)(r) = free_product_pred_relators(p1, p2, r)
    //                       = (p1.relators)(r) || shifted_pred(p2.relators, n1, r).
    //  Part 1: Every AFP relator (and its inverse) acts trivially.  In the
    //  predicate base we quantify over the WORD r satisfying (afp.relators)(r),
    //  case-splitting on the relator-membership DISJUNCTION rather than an index
    //  range.
    assert forall|r: Word, inverted: bool, h: Word, syls: Seq<Syllable>|
        (afp.relators)(r) && is_canonical_state(data, h, syls)
        implies #[trigger] relator_acts_trivially(data, get_relator_pred(r, inverted), h, syls)
    by {
        //  (afp.relators)(r) == added_relators_pred(fp, amalg_rs, r)
        assert(added_relators_pred(fp, amalg_rs, r));
        if (fp.relators)(r) {
            //  free-product relator: split into p1-relator vs shifted p2-relator
            assert(free_product_pred_relators(data.p1, data.p2, r));
            if (data.p1.relators)(r) {
                //  G₁ relator: r ≡ ε in G₁
                assert(word_valid(r, n1));
                crate::pred_presentation_lemmas::lemma_pred_relator_is_identity(data.p1, r);
                if !inverted {
                ...
                //  G₂ relator: r = shift(r2, n1) where (p2.relators)(r2), r2 ≡ ε in G₂
                assert(shifted_pred(data.p2.relators, n1, r));
                let r2 = choose|w0: Word|
                    (data.p2.relators)(w0) && r == crate::free_product::shift_word(w0, n1);
                assert((data.p2.relators)(r2) && r == crate::free_product::shift_word(r2, n1));
                assert(word_valid(r2, n2));
                crate::pred_presentation_lemmas::lemma_pred_relator_is_identity(data.p2, r2);
                if !inverted {
                    lemma_g2_relator_acts_trivially(data, r2, h, syls);
                } else {
                    //  inv(shift(r2)) =~= shift(inv(r2)) and inv(r2) ≡ ε in G₂
                    crate::word::lemma_inverse_word_valid(r2, n2);
                    lemma_equiv_inverse(data.p2, r2, empty_word());
                    crate::free_product::lemma_shift_inverse_word(r2, n1);
                    lemma_g2_relator_acts_trivially(data, inverse_word(r2), h, syls);
                }
            }
        } else {
            //  Identification relator: ∃j. r = amalg_rs[j] = amalgamation_relator_pred(data, j)
            assert(exists|j: int| 0 <= j < amalg_rs.len() && r == amalg_rs[j]);
            let j = choose|j: int| 0 <= j < amalg_rs.len() && r == amalg_rs[j];
            assert(0 <= j < k);
            assert(amalg_rs[j] == crate::pred_amalgamated_free_product::amalgamation_relator_pred(data, j));
            //  word_valid(r, n1+n2) from pred_presentation_valid(afp)
            crate::pred_amalgamated_free_product::lemma_amalgamated_pred_valid(data);
            lemma_afp_num_gens_pred(fp, amalg_rs);
...
pub proof fn lemma_afp_injectivity(
    ...
    requires
        ...
        action_preserves_canonical(data),
        word_valid(w, data.p1.num_generators),
        equiv_in_pred_presentation(amalgamated_free_product_pred(data), w, empty_word()),
    ensures
        equiv_in_pred_presentation(data.p1, w, empty_word()),
{
    ...
    let steps: Seq<PredDerivationStep> = choose|steps: Seq<PredDerivationStep>|
        #[trigger] pred_derivation_produces(afp, steps, w) == Some(empty_word());
    ...
    //  same_a_rcoset(data, w, ε) → in_left_subgroup_pred(data, w)
    ...
    let h_witness: Word = choose|hw: Word|
        ...
        && equiv_in_pred_presentation(data.p1, apply_embedding(a_words(data), hw), w);
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(data.p1, empty_word(), w);
}
...
pub proof fn lemma_afp_injectivity_right(
    ...
    requires
        ...
        action_preserves_canonical(data),
        word_valid(w, data.p2.num_generators),
        equiv_in_pred_presentation(amalgamated_free_product_pred(data), shift_word(w, data.p1.num_generators), empty_word()),
    ensures
        equiv_in_pred_presentation(data.p2, w, empty_word()),
{
    let n1 = data.p1.num_generators;
    ...
    //  PredDerivation → action equality
    let steps: Seq<PredDerivationStep> = choose|steps: Seq<PredDerivationStep>|
        #[trigger] pred_derivation_produces(afp, steps, shift_word(w, n1)) == Some(empty_word());
    ...
    //  w ∈ B (from b_rcoset_rep = ε → in_right_subgroup_pred)
    lemma_subgroup_to_k_word(data.p2, b_words(data), w);
    let h_witness: Word = choose|hw: Word|
        word_valid(hw, k_size(data))
        && equiv_in_pred_presentation(data.p2, apply_embedding(b_words(data), hw), w);

    lemma_b_rcoset_h_satisfiable(data, w, h_witness);
    //  equiv(embed_b(b_rcoset_h(w)), concat(w, inv(rep))) = equiv(embed_b(ε), w) = equiv(ε, w)

    crate::pred_presentation::lemma_pred_equiv_symmetric(data.p2, empty_word(), w);
}
    ...
    }),
{
    ...
    let w: Word = choose|w: Word| word_valid(w, k_size(data)) && w.len() == l
        && equiv_in_pred_presentation(data.p1, apply_embedding(a_words(data), w), target);
    let wr = word_lex_rank_base(w, h_lex_base(data));
...
proof fn lemma_scan_a_rcoset_lex(
    data: PredAmalgamatedData, g: Word, l: nat, current: nat, bound: nat,
)
    requires
...
proof fn lemma_a_rcoset_rep_satisfiable(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
    ensures
        ...
        is_min_a_rcoset_lex(data, g, a_rcoset_min_len(data, g), a_rcoset_min_lex(data, g)),
{
    ...
    //  same_a_rcoset(g, g) = in_left_subgroup_pred(concat(g, inv(g)))
    ...
    //  concat(g, inv(g)) ≡ ε
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(data.p1, g);
    //  ε is in subgroup, concat(g,inv(g)) ≡ ε → in_left_subgroup_pred by equiv
    lemma_identity_in_generated_subgroup_pred(data.p1, a_words(data));
    crate::pred_presentation::lemma_pred_equiv_symmetric(data.p1, concat(g, inverse_word(g)), empty_word());
...
pub proof fn lemma_a_rcoset_in_subgroup(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        word_valid(g, data.p1.num_generators),
        ...
        a_rcoset_rep(data, g) =~= empty_word(),
{
    ...
    //  same_a_rcoset(g, ε) = in_left_subgroup_pred(concat(g, inv(ε)))
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(data.p1, g);
...
proof fn lemma_in_subgroup_both_reps_eps(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        word_valid(g, data.p1.num_generators),
        ...
        left_canonical_rep(data, g) =~= empty_word(),
{
    ...
    //  Right coset rep = ε
    lemma_a_rcoset_in_subgroup(data, g);
    //  Left coset rep = ε: same_left_coset_pred(g, ε) via inv(g) ∈ A
    crate::word::lemma_inverse_word_valid(g, n1);
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p1, inverse_word(g));
    lemma_in_subgroup_equiv(p1, a_words(data),
        inverse_word(g), concat(inverse_word(g), e));
    //  same_left_coset_pred(g, ε) established
    lemma_left_rep_identity(data);
...
pub proof fn lemma_a_rcoset_rep_props(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
    ensures
...
proof fn lemma_a_rcoset_h_satisfiable(data: PredAmalgamatedData, g: Word, h_witness: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
        word_valid(h_witness, k_size(data)),
...
pub proof fn lemma_apply_embedding_in_subgroup(
    p: PredPresentation, gens: Seq<Word>, h: Word,
)
    requires
        pred_presentation_valid(p),
    ...
    ensures
        in_generated_subgroup_pred(p, gens, apply_embedding(gens, h)),
    decreases h.len(),
{
    if h.len() == 0 {
        assert(apply_embedding(gens, h) =~= empty_word());
        lemma_identity_in_generated_subgroup_pred(p, gens);
    } else {
        let s = h.first();
        ...
        match s {
            Symbol::Gen(i) => {
                ...
                lemma_generator_in_generated_subgroup_pred(p, gens, i as int);
            }
            Symbol::Inv(i) => {
...
///  where target = inv(rep) · g. This follows from same_left_coset_pred(g, rep),
///  subgroup closure under inverse, and subgroup_to_k_word.
pub proof fn lemma_h_witness_exists(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        word_valid(g, data.p1.num_generators),
                ...
                concat(inverse_word(left_canonical_rep(data, g)), g)),
{
    ...
    //  same_left_coset_pred(g, rep) from left_rep_props
    lemma_left_rep_props(data, g);
    //  same_left_coset_pred(g, rep) = in_left_subgroup_pred(inv(g) · rep)
    ...
    //  in_left_subgroup_pred(concat(inv(g), rep))
    //  → lemma_subgroup_inverse → in_left_subgroup_pred(inverse_word(concat(inv(g), rep)))
    ...
    //  in_generated_subgroup_pred(p1, a_words, inv_concat) and inv_concat =~= target
    //  → in_generated_subgroup_pred(p1, a_words, target)

    assert(in_generated_subgroup_pred(data.p1, a_words(data), target));
            ...
            == (empty_word(), Seq::<Syllable>::empty()),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(
    ...
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(p1, s_word);
    crate::pred_presentation::lemma_pred_equiv_transitive(p1, product2,
    ...
    lemma_identity_in_generated_subgroup_pred(p1, a_words(data));
    crate::pred_presentation::lemma_pred_equiv_symmetric(p1, product2, e);
...
//  ============================================================
//  Part K: Bridge — in_generated_subgroup_pred ↔ apply_embedding
//  ============================================================

///  If w is in the generated subgroup, there exists a K-word whose embedding ≡ w.
///  This bridges in_generated_subgroup_pred (existential over factors) to
///  apply_embedding (K-word based).
///
///  Proof: in_generated_subgroup_pred gives factors with concat_all(factors) ≡ w.
...
pub proof fn lemma_subgroup_to_k_word(
    ...
    requires
        in_generated_subgroup_pred(p, gens, w),
    ensures
        exists|h: Word|
            word_valid(h, gens.len())
            && equiv_in_pred_presentation(p, apply_embedding(gens, h), w),
{
    ...
    let h: Word = choose|h: Word| word_valid(h, gens.len())
        && equiv_in_pred_presentation(p, apply_embedding(gens, h), concat_all(factors));
    crate::pred_presentation::lemma_pred_equiv_transitive(p,
    ...
    decreases factors.len(),
{
    if factors.len() == 0 {
        ...
        assert(apply_embedding(gens, h) =~= empty_word());
        assert(concat_all(factors) =~= empty_word());
        crate::pred_presentation::lemma_pred_equiv_refl(p, empty_word());
        //  Witness the exists: h = ε satisfies both conditions
        assert(equiv_in_pred_presentation(p, apply_embedding(gens, h), concat_all(factors)));
    } else {
        //  IH on rest
        ...
        let h_rest: Word = choose|h_rest: Word| word_valid(h_rest, gens.len())
            && equiv_in_pred_presentation(p, apply_embedding(gens, h_rest), concat_all(rest));
        ...
        crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p, first,
...
pub proof fn lemma_subgroup_concat(
    p: PredPresentation, gens: Seq<Word>, a: Word, b: Word,
)
    requires
        in_generated_subgroup_pred(p, gens, a),
        in_generated_subgroup_pred(p, gens, b),
    ensures
        in_generated_subgroup_pred(p, gens, concat(a, b)),
{
    //  Extract factor witnesses
    let fa: Seq<Word> = choose|fa: Seq<Word>|
        #[trigger] factors_from_generators(gens, fa)
        && equiv_in_pred_presentation(p, concat_all(fa), a);
    let fb: Seq<Word> = choose|fb: Seq<Word>|
        #[trigger] factors_from_generators(gens, fb)
        && equiv_in_pred_presentation(p, concat_all(fb), b);
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat(p,
...
proof fn lemma_equiv_inverse_helper(
    ...
    ensures
        //  concat(inv(b), a) ≡ ε
        equiv_in_pred_presentation(p, concat(inverse_word(b), a), empty_word()),
{
    let inv_b = inverse_word(b);
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p, inv_b, a, b);
    //  ε ≡ concat(inv(b), b) by symmetry
    crate::word::lemma_concat_word_valid(inv_b, b, p.num_generators);
    crate::pred_presentation::lemma_pred_equiv_symmetric(p, concat(inv_b, b), empty_word());
...
pub proof fn lemma_equiv_inverse(
    ...
    requires
        ...
        word_valid(b, p.num_generators),
    ensures
        equiv_in_pred_presentation(p, inverse_word(a), inverse_word(b)),
{
    ...
    //  a * inv(a) ≡ ε
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p, a);
    //  concat(inv(b), concat(a, inv(a))) ≡ concat(inv(b), ε) by right-congruence
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p, inv_b,
...
proof fn lemma_reverse_invert_is_inverse(
    p: PredPresentation, factors: Seq<Word>,
)
    ensures
        equiv_in_pred_presentation(p,
    ...
    decreases factors.len(),
{
    if factors.len() == 0 {
        ...
        crate::pred_presentation::lemma_pred_equiv_refl(p, concat_all(rif));
        //  Explicitly assert the postcondition using bound variables:
        assert(equiv_in_pred_presentation(p, concat_all(rif), inverse_word(ca)));
        return;
    } else {
        let rest = factors.drop_first();
        let first = factors.first();
...
pub proof fn lemma_subgroup_inverse(
    ...
    ensures
        in_generated_subgroup_pred(p, gens, inverse_word(w)),
{
    let n = p.num_generators;
    let fa: Seq<Word> = choose|fa: Seq<Word>|
        #[trigger] factors_from_generators(gens, fa)
        && equiv_in_pred_presentation(p, concat_all(fa), w);
    ...
    //  in_generated_subgroup_pred(concat_all(rif)): rif satisfies factors_from_generators, and
    //  concat_all(rif) ≡ concat_all(rif) by reflexivity.
    crate::pred_presentation::lemma_pred_equiv_refl(p, concat_all(rif));
    assert(in_generated_subgroup_pred(p, gens, concat_all(rif)));
    //  + equiv(concat_all(rif), inv(w)) → in_generated_subgroup_pred(inv(w))
...
///  same_left_coset_pred is symmetric.
proof fn lemma_same_left_coset_symmetric(
    data: PredAmalgamatedData, g1: Word, g2: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        same_left_coset_pred(data, g1, g2),
        pred_presentation_valid(data.p1),
        word_valid(g1, data.p1.num_generators),
        word_valid(g2, data.p1.num_generators),
    ensures
        same_left_coset_pred(data, g2, g1),
{
    //  same_left_coset_pred(g1, g2) = in_left_subgroup_pred(concat(inv(g1), g2))
    //  same_left_coset_pred(g2, g1) = in_left_subgroup_pred(concat(inv(g2), g1))
    //  concat(inv(g2), g1) = inv(concat(inv(g1), g2)) approximately
    //  inv(concat(inv(g1), g2)) = concat(inv(g2), inv(inv(g1))) = concat(inv(g2), g1)
    //  So: in_left_subgroup_pred(concat(inv(g2), g1)) follows from
    //      in_left_subgroup_pred(concat(inv(g1), g2)) + subgroup inverse closure.

    let diff12 = concat(inverse_word(g1), g2);
    //  in_left_subgroup_pred(diff12) is given.
    ...
    //  in_left_subgroup_pred(diff12) → in_left_subgroup_pred(inv(diff12)) by subgroup inverse
    reveal(pred_presentation_valid);
    ...
    //  in_left_subgroup_pred(inv(diff12))
    //  inv(diff12) =~= diff21, so in_left_subgroup_pred(diff21)
}

///  same_left_coset_pred is transitive.
proof fn lemma_same_left_coset_transitive(
    data: PredAmalgamatedData, g1: Word, g2: Word, g3: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        same_left_coset_pred(data, g1, g2),
        same_left_coset_pred(data, g2, g3),
        pred_presentation_valid(data.p1),
        word_valid(g1, data.p1.num_generators),
        word_valid(g2, data.p1.num_generators),
        word_valid(g3, data.p1.num_generators),
    ensures
        same_left_coset_pred(data, g1, g3),
{
    //  same_left_coset_pred(g1, g2) = in_left_subgroup_pred(inv(g1) * g2)
    //  same_left_coset_pred(g2, g3) = in_left_subgroup_pred(inv(g2) * g3)
    //  same_left_coset_pred(g1, g3) = in_left_subgroup_pred(inv(g1) * g3)
    ...
    //  in_left_subgroup_pred(d12) && in_left_subgroup_pred(d23)
    //  → in_left_subgroup_pred(concat(d12, d23)) by subgroup_concat
    ...
    //  So in_left_subgroup_pred(concat(d12, d23)) and concat(d12, d23) ≡ inv(g1)*g3
    //  → in_left_subgroup_pred(inv(g1)*g3) by equiv closure.
    ...
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(data.p1, g2);
    //  g2 * inv(g2) ≡ ε
    //  concat(d12, d23) = concat(concat(inv(g1), g2), concat(inv(g2), g3))
...
proof fn lemma_no_shorter_coset_word_implies_ge(
    data: PredAmalgamatedData, g: Word, m: nat, k: nat,
)
    requires
...
///  Helper: if same_left_coset_pred(g1, g2), then has_left_coset_word_of_len(g1, l) iff has_...(g2, l).
///  Specifically: has_left_coset_word_of_len(g2, l) → has_left_coset_word_of_len(g1, l).
proof fn lemma_coset_word_transfer(
    data: PredAmalgamatedData, g1: Word, g2: Word, l: nat,
)
    requires
        amalgamated_data_pred_valid(data),
        same_left_coset_pred(data, g1, g2),
...
{
    let w: Word = choose|w: Word| word_valid(w, data.p1.num_generators)
        && same_left_coset_pred(data, g2, w) && w.len() == l;
...
proof fn lemma_no_smaller_coset_lex_implies_ge(
    data: PredAmalgamatedData, g: Word, l: nat, m: nat, k: nat,
)
    requires
...
proof fn lemma_left_min_coset_lex_satisfiable(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p1.num_generators),
    ensures
...
proof fn lemma_coset_lex_transfer(
    data: PredAmalgamatedData, g1: Word, g2: Word, l: nat, r: nat,
)
    requires
        amalgamated_data_pred_valid(data),
        same_left_coset_pred(data, g1, g2),
...
///  Coset invariance: same_left_coset_pred(g1, g2) → left_canonical_rep(g1) =~= left_canonical_rep(g2).
...
pub proof fn lemma_afp_injectivity_textbook(
    ...
    requires
        ...
        word_valid(h_witness, k_size(data)),
        equiv_in_pred_presentation(data.p1, apply_embedding(a_words(data), h_witness), w),
    ensures
        equiv_in_pred_presentation(data.p1, w, empty_word()),
{
...
proof fn lemma_h_witness_transfer(
    data: PredAmalgamatedData, target1: Word, target2: Word, l: nat,
)
    requires
        has_left_h_witness_of_len(data, target1, l),
        equiv_in_pred_presentation(data.p1, target1, target2),
        pred_presentation_valid(data.p1),
    ensures
        has_left_h_witness_of_len(data, target2, l),
{
    let h: Word = choose|h: Word| word_valid(h, k_size(data)) && h.len() == l
        && equiv_in_pred_presentation(data.p1, apply_embedding(a_words(data), h), target1);
    crate::pred_presentation::lemma_pred_equiv_transitive(
...
proof fn lemma_h_witness_rank_transfer(
    data: PredAmalgamatedData, target1: Word, target2: Word, l: nat, r: nat,
)
    requires
...
///  If g1 ≡ g2 in G₁, then same_left_coset_pred(g1, g2).
        ...
        same_left_coset_pred(data, g1, g2),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(data.p1, g1, g2);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(
        data.p1, inverse_word(g1), g2, g1);
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(data.p1, g1);
    crate::pred_presentation::lemma_pred_equiv_transitive(data.p1,
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(data.p1, empty_word());
    //  equiv(ε, inv(g1) · g2) by symmetry
    crate::word::lemma_inverse_word_valid(g1, data.p1.num_generators);
    crate::word::lemma_concat_word_valid(inverse_word(g1), g2, data.p1.num_generators);
    crate::pred_presentation::lemma_pred_equiv_symmetric(data.p1,
        concat(inverse_word(g1), g2), empty_word());
    assert(in_generated_subgroup_pred(data.p1, a_words(data), empty_word()));
    lemma_in_subgroup_equiv(data.p1, a_words(data),
    ...
    }),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(data.p1, inverse_word(rep1));
    //  target1 ≡ target2 by equiv_concat
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat(
        ...
        left_h_min_len(data, g1) == left_h_min_len(data, g2),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_symmetric(data.p1, target1, target2);
    lemma_h_witness_transfer(data, target2, target1, l2);
...
proof fn lemma_inv_s_h_prime_equiv_embed_h(
    data: PredAmalgamatedData, s: Symbol, h: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
    ...
    ensures ({
        ...
        equiv_in_pred_presentation(data.p1, product2, embed_h)
    }),
{
    let n1 = data.p1.num_generators;
    let p1 = data.p1;
    ...
    //  product ∈ A (from a_rcoset_rep = ε): derive in_left_subgroup_pred
    lemma_a_rcoset_rep_props(data, product);
    //  same_a_rcoset(product, ε) → in_left_subgroup_pred(concat(product, inv(ε)))
    crate::pred_presentation::lemma_pred_equiv_refl(p1, product);
    lemma_in_subgroup_equiv(p1, a_words(data),
        concat(product, inverse_word(a_rcoset_rep(data, product))),
        product);
    //  in_left_subgroup_pred(product) established
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p1, inv_s_word, embed_h_prime, product);
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p1, concat(inv_s_word, s_word), e, embed_h);
...
proof fn lemma_embed_in_trivial_coset(data: PredAmalgamatedData, h: Word)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        word_valid(h, k_size(data)),
    ensures
        same_left_coset_pred(data, apply_embedding(a_words(data), h), empty_word()),
        left_canonical_rep(data, apply_embedding(a_words(data), h)) =~= empty_word(),
{
    ...
    //  same_left_coset_pred(embed_h, ε) = in_left_subgroup_pred(concat(inv(embed_h), ε))
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p1, inverse_word(embed_h));
    lemma_in_subgroup_equiv(p1, a_words(data),
...
proof fn lemma_subgroup_rcoset_restore(
    data: PredAmalgamatedData, product2: Word, h: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        ...
        equiv_in_pred_presentation(data.p1, product2, apply_embedding(a_words(data), h)),
        ...
        a_rcoset_h(data, product2) =~= h,
{
    ...
    lemma_apply_embedding_in_subgroup(p1, a_words(data), h);
    crate::pred_presentation::lemma_pred_equiv_symmetric(p1, product2, embed_h);
        ...
        word_valid(a_rcoset_h(data, g), k_size(data)),
{
    ...
    lemma_a_rcoset_rep_props(data, g);
    crate::benign::lemma_apply_embedding_valid(a_words(data), h, n1);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p1, embed_h, target, rep);
...
proof fn lemma_inv_s_rcoset_product_equiv(
    ...
    ensures ({
        ...
        &&& equiv_in_pred_presentation(data.p1, full, embed_h)
        &&& word_valid(h_prime, k_size(data))
    }),
{
    let n1 = data.p1.num_generators;
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(
...
proof fn lemma_subgroup_left_cancel(
    p: PredPresentation, gens: Seq<Word>, x: Word, y: Word,
)
    requires
        pred_presentation_valid(p),
        word_valid(x, p.num_generators),
        word_valid(y, p.num_generators),
        in_generated_subgroup_pred(p, gens, x),
        in_generated_subgroup_pred(p, gens, concat(x, y)),
        forall|i: int| 0 <= i < gens.len() ==> word_valid(#[trigger] gens[i], p.num_generators),
    ensures
        in_generated_subgroup_pred(p, gens, y),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(p, x);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p, concat(inverse_word(x), x), empty_word(), y);
    ...
    }),
{
    ...
    if a_rcoset_rep(data, product_inv) =~= empty_word() {
        ...
        crate::pred_presentation::lemma_pred_equiv_refl(p1, product_inv);
        lemma_in_subgroup_equiv(p1, a_words(data),
        ...
        lemma_apply_embedding_in_subgroup(p1, a_words(data), h);
        crate::word::lemma_concat_word_valid(concat(inv_s_word, embed_h_prime), rep_prime, n1);
        crate::pred_presentation::lemma_pred_equiv_symmetric(p1, full, embed_h);
        ...
        crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(p1, rep_prime);
        crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(
...
proof fn lemma_act_left_sym_merge_absorbed(
    data: PredAmalgamatedData, s: Symbol, h: Word, syllables: Seq<Syllable>,
)
    requires
        !(a_rcoset_rep(data,
            concat(Seq::new(1, |_i: int| s), apply_embedding(a_words(data), h)))
...
proof fn lemma_four_part_cancel(
    p: PredPresentation, a: Word, b: Word, c: Word,
)
    requires
        pred_presentation_valid(p),
        word_valid(a, p.num_generators),
            ...
            concat(a, c)),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p, concat(inverse_word(b), concat(b, c)));
        ...
        has_a_rcoset_word_of_len(data, g2, l),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(data.p1, concat(g2, inverse_word(g1)));
    lemma_in_subgroup_equiv(data.p1, a_words(data),
        ...
        has_a_rcoset_word_of_len_rank(data, g2, l, r),
{
    ...
    //  Actually, we just need: same_a_rcoset(g2, w) = in_left_subgroup_pred(g2·inv(w))
...
proof fn lemma_inv_s_s_cancel(
    p: PredPresentation, s: Symbol, w: Word,
)
    requires
        pred_presentation_valid(p),
        word_valid(w, p.num_generators),
            ...
            w),
{
    ...
    //  [inv(s)]·[s] ≡ ε
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(p, s_word);
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p, concat(inv_s_word, concat(s_word, w)));
    crate::pred_presentation::lemma_pred_equiv_transitive(p,
...
proof fn lemma_inv_s_rcoset_merge_equiv(
    data: PredAmalgamatedData, s: Symbol, h: Word, c1: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
    ...
    ensures ({
        ...
        &&& equiv_in_pred_presentation(data.p1, full, concat(embed_h, c1))
...
proof fn lemma_rcoset_decompose_subgroup_times_rep(
    data: PredAmalgamatedData, h: Word, c1: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        ...
        a_rcoset_h(data, concat(apply_embedding(a_words(data), h), c1)) =~= h,
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p1, c1);
        ...
        same_a_rcoset(data, g1, g2),
{
    ...
    crate::word::lemma_inverse_word_valid(g1, n1);
    crate::word::lemma_inverse_word_valid(g2, n1);
    crate::pred_presentation::lemma_pred_equiv_symmetric(p1, g1, g2);
    lemma_equiv_inverse(p1, g2, g1);
    crate::pred_presentation::lemma_pred_equiv_refl(p1, g1);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat(p1,
        g1, g1, inverse_word(g2), inverse_word(g1));
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p1, g1);
    ...
    lemma_identity_in_generated_subgroup_pred(p1, a_words(data));
    crate::word::lemma_concat_word_valid(g1, inverse_word(g2), n1);
    crate::pred_presentation::lemma_pred_equiv_symmetric(p1, concat(g1, inverse_word(g2)), empty_word());
    lemma_in_subgroup_equiv(p1, a_words(data),
...
proof fn lemma_a_rcoset_h_from_equiv(
    data: PredAmalgamatedData, g: Word, h: Word, c: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        ...
        word_valid(h, k_size(data)),
        word_valid(c, data.p1.num_generators),
        equiv_in_pred_presentation(data.p1, g, concat(apply_embedding(a_words(data), h), c)),
        left_h_part(data, apply_embedding(a_words(data), h)) =~= h,
        a_rcoset_rep(data, c) =~= c,
        ...
        a_rcoset_h(data, g) =~= h,
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p1, g, concat(embed_h, c), inverse_word(c));
    ...
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p1, c);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p1, embed_h,
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p1, concat(concat(embed_h, c), inverse_word(c)));
    crate::pred_presentation::lemma_pred_equiv_transitive(p1,
    ...
    lemma_apply_embedding_in_subgroup(p1, a_words(data), h);
    crate::pred_presentation::lemma_pred_equiv_symmetric(p1, concat(g, inverse_word(c)), embed_h);
...
pub proof fn lemma_right_cancel(p: PredPresentation, a: Word, b: Word)
    requires
        pred_presentation_valid(p),
        word_valid(a, p.num_generators),
        word_valid(b, p.num_generators),
    ensures
        equiv_in_pred_presentation(p, concat(concat(a, b), inverse_word(b)), a),
{
    crate::word::lemma_inverse_word_valid(b, p.num_generators);
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p, b);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p, a,
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p, concat(a, concat(b, inverse_word(b))));
    //  concat(a, concat(b, inv(b))) ≡ concat(a, ε) =~= a. Two-step chain.
}

///  Idempotency: a_rcoset_rep(a_rcoset_rep(g)) =~= a_rcoset_rep(g).
pub proof fn lemma_a_rcoset_rep_idempotent(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        word_valid(g, data.p1.num_generators),
...
proof fn lemma_c2_inverse_merge_step(
    data: PredAmalgamatedData, s: Symbol, h: Word, c1: Word, combined_h: Word,
            ...
            apply_embedding(a_words(data), h)),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p1, full_inv, concat(embed_h, c1), inverse_word(c1));
...
proof fn lemma_b_rcoset_rep_satisfiable(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p2.num_generators),
    ensures
        ...
        is_min_b_rcoset_lex(data, g, b_rcoset_min_len(data, g), b_rcoset_min_lex(data, g)),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(data.p2, g);
    lemma_identity_in_generated_subgroup_pred(data.p2, b_words(data));
    crate::pred_presentation::lemma_pred_equiv_symmetric(data.p2, concat(g, inverse_word(g)), empty_word());
...
pub proof fn lemma_b_rcoset_rep_props(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        word_valid(g, data.p2.num_generators),
    ensures
...
pub proof fn lemma_b_rcoset_in_subgroup(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p2),
        word_valid(g, data.p2.num_generators),
        in_right_subgroup_pred(data, g),
    ensures
        b_rcoset_rep(data, g) =~= empty_word(),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(data.p2, g);
        ...
        same_b_rcoset(data, g2, g1),
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(data.p2, concat(g2, inverse_word(g1)));
    lemma_in_subgroup_equiv(data.p2, b_words(data),
        inv_pair, concat(g2, inverse_word(g1)));
}
        ...
        same_b_rcoset(data, g1, g2),
{
    ...
    crate::word::lemma_inverse_word_valid(g1, n2);
    crate::word::lemma_inverse_word_valid(g2, n2);
    crate::pred_presentation::lemma_pred_equiv_symmetric(p2, g1, g2);
    lemma_equiv_inverse(p2, g2, g1);
    crate::pred_presentation::lemma_pred_equiv_refl(p2, g1);
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat(p2,
        g1, g1, inverse_word(g2), inverse_word(g1));
    crate::pred_presentation_lemmas::lemma_pred_word_inverse_right(p2, g1);
    ...
    lemma_identity_in_generated_subgroup_pred(p2, b_words(data));
    crate::word::lemma_concat_word_valid(g1, inverse_word(g2), n2);
    crate::pred_presentation::lemma_pred_equiv_symmetric(p2, concat(g1, inverse_word(g2)), empty_word());
    lemma_in_subgroup_equiv(p2, b_words(data),
        empty_word(), concat(g1, inverse_word(g2)));
}
...
pub proof fn lemma_b_rcoset_rep_idempotent(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p2),
        word_valid(g, data.p2.num_generators),
        ...
        word_valid(b_rcoset_h(data, g), k_size(data)),
{
    ...
    let l = b_rcoset_h_min_len(data, g);
    let w: Word = choose|w: Word| word_valid(w, k_size(data)) && w.len() == l
        && equiv_in_pred_presentation(p2, apply_embedding(b_words(data), w), target);
    let wr = word_lex_rank_base(w, h_lex_base(data));
    assert(has_right_h_witness_of_len_rank(data, target, l, wr));
    assert(no_smaller_h_lex_g2(data, target, l, 0nat));
    lemma_scan_min_h_lex_g2(data, target, l, 0, wr);
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p2, embed_h, target, rep);
    crate::word::lemma_inverse_word_valid(rep, n2);
    assert(concat(concat(g, inverse_word(rep)), rep) =~=
           concat(g, concat(inverse_word(rep), rep))) by {
        let lhs = concat(concat(g, inverse_word(rep)), rep);
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p2, concat(g, concat(inverse_word(rep), rep)));
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(p2, g,
        concat(inverse_word(rep), rep), empty_word());
    assert(concat(g, empty_word()) =~= g) by {
        assert(concat(g, empty_word()).len() == g.len());
        assert forall|k: int| 0 <= k < g.len()
...
proof fn lemma_inv_s_s_cancel_g2(
    data: PredAmalgamatedData, s: Symbol, w: Word,
)
    requires
        pred_presentation_valid(data.p2),
        word_valid(w, data.p2.num_generators),
...
proof fn lemma_inv_s_rcoset_product_equiv_g2(
    ...
    ensures ({
        ...
        let full = concat(concat(Seq::new(1, |_i: int| inverse_symbol(s)), embed_h_prime), rep_prime);
        &&& equiv_in_pred_presentation(data.p2, full, embed_h)
        &&& word_valid(h_prime, k_size(data))
    }),
{
    let n2 = data.p2.num_generators;
...
proof fn lemma_inv_s_rcoset_merge_equiv_g2(
    ...
    ensures ({
        ...
        let inv_s_word = Seq::new(1, |_i: int| inverse_symbol(s));
        let full = concat(concat(inv_s_word, embed_ch), merged_rep);
        &&& equiv_in_pred_presentation(data.p2, full, concat(embed_h, c1))
        &&& word_valid(combined_h, k_size(data))
    }),
{
    let n2 = data.p2.num_generators;
...
proof fn lemma_h_witness_transfer_g2(
    ...
    requires
        ...
        equiv_in_pred_presentation(data.p2, target1, target2),
        pred_presentation_valid(data.p2),
    ensures
        has_right_h_witness_of_len(data, target2, l),
{
    let h: Word = choose|h: Word| word_valid(h, k_size(data)) && h.len() == l
        && equiv_in_pred_presentation(data.p2, apply_embedding(b_words(data), h), target1);
    crate::pred_presentation::lemma_pred_equiv_transitive(
...
pub proof fn lemma_h_witness_exists_g2(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p2),
        word_valid(g, data.p2.num_generators),
...
proof fn lemma_b_rcoset_h_from_equiv(
    ...
    requires
        ...
        word_valid(h, k_size(data)),
        word_valid(c, data.p2.num_generators),
        equiv_in_pred_presentation(data.p2, g, concat(apply_embedding(b_words(data), h), c)),
        ...
        b_rcoset_h(data, g) =~= h,
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p2, g, concat(embed_h, c), inverse_word(c));
    ...
    lemma_apply_embedding_in_subgroup_g2(p2, b_words(data), h);
    crate::pred_presentation::lemma_pred_equiv_symmetric(p2, concat(g, inverse_word(c)), embed_h);
...
proof fn lemma_subgroup_rcoset_restore_g2(
    ...
    requires
        ...
        equiv_in_pred_presentation(data.p2, product2, apply_embedding(b_words(data), h)),
        is_canonical_state(data, h, Seq::<Syllable>::empty()),
    ensures
        ...
        b_rcoset_h(data, product2) =~= h,
{
    ...
    //  product2 ∈ B-subgroup
    lemma_apply_embedding_in_subgroup_g2(p2, b_words(data), h);
    crate::pred_presentation::lemma_pred_equiv_symmetric(p2, product2, embed_h_b);
        ...
        b_rcoset_h(data, concat(apply_embedding(b_words(data), h), c1)) =~= h,
{
    ...
    crate::pred_presentation::lemma_pred_equiv_refl(p2, product);
    ...
    }),
{
    ...
    if b_rcoset_rep(data, product_inv) =~= empty_word() {
        ...
        crate::pred_presentation::lemma_pred_equiv_refl(p2, product_inv);
        lemma_in_subgroup_equiv(p2, b_words(data),
        ...
        crate::pred_presentation::lemma_pred_equiv_symmetric(p2, full, embed_h);
        ...
        crate::pred_presentation_lemmas::lemma_pred_word_inverse_left(p2, rep_prime);
        crate::pred_presentation_lemmas::lemma_pred_equiv_concat_right(
            ...
            apply_embedding(b_words(data), h)),
{
    ...
    crate::pred_presentation_lemmas::lemma_pred_equiv_concat_left(p2, full_inv, concat(embed_h, c1), inverse_word(c1));
