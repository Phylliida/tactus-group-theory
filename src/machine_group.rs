...
//  ============================================================
//  General HNN pinch-out: delete a pinch, mapping the middle through the iso
//  (forward telescope for t⁻¹·g·t, reverse for t·g·t⁻¹).  The replacement is a
//  BASE word (no stable letter) — so the stable count drops.
//  ============================================================
pub proof fn lemma_hnn_pinch_out(data: HNNData, w: Word, i: int, j: int)
    requires
        hnn_data_valid(data),
        word_valid(w, hnn_presentation(data).num_generators),
    ...
    ensures
        exists|new_mid: Word|
            word_valid(new_mid, data.base.num_generators)
            && #[trigger] equiv_in_presentation(hnn_presentation(data), w,
                 w.subrange(0, i) + new_mid + w.subrange(j + 1, w.len() as int)),
{
    let p = hnn_presentation(data);
    let ng = data.base.num_generators;
    let png = p.num_generators;
    let st = stable_letter(data);
    let stinv = stable_letter_inv(data);
    let si = w[i];
    let sj = w[j];
    let u = w.subrange(i + 1, j);
    let ag = hnn_a_gens(data);
    ...
    assert(has_adjacent_opposite_at(data, w, i, j));
    assert(png == ng + 1);
    assert(st == Symbol::Gen(ng) && stinv == Symbol::Inv(ng));
    assert(ag =~= Seq::new(k, |t: int| data.associations[t].0));
    assert(bg =~= Seq::new(k, |t: int| data.associations[t].1));
    assert(word_valid(u, ng)) by {
        assert forall|t: int| 0 <= t < u.len() implies symbol_valid(#[trigger] u[t], ng)
        by { assert(u[t] == w[i + 1 + t]); assert(symbol_valid(w[i + 1 + t], png));
             assert(!is_stable(data, w[i + 1 + t])); }
    }
    let mid = w.subrange(i, j + 1);
    assert(mid =~= seq![si] + u + seq![sj]);
    let pre = w.subrange(0, i);
    let post = w.subrange(j + 1, w.len() as int);
    assert(w =~= pre + mid + post);
    //  generic-gens validity: ag[t]/bg[t] are valid base words
    assert forall|t: int| 0 <= t < k implies (word_valid(#[trigger] ag[t], ng) && word_valid(bg[t], ng))
    by { assert(ag[t] == data.associations[t].0 && bg[t] == data.associations[t].1); }
    if si == st {
        //  t · g · t⁻¹ with g ∈ ⟨b_gens⟩  ⟹  ≡ emb(a_gens, uw)  (reverse telescope)
        assert(sj == stinv && in_generated_subgroup(data.base, bg, u));
        lemma_in_gen_implies_emb(data.base, bg, u);
        let uw = choose|uw: Word| word_valid(uw, bg.len() as nat)
            && equiv_in_presentation(data.base, apply_embedding(bg, uw), u);
        assert(word_valid(uw, k as nat) && equiv_in_presentation(data.base, apply_embedding(bg, uw), u));
        let g = apply_embedding(bg, uw);
        let new_mid = apply_embedding(ag, uw);
        lemma_apply_embedding_valid(bg, uw, ng);
        lemma_apply_embedding_valid(ag, uw, ng);
        lemma_word_valid_mono(g, ng, png);
        lemma_word_valid_mono(new_mid, ng, png);
        lemma_base_embeds_in_hnn(data, g, u);                 //  g ≡_p u
        lemma_equiv_symmetric(p, g, u);                       //  u ≡_p g
        lemma_stable_conj_factorization_rev(data, uw);        //  [st]·g·[stinv] ≡ new_mid
        //  mid = [si]·u·[sj] ≡ [si]·g·[sj] ≡ new_mid
        lemma_equiv_concat_right(p, seq![si], u, g);
        lemma_equiv_concat_left(p, seq![si] + u, seq![si] + g, seq![sj]);
        assert((seq![si] + u) + seq![sj] =~= mid);
        assert((seq![si] + g) + seq![sj] =~= seq![st] + g + seq![stinv]);
        lemma_equiv_transitive(p, mid, (seq![si] + g) + seq![sj], new_mid);
        lemma_pinch_assemble(p, w, i, j, new_mid);
        assert(word_valid(new_mid, ng)
            && equiv_in_presentation(p, w, pre + new_mid + post));
    } else {
        //  t⁻¹ · g · t with g ∈ ⟨a_gens⟩  ⟹  ≡ emb(b_gens, uw)  (forward telescope)
        assert(si == stinv && sj == st && in_generated_subgroup(data.base, ag, u));
        lemma_in_gen_implies_emb(data.base, ag, u);
        let uw = choose|uw: Word| word_valid(uw, ag.len() as nat)
            && equiv_in_presentation(data.base, apply_embedding(ag, uw), u);
        assert(word_valid(uw, k as nat) && equiv_in_presentation(data.base, apply_embedding(ag, uw), u));
        let g = apply_embedding(ag, uw);
        let new_mid = apply_embedding(bg, uw);
        lemma_apply_embedding_valid(ag, uw, ng);
        lemma_apply_embedding_valid(bg, uw, ng);
        lemma_word_valid_mono(g, ng, png);
        lemma_word_valid_mono(new_mid, ng, png);
        lemma_base_embeds_in_hnn(data, g, u);                 //  g ≡_p u
        lemma_equiv_symmetric(p, g, u);                       //  u ≡_p g
        lemma_stable_conj_factorization(data, uw);            //  [stinv]·g·[st] ≡ new_mid
        lemma_equiv_concat_right(p, seq![si], u, g);
        lemma_equiv_concat_left(p, seq![si] + u, seq![si] + g, seq![sj]);
        assert((seq![si] + u) + seq![sj] =~= mid);
        assert((seq![si] + g) + seq![sj] =~= seq![stinv] + g + seq![st]);
        lemma_equiv_transitive(p, mid, (seq![si] + g) + seq![sj], new_mid);
        lemma_pinch_assemble(p, w, i, j, new_mid);
        assert(word_valid(new_mid, ng)
            && equiv_in_presentation(p, w, pre + new_mid + post));
...
//  Assembly: w ≡ pre · new_mid · post  given  w[i..j+1] ≡ new_mid.
pub proof fn lemma_pinch_assemble(p: Presentation, w: Word, i: int, j: int, new_mid: Word)
    requires
        presentation_valid(p),
        word_valid(w, p.num_generators),
        word_valid(new_mid, p.num_generators),
        0 <= i < j < w.len(),
        equiv_in_presentation(p, w.subrange(i, j + 1), new_mid),
    ensures
        equiv_in_presentation(p, w,
            w.subrange(0, i) + new_mid + w.subrange(j + 1, w.len() as int)),
{
    let mid = w.subrange(i, j + 1);
    let pre = w.subrange(0, i);
    let post = w.subrange(j + 1, w.len() as int);
    assert(word_valid(pre, p.num_generators)) by {
        assert forall|t: int| 0 <= t < pre.len() implies symbol_valid(#[trigger] pre[t], p.num_generators)
        by { assert(pre[t] == w[t]); }
    }
    assert(word_valid(mid, p.num_generators)) by {
        assert forall|t: int| 0 <= t < mid.len() implies symbol_valid(#[trigger] mid[t], p.num_generators)
        by { assert(mid[t] == w[i + t]); }
    }
    assert(word_valid(post, p.num_generators)) by {
        assert forall|t: int| 0 <= t < post.len() implies symbol_valid(#[trigger] post[t], p.num_generators)
        by { assert(post[t] == w[j + 1 + t]); }
    }
    assert(w =~= pre + mid + post);
    lemma_equiv_concat_right(p, pre, mid, new_mid);
    lemma_equiv_concat_left(p, pre + mid, pre + new_mid, post);
    assert((pre + mid) + post =~= w);
...
//  ============================================================
//  Signed config-conjugation:  x⁻ᴾ · sconfig(r,s) · xᴾ ≡ sconfig(r+P, s)
//  (the property-(ii) accumulator's per-config move).  Split prefix/suffix to
//  stay under rlimit.
//  ============================================================

//  Prefix:  x⁻ᴾ · y⁻ˢ · x⁻ʳ  ≡  y⁻ˢ · x⁻⁽ʳ⁺ᴾ⁾.
pub proof fn lemma_sconfig_prefix_conj(r: int, s: int, pp: int)
    ensures
        equiv_in_presentation(base_A(),
            signed_power(1, -pp) + signed_power(2, -s) + signed_power(1, -r),
            signed_power(2, -s) + signed_power(1, -(r + pp))),
{
    let a = base_A();
    lemma_base_A_valid();
    assert(presentation_valid(a)) by { reveal(presentation_valid); }
    ...
    let xmrp = signed_power(1, -(r + pp));
    lemma_signed_xy_commute(-pp, -s);                 //  xmP·yms ≡ yms·xmP
    lemma_signed_power_add(a, 1, -pp, -r);            //  xmP·xmr ≡ x^(-pp-r)
    assert(signed_power(1, -pp + -r) == xmrp) by { assert(-pp + -r == -(r + pp)); }
    let lhs = xmP + yms + xmr;
    let mid = yms + xmP + xmr;
    let rhs = yms + xmrp;
    assert(equiv_in_presentation(a, lhs, mid)) by {
        lemma_equiv_concat_left(a, xmP + yms, yms + xmP, xmr);
        assert(lhs =~= (xmP + yms) + xmr);
        assert(mid =~= (yms + xmP) + xmr);
    }
    assert(equiv_in_presentation(a, mid, rhs)) by {
        lemma_equiv_concat_right(a, yms, xmP + xmr, xmrp);
        assert(mid =~= yms + (xmP + xmr));
        assert(rhs =~= yms + xmrp);
    }
...
//  Suffix:  xʳ · yˢ · xᴾ  ≡  x⁽ʳ⁺ᴾ⁾ · yˢ.
pub proof fn lemma_sconfig_suffix_conj(r: int, s: int, pp: int)
    ensures
        equiv_in_presentation(base_A(),
            signed_power(1, r) + signed_power(2, s) + signed_power(1, pp),
            signed_power(1, (r + pp)) + signed_power(2, s)),
{
    let a = base_A();
    lemma_base_A_valid();
    assert(presentation_valid(a)) by { reveal(presentation_valid); }
    ...
    let xrp = signed_power(1, (r + pp));
    lemma_signed_xy_commute(pp, s);                   //  xP·ys ≡ ys·xP
    lemma_equiv_symmetric(a, xP + ys, ys + xP);        //  ys·xP ≡ xP·ys
    lemma_signed_power_add(a, 1, r, pp);              //  xr·xP ≡ x^(r+pp)
    let lhs = xr + ys + xP;
    let mid = xr + xP + ys;
    let rhs = xrp + ys;
    assert(equiv_in_presentation(a, lhs, mid)) by {
        lemma_equiv_concat_right(a, xr, ys + xP, xP + ys);
        assert(lhs =~= xr + (ys + xP));
        assert(mid =~= xr + (xP + ys));
    }
    assert(equiv_in_presentation(a, mid, rhs)) by {
        lemma_equiv_concat_left(a, xr + xP, xrp, ys);
        assert(mid =~= (xr + xP) + ys);
        assert(rhs =~= xrp + ys);
    }
    lemma_equiv_transitive(a, lhs, mid, rhs);
...
pub proof fn lemma_sconfig_conj_x(r: int, s: int, pp: int)
    ensures
        equiv_in_presentation(base_A(),
            signed_power(1, -pp) + sconfig(r, s) + signed_power(1, pp),
            sconfig((r + pp), s)),
{
    let a = base_A();
    lemma_base_A_valid();
    ...
    lemma_sconfig_prefix_conj(r, s, pp);
    lemma_sconfig_suffix_conj(r, s, pp);
    let lhs = xmP + sconfig(r, s) + xP;
    let rhs = sconfig((r + pp), s);
    assert(lhs =~= preL + (t0 + sufR));
    assert(rhs =~= preL2 + (t0 + sufR2));
    let m1 = preL2 + (t0 + sufR);
    assert(equiv_in_presentation(a, lhs, m1)) by {
        lemma_equiv_concat_left(a, preL, preL2, t0 + sufR);
        assert(lhs =~= preL + (t0 + sufR));
        assert(m1 =~= preL2 + (t0 + sufR));
    }
    assert(equiv_in_presentation(a, m1, rhs)) by {
        lemma_equiv_concat_right(a, preL2 + t0, sufR, sufR2);
        assert(m1 =~= (preL2 + t0) + sufR);
        assert(rhs =~= (preL2 + t0) + sufR2);
    }
...
//  Signed config-conjugation by y:  y⁻�Q · sconfig(r,s) · y�Q ≡ sconfig(r, s+Q)
//  (pure merge — the y-powers absorb directly into y⁻ˢ / yˢ; no commute).
pub proof fn lemma_sconfig_conj_y(r: int, s: int, qq: int)
    ensures
        equiv_in_presentation(base_A(),
            signed_power(2, -qq) + sconfig(r, s) + signed_power(2, qq),
            sconfig(r, (s + qq))),
{
    let a = base_A();
    lemma_base_A_valid();
    ...
    let middle = signed_power(1, -r) + seq![Symbol::Gen(0)] + signed_power(1, r);
    ...
    let lhs = ymQ + sconfig(r, s) + yQ;
    let rhs = sconfig(r, (s + qq));
...
//  ============================================================
//  Predicate-based subgroups (for the infinitely-generated T(M))
//  ============================================================
//
//  T(M) = ⟨ t(a,b) : (a,b) ∈ H₀(M) ⟩ is infinitely generated, so we describe its
//  generating set by a PREDICATE rather than a finite Seq<Word>.

//  All factors satisfy a generator predicate.
pub open spec fn factors_from_pred(pred: spec_fn(Word) -> bool, factors: Seq<Word>) -> bool {
    forall|k: int| #![trigger factors[k]] 0 <= k < factors.len() ==> pred(factors[k])
}

//  Membership in the subgroup generated by { w : pred(w) }  (pred already closed under inverses).
pub open spec fn in_subgroup_pred(p: Presentation, pred: spec_fn(Word) -> bool, w: Word) -> bool {
    exists|factors: Seq<Word>| #[trigger] factors_from_pred(pred, factors)
        && equiv_in_presentation(p, concat_all(factors), w)
}

//  The predicate-generated subgroup is closed under products.
pub proof fn lemma_product_in_subgroup_pred(p: Presentation, pred: spec_fn(Word) -> bool, w1: Word, w2: Word)
    requires
        in_subgroup_pred(p, pred, w1),
        in_subgroup_pred(p, pred, w2),
    ensures
        in_subgroup_pred(p, pred, w1 + w2),
{
    let f1 = choose|f: Seq<Word>| factors_from_pred(pred, f) && equiv_in_presentation(p, concat_all(f), w1);
    let f2 = choose|f: Seq<Word>| factors_from_pred(pred, f) && equiv_in_presentation(p, concat_all(f), w2);
    assert(factors_from_pred(pred, f1) && equiv_in_presentation(p, concat_all(f1), w1));
    assert(factors_from_pred(pred, f2) && equiv_in_presentation(p, concat_all(f2), w2));
    let f = f1 + f2;
    assert(factors_from_pred(pred, f)) by {
        assert forall|k: int| 0 <= k < f.len() implies pred(#[trigger] f[k]) by {
            if k < f1.len() { assert(f[k] == f1[k]); } else { assert(f[k] == f2[k - f1.len()]); }
        }
    }
    lemma_concat_all_distributes(f1, f2);
    ...
    lemma_equiv_transitive(p, concat_all(f1) + concat_all(f2), w1 + concat_all(f2), w1 + w2);
    assert(concat_all(f) =~= concat_all(f1) + concat_all(f2));
    assert(in_subgroup_pred(p, pred, w1 + w2)) by {
        assert(factors_from_pred(pred, f) && equiv_in_presentation(p, concat_all(f), w1 + w2));
    }
}

//  Membership respects equivalence.
pub proof fn lemma_in_subgroup_pred_respects_equiv(p: Presentation, pred: spec_fn(Word) -> bool, v: Word, u: Word)
    requires
        in_subgroup_pred(p, pred, v),
        equiv_in_presentation(p, v, u),
    ensures
        in_subgroup_pred(p, pred, u),
{
    let f = choose|f: Seq<Word>| factors_from_pred(pred, f) && equiv_in_presentation(p, concat_all(f), v);
    assert(factors_from_pred(pred, f) && equiv_in_presentation(p, concat_all(f), v));
    lemma_equiv_transitive(p, concat_all(f), v, u);
    assert(in_subgroup_pred(p, pred, u)) by {
        assert(factors_from_pred(pred, f) && equiv_in_presentation(p, concat_all(f), u));
    }
}

//  A single generator (a word satisfying the predicate) is in the subgroup.
pub proof fn lemma_gen_in_subgroup_pred(p: Presentation, pred: spec_fn(Word) -> bool, g: Word)
    requires
        pred(g),
    ensures
        in_subgroup_pred(p, pred, g),
{
    let f = seq![g];
    assert(factors_from_pred(pred, f)) by {
        assert forall|k: int| 0 <= k < f.len() implies pred(#[trigger] f[k]) by { assert(f[k] == g); }
    }
    lemma_concat_all_singleton(g);
    lemma_equiv_refl(p, g);
    assert(in_subgroup_pred(p, pred, g)) by {
        assert(factors_from_pred(pred, f) && equiv_in_presentation(p, concat_all(f), g));
    }
}
