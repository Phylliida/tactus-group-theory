//  Britton's Lemma via Tower Construction — PREDICATE-BASE PORT (FA-9b).
//
//  Port of `britton_via_tower.rs` over a `PredHNNData`/`PredPresentation` base
//  (relators = `spec_fn(Word)->bool`).  Translates HNN extension derivations to
//  tower derivations.  Faithful to Lyndon-Schupp Chapter IV: the tower unfolds
//  the HNN extension by replacing the stable letter with explicit copies of the
//  base group.
//
...
//
//  Cayley LEAPFROG: consumes pred_tower's TEXTBOOK chain only (the heavy AFP
//  coset/van-der-Waerden machinery — a_words/b_words/rcoset_rep/k_size/... — is
//  already ported in FA-8's pred_normal_form_afp_textbook).  The relator-by-INDEX
//  friction (`DerivationStep::RelatorInsert{relator_index}` →
//  `PredDerivationStep::RelatorInsert{relator: Word}`, guard `idx<relators.len()`
//  → `(p.relators)(word)`) is hand-rewritten in ~14 translation-core lemmas.
...
use crate::benign::*;
use crate::pred_hnn::*;
use crate::pred_tower::*;
use crate::pred_normal_form_afp_textbook::Syllable;

verus! {

//  ============================================================
//  Local helper: inverse of a trivial word is trivial.
//  Predicate-base port of `normal_form_amalgamated::lemma_inverse_of_trivial`
//  (no pred analog in pred_normal_form_amalgamated — ported here, the body is
//  pure presentation algebra so the symbol-map carries it 1:1).
//  ============================================================
pub proof fn lemma_pred_inverse_of_trivial(p: PredPresentation, w: Word)
    requires
        pred_presentation_valid(p),
        word_valid(w, p.num_generators),
        equiv_in_pred_presentation(p, w, empty_word()),
    ensures
        equiv_in_pred_presentation(p, inverse_word(w), empty_word()),
{
    lemma_pred_equiv_symmetric(p, w, empty_word());
    lemma_pred_equiv_concat_right(p, inverse_word(w), empty_word(), w);
    assert(concat(inverse_word(w), empty_word()) =~= inverse_word(w));
    lemma_pred_word_inverse_left(p, w);
    lemma_pred_equiv_transitive(p, concat(inverse_word(w), empty_word()),
        concat(inverse_word(w), w), empty_word());
    lemma_pred_equiv_refl(p, inverse_word(w));
    lemma_pred_equiv_transitive(p, inverse_word(w),
        concat(inverse_word(w), empty_word()), empty_word());
}

//  Local port of `coset_group::lemma_cancel_inverse_right` (no pred analog; body
//  is pure presentation algebra, symbol-mapped 1:1).
proof fn lemma_pred_cancel_inverse_right(p: PredPresentation, u: Word, v: Word)
    requires
        equiv_in_pred_presentation(p, concat(u, inverse_word(v)), empty_word()),
        word_valid(u, p.num_generators),
        word_valid(v, p.num_generators),
        pred_presentation_valid(p),
    ensures
        equiv_in_pred_presentation(p, u, v),
{
    let inv_v = inverse_word(v);
    let n = p.num_generators;
    lemma_pred_equiv_concat_left(p, concat(u, inv_v), empty_word(), v);
    lemma_pred_concat_identity_left(p, v);
    lemma_pred_equiv_transitive(p, concat(concat(u, inv_v), v), concat(empty_word(), v), v);
    crate::word::lemma_concat_assoc(u, inv_v, v);
    lemma_pred_word_inverse_left(p, v);
    lemma_pred_equiv_concat_right(p, u, concat(inv_v, v), empty_word());
    lemma_pred_concat_identity_right(p, u);
    lemma_pred_equiv_transitive(p, concat(u, concat(inv_v, v)), concat(u, empty_word()), u);
    crate::word::lemma_inverse_word_valid(v, n);
    crate::word::lemma_concat_word_valid(inv_v, v, n);
    crate::word::lemma_concat_word_valid(u, concat(inv_v, v), n);
    lemma_pred_equiv_symmetric(p, concat(u, concat(inv_v, v)), u);
    lemma_pred_equiv_transitive(p, u, concat(u, concat(inv_v, v)), v);
}

//  Local wrapper for `britton_infra::lemma_step_preserves_word_valid` (HNN-specific;
//  routes through the presentation-level `lemma_pred_step_preserves_word_valid`).
proof fn lemma_hnn_step_preserves_word_valid(data: PredHNNData, w: Word, step: PredDerivationStep)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
        apply_step_pred(hnn_pred_presentation(data), w, step) is Some,
    ensures
        word_valid(
            apply_step_pred(hnn_pred_presentation(data), w, step).unwrap(),
            hnn_pred_presentation(data).num_generators),
{
    let hp = hnn_pred_presentation(data);
    lemma_hnn_pred_presentation_valid(data);
    let w_next = apply_step_pred(hp, w, step).unwrap();
    lemma_pred_step_preserves_word_valid(hp, w, step, w_next);
}
...
pub open spec fn is_stable(data: PredHNNData, s: Symbol) -> bool {
    let ng = data.base.num_generators;
    s == Symbol::Gen(ng) || s == Symbol::Inv(ng)
}

///  Net level change of a word: count of t minus count of t⁻¹.
pub open spec fn net_level(data: PredHNNData, w: Word) -> int
    decreases w.len(),
{
    let ng = data.base.num_generators;
    if w.len() == 0 {
...
pub open spec fn translate_word_at(data: PredHNNData, w: Word, base_level: int) -> Word
    decreases w.len(),
{
    let ng = data.base.num_generators;
    if w.len() == 0 {
...
pub open spec fn translate_word(data: PredHNNData, w: Word) -> Word {
...
proof fn lemma_base_word_net_level_zero(data: PredHNNData, w: Word)
    requires
        hnn_pred_data_valid(data),
...
pub proof fn lemma_translate_base_word(data: PredHNNData, w: Word)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, data.base.num_generators),
    ensures
...
pub proof fn lemma_translate_empty(data: PredHNNData)
...
pub proof fn lemma_translate_concat(data: PredHNNData, w1: Word, w2: Word, base_level: int)
    ensures
...
//  ============================================================
//  Part D: PredDerivation lifting — equiv in p1 → equiv in free_product_pred(p1, p2)
//  ============================================================

///  A single derivation step valid in p1 is also valid in free_product_pred(p1, p2).
proof fn lemma_step_valid_in_fp_left(
    p1: PredPresentation, p2: PredPresentation,
    w: Word, step: PredDerivationStep,
)
    requires
        apply_step_pred(p1, w, step) is Some,
    ensures
        apply_step_pred(free_product_pred(p1, p2), w, step) == apply_step_pred(p1, w, step),
{
    ...
    match step {
        ...
        PredDerivationStep::RelatorInsert { position, relator, inverted } => {
            //  apply_step_pred(p1,..) is Some ⟹ (p1.relators)(relator);
            //  then (fp.relators)(relator) holds via the left disjunct.
            assert((p1.relators)(relator));
            assert((fp.relators)(relator) == free_product_pred_relators(p1, p2, relator));
        },
...
///  A full derivation valid in p1 is also valid in free_product_pred(p1, p2).
    ...
    decreases steps.len(),
{
    if steps.len() == 0 {
        ...
        let w_mid = apply_step_pred(p1, w1, steps.first()).unwrap();
        lemma_step_valid_in_fp_left(p1, p2, w1, steps.first());
        lemma_derivation_valid_in_fp_left(p1, p2, steps.drop_first(), w_mid, w2);
    }
}

///  Equivalence in p1 implies equivalence in free_product_pred(p1, p2).
...
{
    let d: PredDerivation = choose|d: PredDerivation| pred_derivation_valid(p1, d, w1, w2);
...
pub proof fn lemma_tower_monotone(
    data: PredHNNData, k: nat, m: nat, w1: Word, w2: Word,
)
    requires
        hnn_pred_data_valid(data),
        k <= m,
        equiv_in_pred_presentation(tower_presentation(data, k), w1, w2),
    ensures
        equiv_in_pred_presentation(tower_presentation(data, m), w1, w2),
    decreases m - k,
{
    if k == m {
    } else {
        ...
        crate::pred_amalgamated_free_product::lemma_add_relators_pred_preserves_equiv(
            ...
            amalgamation_relators_pred(afp_data), w1, w2);
...
pub proof fn lemma_base_relator_in_tower(
    data: PredHNNData, m: nat, k: nat, relator: Word,
)
    requires
        hnn_pred_data_valid(data),
        k <= m,
        (data.base.relators)(relator),
    ensures
        equiv_in_pred_presentation(tower_presentation(data, m),
            shift_word(relator, k * data.base.num_generators),
            empty_word()),
    decreases m,
{
    let ng = data.base.num_generators;
    if k == 0 && m == 0 {
        assert(shift_word(relator, 0) =~= relator);
        lemma_pred_relator_is_identity(data.base, relator);
    } else if k < m {
        if k == 0 {
            assert(shift_word(relator, 0) =~= relator);
            lemma_pred_relator_is_identity(data.base, relator);
            lemma_tower_monotone(data, 0, m, relator, empty_word());
        } else {
            lemma_base_relator_in_tower(data, (m - 1) as nat, k, relator);
            lemma_tower_monotone(data, (m - 1) as nat, m,
                shift_word(relator, k * ng), empty_word());
        }
    } else {
        //  k == m > 0: relator in the new copy (right factor of the AFP).
        let prev = (m - 1) as nat;
        let afp_data = tower_afp_data(data, prev);
        let fp = free_product_pred(afp_data.p1, afp_data.p2);
        lemma_tower_num_generators(data, prev);
        assert(afp_data.p1.num_generators == m * ng);
        assert(afp_data.p2 == data.base);
        let shifted_r = shift_word(relator, m * ng);
        //  (fp.relators)(shifted_r) via the right disjunct shifted_pred (witness `relator`).
        assert(shifted_pred(afp_data.p2.relators, afp_data.p1.num_generators, shifted_r)) by {
            assert((afp_data.p2.relators)(relator)
                && shifted_r == shift_word(relator, afp_data.p1.num_generators));
        }
        assert((fp.relators)(shifted_r)
            == free_product_pred_relators(afp_data.p1, afp_data.p2, shifted_r));
        lemma_pred_relator_is_identity(fp, shifted_r);
        crate::pred_amalgamated_free_product::lemma_add_relators_pred_preserves_equiv(
            fp, amalgamation_relators_pred(afp_data),
            shifted_r, empty_word());
    }
}

///  An identification relator at junction k↔k+1 is equiv to ε in tower(m) when k+1 ≤ m.
pub proof fn lemma_ident_relator_in_tower(
    data: PredHNNData, m: nat, k: nat, i: int,
)
    requires
        hnn_pred_data_valid(data),
        k + 1 <= m,
    ...
    ensures
        equiv_in_pred_presentation(tower_presentation(data, m),
            amalgamation_relator_pred(tower_afp_data(data, k), i),
            empty_word()),
    decreases m,
{
    let prev = (m - 1) as nat;
    ...
    if k == prev {
        crate::pred_amalgamated_free_product::lemma_each_added_relator_pred_is_identity(
            fp, amalgamation_relators_pred(afp_data), i);
    } else {
        lemma_ident_relator_in_tower(data, prev, k, i);
        lemma_tower_monotone(data, prev, m,
            amalgamation_relator_pred(tower_afp_data(data, k), i), empty_word());
    }
}
...
pub proof fn lemma_delete_equiv_empty(
    p: PredPresentation, prefix: Word, r: Word, suffix: Word,
)
    requires
        equiv_in_pred_presentation(p, r, empty_word()),
    ensures
        equiv_in_pred_presentation(p, concat(prefix, concat(r, suffix)),
...
///  Requires symmetry infrastructure (word_valid + pred_presentation_valid).
pub proof fn lemma_insert_equiv_empty(
    ...
    ensures
        equiv_in_pred_presentation(p, concat(prefix, suffix),
            concat(prefix, concat(r, suffix))),
{
    //  ε ≡ r (by symmetry)
    crate::pred_presentation::lemma_pred_equiv_symmetric(p, r, empty_word());
    //  concat(ε, suffix) ≡ concat(r, suffix) → suffix ≡ concat(r, suffix)
    lemma_pred_equiv_concat_left(p, empty_word(), r, suffix);
    //  concat(prefix, suffix) ≡ concat(prefix, concat(r, suffix))
    lemma_pred_equiv_concat_right(p, prefix, suffix, concat(r, suffix));
}
...
pub proof fn lemma_translate_base_word_at(data: PredHNNData, w: Word, base_level: nat)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, data.base.num_generators),
    ensures
...
proof fn lemma_translate_stable_empty(data: PredHNNData, s: Symbol, base_level: int)
...
proof fn lemma_net_level_base_word(data: PredHNNData, w: Word)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, data.base.num_generators),
    ensures
...
proof fn lemma_net_level_stable(data: PredHNNData, s: Symbol)
    requires is_stable(data, s),
    ensures
...
pub proof fn lemma_net_level_concat(data: PredHNNData, w1: Word, w2: Word)
    ensures
    ...
    }),
{
    ...
    //  Step 6: connect to amalgamation_relator_pred
    lemma_tower_num_generators(data, (k - 1) as nat);
    assert(translate_word_at(data, part2, k - 1)
        =~= inverse_word(shift_word(b_i, k as nat * ng)));

    //  Connect hnn_relator_pred to concat(part1, part2)
    assert(hnn_relator_pred(data, i) =~= concat(part1, part2));
...
pub proof fn lemma_translate_delete_middle(
    data: PredHNNData, m: nat, base_level: int,
    ...
    requires
        ...
        equiv_in_pred_presentation(tower_presentation(data, m),
            ...
            empty_word()),
        pred_presentation_valid(tower_presentation(data, m)),
...
proof fn lemma_translate_stable_pair(data: PredHNNData, s: Symbol, base_level: int)
    requires
        is_stable(data, s),
    ensures
...
pub open spec fn shift_hom(data: PredHNNData, m: nat, k: nat) -> crate::pred_homomorphism::PredHomomorphismData {
    let ng = data.base.num_generators;
    crate::pred_homomorphism::PredHomomorphismData {
        source: data.base,
...
#[verifier::rlimit(200)]
proof fn lemma_shift_hom_applies(data: PredHNNData, k: nat, m: nat, w: Word)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, data.base.num_generators),
    ensures
        crate::pred_homomorphism::apply_hom_pred(shift_hom(data, m, k), w)
            =~= shift_word(w, k * data.base.num_generators),
    decreases w.len(),
{
    let h = shift_hom(data, m, k);
    let ng = data.base.num_generators;
    let result = crate::pred_homomorphism::apply_hom_pred(h, w);
    ...
    if w.len() == 0 {
        ...
        //  IH: apply_hom_pred(h, rest) =~= shift_word(rest, k*ng)
        //  result = concat(apply_hom_symbol_pred(h, w.first()), apply_hom_pred(h, rest))
        ...
        let s = w.first();
        assert(symbol_valid(s, ng));
        let sym_img = crate::pred_homomorphism::apply_hom_symbol_pred(h, s);
...
proof fn lemma_shift_hom_valid(data: PredHNNData, m: nat, k: nat)
    requires
        hnn_pred_data_valid(data),
        k <= m,
    ensures
        crate::pred_homomorphism::is_valid_pred_homomorphism(shift_hom(data, m, k)),
{
    let h = shift_hom(data, m, k);
    let ng = data.base.num_generators;
    ...
    //  h.source == data.base, so (h.source.relators)(r) == (data.base.relators)(r).
    assert forall|r: Word| #![trigger (h.source.relators)(r)] (h.source.relators)(r)
        implies equiv_in_pred_presentation(h.target,
            crate::pred_homomorphism::apply_hom_pred(h, r), empty_word())
    by {
        //  word_valid(r, ng) from (data.base.relators)(r) + pred_presentation_valid(data.base)
        lemma_shift_hom_applies(data, k, m, r);
        //  apply_hom_pred(h, r) =~= shift(r, k*ng)
        lemma_base_relator_in_tower(data, m, k, r);
        //  shift(r, k*ng) ≡ ε in tower(m)
    }
}
...
pub proof fn lemma_base_at_copy_k_embeds(
    data: PredHNNData, m: nat, k: nat, w1: Word, w2: Word,
)
    requires
        hnn_pred_data_valid(data),
        k <= m,
            ...
            shift_word(w2, k * data.base.num_generators)),
{
    ...
    crate::pred_homomorphism::lemma_hom_pred_preserves_equiv(shift_hom(data, m, k), w1, w2);
    lemma_shift_hom_applies(data, k, m, w1);
    lemma_shift_hom_applies(data, k, m, w2);
}

//  Tower identifications_isomorphic_pred from hnn_pred_associations_isomorphic.
//
...
//  - Combine with hnn_pred_associations_isomorphic for the biconditional
...
proof fn lemma_tower_iso_forward_mid(
    data: PredHNNData, k: nat, embed_a_hnn: Word,
)
    requires
        hnn_pred_data_valid(data),
        tower_textbook_chain(data, k),
        word_valid(embed_a_hnn, data.base.num_generators),
        equiv_in_pred_presentation(tower_presentation(data, k),
            shift_word(embed_a_hnn, k * data.base.num_generators), empty_word()),
    ensures
        equiv_in_pred_presentation(data.base, embed_a_hnn, empty_word()),
{
    let ng = data.base.num_generators;
    reveal(pred_presentation_valid);
    if k == 0 {
        ...
        lemma_tower_num_generators(data, (k - 1) as nat);
        crate::pred_normal_form_afp_textbook::lemma_afp_injectivity_right(
...
proof fn lemma_tower_iso_per_word(
    ...
    ensures ({
        ...
        equiv_in_pred_presentation(afp_data.p1, apply_embedding(a_words_tower, w), empty_word())
        <==>
        equiv_in_pred_presentation(afp_data.p2, apply_embedding(b_words_tower, w), empty_word())
    }),
{
    let ng = data.base.num_generators;
    let afp_data = tower_afp_data(data, k);
    ...
    //  HNN biconditional (should fire from hnn_pred_associations_isomorphic)
    assert(equiv_in_pred_presentation(data.base, embed_a_hnn, empty_word())
        <==> equiv_in_pred_presentation(data.base, apply_embedding(b_words_hnn, w), empty_word()));
    ...
    let mid = equiv_in_pred_presentation(data.base, embed_a_hnn, empty_word());
    let lhs = equiv_in_pred_presentation(afp_data.p1, apply_embedding(a_words_tower, w), empty_word());
    let rhs = equiv_in_pred_presentation(afp_data.p2, apply_embedding(b_words_tower, w), empty_word());
...
pub proof fn lemma_tower_identifications_isomorphic(
    ...
    requires
        ...
        tower_textbook_chain(data, k),
    ensures
        crate::pred_normal_form_amalgamated::identifications_isomorphic_pred(tower_afp_data(data, k)),
{
    let ng = data.base.num_generators;
    let afp_data = tower_afp_data(data, k);
    ...
    assert forall|w: Word| word_valid(w, kk as nat) implies (
        equiv_in_pred_presentation(afp_data.p1,
            apply_embedding(Seq::new(kk, |i: int| afp_data.identifications[i].0), w),
            empty_word())
        <==>
        equiv_in_pred_presentation(afp_data.p2,
...
proof fn lemma_translate_base_pair_trivial(
    data: PredHNNData, m: nat, s: Symbol, base_level: nat,
)
    requires
        hnn_pred_data_valid(data),
        !is_stable(data, s),
    ...
    }),
{
    ...
    let step = PredDerivationStep::FreeReduce { position: 0 };
    assert(apply_step_pred(tower_presentation(data, m), translated, step)
        == Some(empty_word()));
    ...
    reveal_with_fuel(pred_derivation_produces, 2);
    assert(pred_derivation_valid(tower_presentation(data, m), d, translated, empty_word()));
}
...
pub open spec fn prefix_levels_bounded(data: PredHNNData, w: Word, m: nat) -> bool {
...
proof fn lemma_net_level_subrange_prefix(data: PredHNNData, w: Word, pos: int)
...
proof fn lemma_shift_word_valid_for_tower(
    data: PredHNNData, w: Word, k: nat, m: nat,
)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, data.base.num_generators),
...
proof fn lemma_net_level_inverse(data: PredHNNData, w: Word)
    ensures
...
proof fn lemma_net_level_hnn_relator(data: PredHNNData, i: int)
    requires
        hnn_pred_data_valid(data),
        0 <= i < data.associations.len() as int,
    ensures
        net_level(data, hnn_relator_pred(data, i)) == 0,
{
    let ng = data.base.num_generators;
    let (a_i, b_i) = (data.associations[i].0, data.associations[i].1);
    let t_inv = Seq::new(1, |_j: int| Symbol::Inv(ng));
...
///  Any relator in hnn_pred_presentation has net_level 0.
proof fn lemma_net_level_hnn_pres_relator(data: PredHNNData, relator: Word)
    requires
        hnn_pred_data_valid(data),
        (hnn_pred_presentation(data).relators)(relator),
    ensures
        net_level(data, relator) == 0,
{
    //  (p.relators)(relator) == hnn_all_relator_pred(data, relator)
    //    == (data.base.relators)(relator) || hnn_extra_relator_pred(data, relator)
    lemma_hnn_relators_unfold(data, relator);
    if (data.base.relators)(relator) {
        reveal(pred_presentation_valid);
        lemma_net_level_base_word(data, relator);
    } else {
        assert(hnn_extra_relator_pred(data, relator));
        let hi = choose|i: int| 0 <= i < data.associations.len() && relator == hnn_relator_pred(data, i);
        lemma_net_level_hnn_relator(data, hi);
    }
}

///  get_relator_pred has net_level 0 when the underlying relator does.
proof fn lemma_net_level_get_relator(data: PredHNNData, relator: Word, inverted: bool)
    requires
        hnn_pred_data_valid(data),
        (hnn_pred_presentation(data).relators)(relator),
    ensures
        net_level(data, get_relator_pred(relator, inverted)) == 0,
{
    lemma_net_level_hnn_pres_relator(data, relator);
    if inverted {
        lemma_net_level_inverse(data, relator);
    }
}

///  Decompose inverse_word(hnn_relator_pred):
///  inv(t⁻¹ · a_i · t · inv(b_i)) = b_i · t⁻¹ · inv(a_i) · t
proof fn lemma_inverse_hnn_relator_decomp(data: PredHNNData, i: int)
    requires
        hnn_pred_data_valid(data),
        0 <= i < data.associations.len() as int,
    ensures ({
        ...
        inverse_word(hnn_relator_pred(data, i)) =~=
    ...
    }),
{
    ...
    //  hnn_relator_pred = concat(t_inv_word, concat(a_i, concat(t_word, inv_b_i)))
    let r = hnn_relator_pred(data, i);
...
proof fn lemma_pair_translate_equiv_empty(
    data: PredHNNData, m: nat, s: Symbol, base_level: int,
)
    requires
        hnn_pred_data_valid(data),
        symbol_valid(s, hnn_pred_presentation(data).num_generators),
...
proof fn lemma_translate_base_relator_valid(
    ...
    ensures
        word_valid(
            translate_word_at(data, relator, k as int),
            tower_presentation(data, m).num_generators),
{
    let ng = data.base.num_generators;
    reveal(pred_presentation_valid);
    lemma_translate_base_word_at(data, relator, k);
    lemma_tower_num_generators(data, m);
    lemma_shift_word_valid_for_tower(data, relator, k, m);
}
...
proof fn lemma_translate_hnn_relator_valid(
    ...
    ensures
        word_valid(
            translate_word_at(data, hnn_relator_pred(data, i), k as int),
            tower_presentation(data, m).num_generators),
{
    let ng = data.base.num_generators;
    let (a_i, b_i) = (data.associations[i].0, data.associations[i].1);
    ...
    lemma_tower_num_generators(data, m);
    //  translate = amalgamation_relator_pred(tower_afp_data(data, (k-1)), i)
    //            = concat(shift_word(a_i, (k-1)*ng), inverse_word(shift_word(b_i, k*ng)))
    let afp_data = tower_afp_data(data, (k - 1) as nat);
    let tr = amalgamation_relator_pred(afp_data, i);
    assert(translate_word_at(data, hnn_relator_pred(data, i), k as int) =~= tr);
    ...
    let tw = translate_word_at(data, hnn_relator_pred(data, i), k as int);
...
proof fn lemma_translate_relator_valid(
    data: PredHNNData, m: nat, relator: Word, inverted: bool, level: int,
)
    requires
        hnn_pred_data_valid(data),
        (hnn_pred_presentation(data).relators)(relator),
        0 <= level <= m as int,
        //  HNN relators (= non-base relators) need level >= 1
        !(data.base.relators)(relator) ==> level >= 1,
    ensures
        word_valid(
            translate_word_at(data,
                get_relator_pred(relator, inverted), level),
            tower_presentation(data, m).num_generators),
{
    let p = hnn_pred_presentation(data);
    let ng = data.base.num_generators;
    let tp = tower_presentation(data, m);
    ...
    if !inverted {
        if (data.base.relators)(relator) {
            //  Base relator: get_relator_pred(relator, false) == relator
            lemma_translate_base_relator_valid(data, m, level as nat, relator);
        } else {
            //  HNN relator
            assert(hnn_extra_relator_pred(data, relator));
            let hi = choose|i: int| 0 <= i < data.associations.len() && relator == hnn_relator_pred(data, i);
        ...
        //  Inverted relator: get_relator_pred(relator, true) = inverse_word(relator)
        if (data.base.relators)(relator) {
            //  Inverted base relator: inverse_word of a base word is still base-valid
            reveal(pred_presentation_valid);
            crate::word::lemma_inverse_word_valid(relator, ng);
            lemma_translate_base_word_at(data, inverse_word(relator), level as nat);
            lemma_tower_num_generators(data, m);
            lemma_shift_word_valid_for_tower(data, inverse_word(relator), level as nat, m);
        } else {
            ...
            assert(inverse_word(hnn_relator_pred(data, hi))
            ...
            let tr = translate_word_at(data, inverse_word(hnn_relator_pred(data, hi)), k as int);
            assert(tr =~= concat(
                shift_word(b_i, k * ng),
            ...
            crate::word::lemma_concat_word_valid(
                ...
                shift_word(inv_a_i, ((k - 1) as nat) * ng),
                (m + 1) * ng);
            //  Bridge: goal word == tr.  get_relator_pred(relator,true)=inverse_word(relator),
            //  relator == hnn_relator_pred(data,hi), and level == k as int (level >= 1).
            assert(k as int == level);
            assert(get_relator_pred(relator, inverted) == inverse_word(hnn_relator_pred(data, hi)));
        }
    }
}
    ...
    }),
{
    ...
    lemma_net_level_get_relator(data, relator, inverted);
    lemma_hnn_relators_unfold(data, relator);

    if !inverted {
        if (data.base.relators)(relator) {
            //  Base relator at level k: r == relator
            reveal(pred_presentation_valid);
            lemma_translate_base_word_at(data, relator, level as nat);
            lemma_base_relator_in_tower(data, m, level as nat, relator);
        } else {
        ...
        //  Inverted: get_relator_pred(relator, true) = inverse_word(relator)
        if (data.base.relators)(relator) {
            reveal(pred_presentation_valid);
            ...
            //  Now show inverted: inverse_word(relator) is still base-valid
            crate::word::lemma_inverse_word_valid(relator, ng);
            lemma_translate_base_word_at(data, inverse_word(relator), level as nat);

            //  shift(inv(r), k*ng) = inv(shift(r, k*ng))
            crate::free_product::lemma_shift_inverse_word(relator, (level as nat) * ng);

            //  translate(inv(r), k) =~= inv(shift(r, k*ng)) and translate(r, k) ≡ ε
            //  so inv(translate(r, k)) ≡ ε
            lemma_tower_valid(data, m);
            lemma_tower_num_generators(data, m);
            lemma_shift_word_valid_for_tower(data, relator, level as nat, m);
            lemma_pred_inverse_of_trivial(
                tp,
                shift_word(relator, (level as nat) * ng));
        } else {
            assert(hnn_extra_relator_pred(data, relator));
            let hi = choose|i: int| 0 <= i < data.associations.len() && relator == hnn_relator_pred(data, i);
            assert(r == inverse_word(hnn_relator_pred(data, hi)));
            //  Decompose inv(hnn_relator_pred) = b_i · t⁻¹ · inv(a_i) · t
            lemma_inverse_hnn_relator_decomp(data, hi);
            let (a_i, b_i) = (data.associations[hi].0, data.associations[hi].1);
            let t_word = Seq::new(1, |_j: int| Symbol::Gen(ng));
            let t_inv_word = Seq::new(1, |_j: int| Symbol::Inv(ng));
            ...
            //  This equals inverse_word(amalgamation_relator_pred(afp_data, hi))
            ...
            let amal_r = amalgamation_relator_pred(afp_data, hi);
            lemma_translate_hnn_relator(data, hi, level);
            lemma_ident_relator_in_tower(data, m, (level - 1) as nat, hi);
            ...
            lemma_pred_inverse_of_trivial(tp, amal_r);
...
pub proof fn lemma_hnn_step_tower_equiv(
    data: PredHNNData, m: nat, base_level: int, w: Word, step: PredDerivationStep,
)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
    ...
    ensures
        equiv_in_pred_presentation(tower_presentation(data, m),
            ...
            translate_word_at(data, apply_step_pred(hnn_pred_presentation(data), w, step).unwrap(), base_level)),
{
    let p = hnn_pred_presentation(data);
    let tp = tower_presentation(data, m);
    let w_next = apply_step_pred(p, w, step).unwrap();
    lemma_tower_valid(data, m);
    lemma_tower_num_generators(data, m);
    ...
    match step {
        ...
        PredDerivationStep::RelatorDelete { position, relator, inverted } => {
            let pos = position;
            //  apply_step_pred Some ⟹ (p.relators)(relator); step_is_hnn_relator ⟹ !(base)(relator)
            ...
            let level = base_level + net_level(data, prefix);
            assert(step_position(step) == pos);

            lemma_translate_relator_equiv_empty(data, m, relator, inverted, level);
            lemma_translate_relator_valid(data, m, relator, inverted, level);
...
//  ============================================================
//  Part P: PredDerivation-level induction
//  ============================================================

///  Get the position of a derivation step.
pub open spec fn step_position(step: PredDerivationStep) -> int {
    match step {
        PredDerivationStep::FreeReduce { position } => position,
        PredDerivationStep::FreeExpand { position, .. } => position,
        PredDerivationStep::RelatorInsert { position, .. } => position,
        PredDerivationStep::RelatorDelete { position, .. } => position,
    }
}

///  Whether a step involves an HNN relator (not a base relator).
pub open spec fn step_is_hnn_relator(data: PredHNNData, step: PredDerivationStep) -> bool {
    match step {
        PredDerivationStep::RelatorInsert { relator, .. } |
        PredDerivationStep::RelatorDelete { relator, .. } =>
            !(data.base.relators)(relator),
        _ => false,
    }
}
...
pub open spec fn step_level_ok(data: PredHNNData, m: nat, base_level: int, w: Word, step: PredDerivationStep) -> bool {
...
pub open spec fn derivation_levels_ok(
    data: PredHNNData, m: nat, base_level: int,
    steps: Seq<PredDerivationStep>, start: Word,
) -> bool
    decreases steps.len(),
{
    if steps.len() == 0 {
...
pub proof fn lemma_hnn_derivation_to_tower_equiv(
    data: PredHNNData, m: nat, base_level: int,
    steps: Seq<PredDerivationStep>, start: Word, end: Word,
)
    requires
        hnn_pred_data_valid(data),
        word_valid(start, hnn_pred_presentation(data).num_generators),
        pred_derivation_produces(hnn_pred_presentation(data), steps, start) == Some(end),
    ...
    decreases steps.len(),
{
    ...
    if steps.len() == 0 {
        assert(start == end);
        lemma_pred_equiv_refl(tp, translate_word_at(data, start, base_level));
    } else {
        let step = steps.first();
        let mid = apply_step_pred(p, start, step).unwrap();
        ...
        lemma_hnn_pred_presentation_valid(data);
        crate::pred_presentation::lemma_pred_step_preserves_word_valid(p, start, step, mid);
        ...
        lemma_pred_equiv_transitive(tp,
            translate_word_at(data, start, base_level),
...
pub proof fn britton_lemma(
    ...
    requires
        ...
        ({
            let d: PredDerivation = choose|d: PredDerivation|
                pred_derivation_valid(hnn_pred_presentation(data), d, w, empty_word());
        ...
        equiv_in_pred_presentation(data.base, w, empty_word()),
{
    let hp = hnn_pred_presentation(data);
    let d: PredDerivation = choose|d: PredDerivation|
        pred_derivation_valid(hp, d, w, empty_word());
...
//  ============================================================
//  Part S: PredDerivation level bounds for shifted translation
//  ============================================================
...
pub open spec fn derivation_min_adj_level(
    data: PredHNNData, steps: Seq<PredDerivationStep>, start: Word,
) -> int
    decreases steps.len(),
{
    let hp = hnn_pred_presentation(data);
    if steps.len() == 0 { 0 }
        ...
        match apply_step_pred(hp, start, steps.first()) {
            Some(next) => {
...
proof fn lemma_derivation_levels_ok_from_bounds(
    ...
    requires
        pred_derivation_produces(hnn_pred_presentation(data), steps, start) is Some,
    ...
    decreases steps.len(),
{
    if steps.len() == 0 {} else {
        let hp = hnn_pred_presentation(data);
        let step = steps.first();
        let next = apply_step_pred(hp, start, step).unwrap();
        let pos = step_position(step);
        let level = net_level(data, start.subrange(0, pos));
...
///  Derive tower_textbook_chain from hnn_pred_associations_isomorphic by induction.
pub proof fn lemma_tower_textbook_chain_from_hnn_iso(data: PredHNNData, m: nat)
    requires
        hnn_pred_data_valid(data),
        hnn_pred_associations_isomorphic(data),
    ensures
    ...
    decreases m,
{
    if m == 0 {
        ...
        //  Prove identifications_isomorphic_pred at level k
        ...
        reveal(pred_presentation_valid);
        crate::pred_normal_form_afp_textbook::lemma_iso_implies_apc(afp_data);
...
pub proof fn lemma_copy_s_embeds(data: PredHNNData, m: nat, s: nat, w: Word)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, data.base.num_generators),
        s <= m,
    ...
    decreases m,
{
    ...
    if m == 0 {
        ...
        crate::pred_normal_form_afp_textbook::lemma_afp_injectivity(
...
proof fn lemma_translate_empty_at(data: PredHNNData, base_level: int)
    ensures
...
///
///  No additional assumptions beyond hnn_pred_data_valid and hnn_pred_associations_isomorphic.
///  The tower textbook prerequisites are derived from hnn_pred_associations_isomorphic,
///  and the derivation levels are handled by shifting to a non-negative base level.
pub proof fn britton_lemma_unconditional(
    data: PredHNNData, w: Word,
)
    requires
        hnn_pred_data_valid(data),
        hnn_pred_associations_isomorphic(data),
        ...
        equiv_in_pred_presentation(data.base, w, empty_word()),
{
    ...
    //  Step 4: Tower textbook chain from hnn_pred_associations_isomorphic
...
proof fn lemma_act_left_sym_preserves_right_count(
    data: PredAmalgamatedData, s: Symbol, h: Word, syls: Seq<Syllable>,
)
    requires generator_index(s) < data.p1.num_generators,
    ensures
        right_syllable_count(
            crate::pred_normal_form_afp_textbook::act_left_sym(data, s, h, syls).1)
            == right_syllable_count(syls),
{
    let product = concat(Seq::new(1, |_j: int| s),
        apply_embedding(crate::pred_normal_form_afp_textbook::a_words(data), h));
    let new_rep = crate::pred_normal_form_afp_textbook::a_rcoset_rep(data, product);
    ...
    if new_rep =~= empty_word() {
        ...
        let merged_rep = crate::pred_normal_form_afp_textbook::a_rcoset_rep(data, full_product);
...
proof fn lemma_act_g1_word_preserves_right_count(
    data: PredAmalgamatedData, w: Word, h: Word, syls: Seq<Syllable>,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        pred_presentation_valid(data.p2),
        crate::pred_normal_form_amalgamated::identifications_isomorphic_pred(data),
        crate::pred_normal_form_afp_textbook::action_preserves_canonical(data),
        crate::pred_normal_form_afp_textbook::is_canonical_state(data, h, syls),
        word_valid(w, data.p1.num_generators),
    ensures
        right_syllable_count(
            crate::pred_normal_form_afp_textbook::act_word(data, w, h, syls).1)
            == right_syllable_count(syls),
    decreases w.len(),
{
    use crate::pred_normal_form_afp_textbook::*;
    if w.len() == 0 {
...
pub open spec fn has_stable_letter(data: PredHNNData, w: Word) -> bool {
    exists|i: int| 0 <= i < w.len() && is_stable(data, #[trigger] w[i])
}

///  Adjacent opposite stable letters with only base symbols between.
pub open spec fn has_adjacent_opposite_at(data: PredHNNData, w: Word, i: int, j: int) -> bool {
    let ng = data.base.num_generators;
    &&& 0 <= i < j < w.len()
    &&& is_stable(data, w[i])
    &&& is_stable(data, w[j])
...
pub open spec fn has_pinch_at(data: PredHNNData, w: Word, i: int, j: int) -> bool {
    let ng = data.base.num_generators;
    ...
    &&& (
        ...
        (w[i] == Symbol::Gen(ng) && w[j] == Symbol::Inv(ng)
         && crate::pred_normal_form_amalgamated::in_generated_subgroup_pred(data.base, b_gens, base_word))
        ||
        //  t⁻¹·g·t: pinch iff g ∈ A
        (w[i] == Symbol::Inv(ng) && w[j] == Symbol::Gen(ng)
         && crate::pred_normal_form_amalgamated::in_generated_subgroup_pred(data.base, a_gens, base_word))
    )
}

///  Word w has a pinch somewhere.
pub open spec fn has_pinch(data: PredHNNData, w: Word) -> bool {
...
proof fn lemma_net_level_single(data: PredHNNData, s: Symbol)
    ensures
        net_level(data, Seq::new(1, |_j: int| s)) == (
...
proof fn lemma_net_level_inverse_pair(data: PredHNNData, s: Symbol)
    ensures
        net_level(data, Seq::new(1, |_j: int| s)
...
proof fn lemma_step_preserves_net_level(data: PredHNNData, w: Word, step: PredDerivationStep)
    requires
        hnn_pred_data_valid(data),
        apply_step_pred(hnn_pred_presentation(data), w, step) is Some,
    ensures
        net_level(data, apply_step_pred(hnn_pred_presentation(data), w, step).unwrap())
            == net_level(data, w),
{
    let hp = hnn_pred_presentation(data);
    let w2 = apply_step_pred(hp, w, step).unwrap();
    match step {
        PredDerivationStep::FreeReduce { position } => {
            let p = position;
...
proof fn lemma_derivation_preserves_net_level(
    data: PredHNNData, steps: Seq<PredDerivationStep>, w: Word,
)
    requires
        hnn_pred_data_valid(data),
        pred_derivation_produces(hnn_pred_presentation(data), steps, w) is Some,
    ensures
        net_level(data, pred_derivation_produces(hnn_pred_presentation(data), steps, w).unwrap())
            == net_level(data, w),
    decreases steps.len(),
{
    if steps.len() == 0 {
        ...
        let w2 = apply_step_pred(hp, w, first).unwrap();
...
proof fn lemma_equiv_net_level_zero(data: PredHNNData, w: Word)
    requires
        hnn_pred_data_valid(data),
        equiv_in_pred_presentation(hnn_pred_presentation(data), w, empty_word()),
    ensures
...
{
    let hp = hnn_pred_presentation(data);
    let d: PredDerivation = choose|d: PredDerivation| pred_derivation_valid(hp, d, w, empty_word());
...
pub open spec fn stable_count_from(data: PredHNNData, w: Word, from: int) -> nat
...
pub open spec fn next_stable(data: PredHNNData, w: Word, from: int) -> int
    decreases (w.len() - from),
{
...
proof fn lemma_next_stable_props(data: PredHNNData, w: Word, from: int)
...
proof fn lemma_net_level_same_sign_nonneg(data: PredHNNData, w: Word)
    requires
...
proof fn lemma_net_level_same_sign_nonpos(data: PredHNNData, w: Word)
    requires
        forall|k: int| 0 <= k < w.len() && is_stable(data, #[trigger] w[k])
...
proof fn lemma_adjacent_opposite_exists(data: PredHNNData, w: Word)
    requires
        hnn_pred_data_valid(data),
...
proof fn lemma_scan_for_sign_change(data: PredHNNData, w: Word, pos: int)
    requires
        hnn_pred_data_valid(data),
        0 <= pos < w.len(),
...
proof fn lemma_net_level_no_stable(data: PredHNNData, w: Word, offset: int)
    requires
        hnn_pred_data_valid(data),
...
proof fn lemma_b_rcoset_empty_implies_in_right_subgroup(
    data: PredAmalgamatedData, g: Word,
)
    requires
        amalgamated_data_pred_valid(data),
    ...
    ensures
        crate::pred_normal_form_amalgamated::in_right_subgroup_pred(data, g),
{
    use crate::pred_normal_form_afp_textbook::*;
    lemma_b_rcoset_rep_props(data, g);
    //  same_b_rcoset(data, g, rep) where rep =~= ε
    //  same_b_rcoset(data, g, ε) = in_right_subgroup_pred(data, concat(g, inverse_word(ε)))
...
proof fn lemma_not_in_left_subgroup_rep_nonempty(
    ...
    requires
        ...
        !crate::pred_normal_form_amalgamated::in_left_subgroup_pred(data, g),
    ensures
        !(crate::pred_normal_form_afp_textbook::a_rcoset_rep(data, g) =~= empty_word()),
{
    if crate::pred_normal_form_afp_textbook::a_rcoset_rep(data, g) =~= empty_word() {
...
proof fn lemma_subgroup_right_cancel(
    p: PredPresentation, gens: Seq<Word>, g: Word, b: Word,
)
    requires
        pred_presentation_valid(p),
        word_valid(g, p.num_generators),
        word_valid(b, p.num_generators),
        crate::pred_normal_form_amalgamated::in_generated_subgroup_pred(p, gens, concat(g, b)),
        crate::pred_normal_form_amalgamated::in_generated_subgroup_pred(p, gens, b),
        forall|i: int| 0 <= i < gens.len()
            ==> word_valid(#[trigger] gens[i], p.num_generators),
    ensures
        crate::pred_normal_form_amalgamated::in_generated_subgroup_pred(p, gens, g),
{
    use crate::pred_normal_form_afp_textbook::*;
    //  inv(b) ∈ S
...
proof fn lemma_not_in_subgroup_concat_embed_b(
    data: PredAmalgamatedData, g: Word, h: Word,
)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p2),
        word_valid(g, data.p2.num_generators),
        word_valid(h, crate::pred_normal_form_afp_textbook::k_size(data)),
        !crate::pred_normal_form_amalgamated::in_right_subgroup_pred(data, g),
        forall|i: int| 0 <= i < crate::pred_normal_form_afp_textbook::b_words(data).len()
            ==> word_valid(
                #[trigger] crate::pred_normal_form_afp_textbook::b_words(data)[i],
                data.p2.num_generators),
    ensures
        !crate::pred_normal_form_amalgamated::in_right_subgroup_pred(
            data, concat(g, apply_embedding(
                crate::pred_normal_form_afp_textbook::b_words(data), h))),
{
    use crate::pred_normal_form_afp_textbook::*;
    use crate::pred_normal_form_amalgamated::in_right_subgroup_pred;
    ...
    //  in_right_subgroup_pred(data, w) == crate::pred_normal_form_amalgamated::in_generated_subgroup_pred(data.p2, b_words(data), w)
    ...
    if in_right_subgroup_pred(data, concat(g, embed)) {
...
pub proof fn lemma_tower_injectivity_peel(
    data: PredHNNData, k: nat, m: nat, w: Word,
)
    requires
        hnn_pred_data_valid(data),
        k <= m,
        ...
        equiv_in_pred_presentation(tower_presentation(data, m), w, empty_word()),
    ensures
        equiv_in_pred_presentation(tower_presentation(data, k), w, empty_word()),
    decreases m - k,
{
    if k == m {
    } else {
...
proof fn lemma_translate_word_valid_for_level(
    data: PredHNNData, w: Word, bl: int, m: nat,
)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
...
///  the level is in [-k, k].
proof fn lemma_prefix_level_bounded_by_k(data: PredHNNData, w: Word, k: int)
    requires 0 <= k <= w.len(),
    ensures
...
proof fn lemma_word_level_bounds(data: PredHNNData, w: Word, k: int)
    requires 0 <= k <= w.len(),
    ensures
...
proof fn lemma_act_right_sym_preserves_left_count(
    ...
    ensures
        left_syllable_count(
            crate::pred_normal_form_afp_textbook::act_right_sym(data, s, h, syls).1)
            == left_syllable_count(syls),
{
    let product = concat(Seq::new(1, |_i: int| s),
        apply_embedding(crate::pred_normal_form_afp_textbook::b_words(data), h));
    let new_rep = crate::pred_normal_form_afp_textbook::b_rcoset_rep(data, product);
    ...
    if new_rep =~= empty_word() {
        ...
        let full_product = concat(product, syls.first().rep);
        let merged_rep = crate::pred_normal_form_afp_textbook::b_rcoset_rep(data, full_product);
        if merged_rep =~= empty_word() {
            assert(!syls.first().is_left);
        } else {
...
proof fn lemma_act_g2_word_preserves_left_count(
    ...
    ensures
        left_syllable_count(
            crate::pred_normal_form_afp_textbook::act_word(
...
proof fn lemma_not_in_left_subgroup_concat_embed_a(
    ...
    requires
        ...
        word_valid(h, crate::pred_normal_form_afp_textbook::k_size(data)),
        !crate::pred_normal_form_amalgamated::in_left_subgroup_pred(data, g),
        forall|i: int| 0 <= i < crate::pred_normal_form_afp_textbook::a_words(data).len()
            ==> word_valid(
                #[trigger] crate::pred_normal_form_afp_textbook::a_words(data)[i],
                data.p1.num_generators),
    ensures
        !crate::pred_normal_form_amalgamated::in_left_subgroup_pred(
            data, concat(g, apply_embedding(
                crate::pred_normal_form_afp_textbook::a_words(data), h))),
{
    use crate::pred_normal_form_afp_textbook::*;
    use crate::pred_normal_form_amalgamated::in_left_subgroup_pred;
    ...
    if in_left_subgroup_pred(data, concat(g, embed)) {
...
proof fn lemma_translate_gen_base_inv(data: PredHNNData, g: Word, bl: int)
    requires
        hnn_pred_data_valid(data),
...
pub open spec fn max_prefix_level(data: PredHNNData, w: Word) -> int
    decreases w.len(),
{
    if w.len() == 0 { 0 }
    else {
...
proof fn lemma_max_prefix_bounds(data: PredHNNData, w: Word, k: int)
    requires 0 <= k <= w.len(),
    ensures
...
proof fn lemma_max_prefix_achieved(data: PredHNNData, w: Word)
    ensures
...
proof fn lemma_find_gen_inv_forward(data: PredHNNData, w: Word, from: int, bound: int)
    requires
        hnn_pred_data_valid(data),
        has_stable_letter(data, w),
        net_level(data, w) == 0,
...
proof fn lemma_gen_inv_pair_when_max_ge_1(data: PredHNNData, w: Word)
    requires
        hnn_pred_data_valid(data),
        has_stable_letter(data, w),
        net_level(data, w) == 0,
...
proof fn lemma_last_max_position(data: PredHNNData, w: Word, pos: int)
    requires
        0 <= pos <= w.len(),
...
proof fn lemma_last_max_scan(data: PredHNNData, w: Word, last_known: int, from: int)
    requires
...
proof fn lemma_rightmost_gen_inv(data: PredHNNData, w: Word)
    requires
        hnn_pred_data_valid(data),
        has_stable_letter(data, w),
        net_level(data, w) == 0,
...
proof fn lemma_find_gen_before_last_inv(data: PredHNNData, w: Word, inv_pos: int)
    requires
        hnn_pred_data_valid(data),
        0 < inv_pos < w.len(),
        max_prefix_level(data, w) >= 1,
...
proof fn lemma_find_plateau_gen(data: PredHNNData, w: Word, pos: int, inv_pos: int)
    requires
        hnn_pred_data_valid(data),
        0 < pos < inv_pos < w.len(),
        max_prefix_level(data, w) >= 1,
...
proof fn lemma_tower_setup(data: PredHNNData, w: Word) -> (result: (nat, nat))
    requires
        hnn_pred_data_valid(data),
        hnn_pred_associations_isomorphic(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
    ...
    ensures ({
        ...
        &&& equiv_in_pred_presentation(tower_presentation(data, pl), tw, empty_word())
    }),
{
    let hp = hnn_pred_presentation(data);
    let ng = data.base.num_generators;
...
proof fn lemma_suffix_net_level(data: PredHNNData, w: Word, split: int, k: int)
    requires
        0 <= split <= w.len(),
        0 <= k <= w.len() - split,
    ensures
...
proof fn lemma_suffix_levels_bounded(
    data: PredHNNData, w: Word, split: int, bl: int, pl: nat,
)
    requires
        hnn_pred_data_valid(data),
        0 <= split <= w.len(),
...
proof fn lemma_pair_net_level_return(
    data: PredHNNData, w: Word, pair_i: int, pair_j: int,
)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
...
proof fn lemma_suffix_translate_g1(
    data: PredHNNData, w: Word, pair_i: int, pair_j: int, bl: nat, pl: nat,
)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
    ...
    }),
{
    ...
    assert(word_valid(w_suffix, hnn_pred_presentation(data).num_generators)) by {
        assert forall|k: int| 0 <= k < w_suffix.len()
            implies symbol_valid(#[trigger] w_suffix[k], hnn_pred_presentation(data).num_generators)
...
proof fn lemma_translate_decompose_at_pair(
    data: PredHNNData, w: Word, pair_i: int, pair_j: int, bl: int,
)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
...
proof fn lemma_g2_creates_right_syllable(
    data: PredHNNData, base_word: Word,
    afp: PredAmalgamatedData, h: Word, syls: Seq<crate::pred_normal_form_afp_textbook::Syllable>,
)
    requires
        hnn_pred_data_valid(data),
        amalgamated_data_pred_valid(afp),
        pred_presentation_valid(afp.p1),
        pred_presentation_valid(afp.p2),
        crate::pred_normal_form_amalgamated::identifications_isomorphic_pred(afp),
        crate::pred_normal_form_afp_textbook::action_preserves_canonical(afp),
        crate::pred_normal_form_afp_textbook::is_canonical_state(afp, h, syls),
        word_valid(base_word, afp.p2.num_generators),
        //  base_word ∉ B (in the AFP's B-subgroup)
        !crate::pred_normal_form_amalgamated::in_right_subgroup_pred(afp, base_word),
        //  State has right_count = 0
        right_syllable_count(syls) == 0,
        //  b_words are valid
        forall|i: int| 0 <= i < crate::pred_normal_form_afp_textbook::b_words(afp).len()
            ==> word_valid(
                #[trigger] crate::pred_normal_form_afp_textbook::b_words(afp)[i],
                afp.p2.num_generators),
        //  K-size and h valid
        word_valid(h, crate::pred_normal_form_afp_textbook::k_size(afp)),
    ensures ({
        use crate::pred_normal_form_afp_textbook::*;
...
proof fn lemma_case_a_act_identity(data: PredHNNData, w: Word) -> (result: (nat, nat))
    requires
        hnn_pred_data_valid(data),
        hnn_pred_associations_isomorphic(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
    ...
    ensures ({
        ...
        let e = empty_word();
        let es = Seq::<crate::pred_normal_form_afp_textbook::Syllable>::empty();
        &&& pl >= 1
        &&& pl == (bl + max_prefix_level(data, w)) as nat
        &&& bl >= w.len()
        &&& tower_textbook_chain(data, pl)
        &&& word_valid(tw, tower_presentation(data, pl).num_generators)
        &&& crate::pred_normal_form_afp_textbook::act_word(afp, tw, e, es) == (e, es)
    }),
{
    use crate::pred_normal_form_afp_textbook::*;
    ...
    let d_tw: PredDerivation = choose|d: PredDerivation|
        pred_derivation_valid(tower_presentation(data, pl), d, tw, empty_word());
...
pub open spec fn textbook_g2_action(
    data: PredAmalgamatedData, g: Word, syls: Seq<Syllable>,
) -> (Word, Seq<Syllable>) {
    let rep = crate::pred_normal_form_afp_textbook::b_rcoset_rep(data, g);
    let h_new = crate::pred_normal_form_afp_textbook::b_rcoset_h(data, g);
    if rep =~= empty_word() && syls.len() > 0 && syls.first().is_left {
        ...
        let phi_inv = apply_embedding(
            crate::pred_normal_form_afp_textbook::a_words(data), h_new);
        ...
        let collapsed_h = crate::pred_normal_form_afp_textbook::a_rcoset_h(
            data, new_leading);
...
pub open spec fn textbook_g1_action(
    ...
    let rep = crate::pred_normal_form_afp_textbook::a_rcoset_rep(data, g);
    let h_new = crate::pred_normal_form_afp_textbook::a_rcoset_h(data, g);
    if rep =~= empty_word() && syls.len() > 0 && !syls.first().is_left {
        //  COLLAPSE: y₀ = 1 AND ε₁ = +1 (opposite type for p⁻¹)
        let phi = apply_embedding(
            crate::pred_normal_form_afp_textbook::b_words(data), h_new);
        let new_leading = concat(phi, syls.first().rep);
        let collapsed_h = crate::pred_normal_form_afp_textbook::b_rcoset_h(
            data, new_leading);
        (collapsed_h, syls.drop_first())
    } else {
        //  PREPEND
...
proof fn lemma_textbook_g2_prepend_length(
    ...
    requires
        ...
        !(crate::pred_normal_form_afp_textbook::b_rcoset_rep(data, g) =~= empty_word()
...
pub open spec fn textbook_psi_p(
    data: PredHNNData, h: Word, syls: Seq<Syllable>,
) -> (Word, Seq<Syllable>) {
    let afp = tower_afp_data(data, 0);
    let rep = crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h);
    let h_id = crate::pred_normal_form_afp_textbook::b_rcoset_h(afp, h);
    //  φ⁻¹(b): map B-part to A via identification
    let phi_inv_h = apply_embedding(
        crate::pred_normal_form_afp_textbook::a_words(afp), h_id);
    if rep =~= empty_word() && syls.len() > 0 && syls.first().is_left {
...
pub open spec fn textbook_psi_p_inv(
    ...
    let afp = tower_afp_data(data, 0);
    let rep = crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h);
    let h_id = crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, h);
    //  φ(a): map A-part to B via identification
    let phi_h = apply_embedding(
        crate::pred_normal_form_afp_textbook::b_words(afp), h_id);
    if rep =~= empty_word() && syls.len() > 0 && !syls.first().is_left {
...
pub open spec fn textbook_act_hnn(
    data: PredHNNData, w: Word, h: Word, syls: Seq<Syllable>,
) -> (Word, Seq<Syllable>)
    decreases w.len(),
{
    if w.len() == 0 {
...
pub open spec fn stable_count(data: PredHNNData, w: Word) -> nat
    decreases w.len(),
{
    if w.len() == 0 { 0 }
...
pub open spec fn last_stable_pos(data: PredHNNData, w: Word) -> int
    decreases w.len(),
{
    if w.len() == 0 { -1 }
...
pub open spec fn trailing_segment(data: PredHNNData, w: Word) -> Word {
...
pub open spec fn leading_part(data: PredHNNData, w: Word) -> Word {
    let lsp = last_stable_pos(data, w);
    if lsp < 0 { empty_word() }
...
proof fn lemma_last_stable_pos_bounds(data: PredHNNData, w: Word)
    ensures
...
proof fn lemma_segment_partition(data: PredHNNData, w: Word)
    ensures
...
proof fn lemma_trailing_no_stable(data: PredHNNData, w: Word)
    ensures
...
proof fn lemma_no_stable_when_lsp_neg(data: PredHNNData, w: Word, pos: int)
    requires
        last_stable_pos(data, w) < 0,
        0 <= pos < w.len(),
...
proof fn lemma_no_stable_after_lsp(data: PredHNNData, w: Word, pos: int)
    requires
        last_stable_pos(data, w) >= 0,
...
pub proof fn lemma_stable_count_concat(data: PredHNNData, a: Word, b: Word)
...
pub proof fn lemma_stable_count_no_stable(data: PredHNNData, w: Word)
...
proof fn lemma_stable_count_single(data: PredHNNData, w: Word)
...
proof fn lemma_leading_part_props(data: PredHNNData, w: Word)
...
proof fn lemma_lsp_ge_zero_when_stable(data: PredHNNData, w: Word)
    requires has_stable_letter(data, w),
...
proof fn lemma_textbook_base_only(data: PredHNNData, w: Word, h: Word, syls: Seq<Syllable>)
    requires
        hnn_pred_data_valid(data),
        forall|k: int| 0 <= k < w.len() ==> !is_stable(data, #[trigger] w[k]),
    ensures
...
proof fn lemma_textbook_act_concat(
    data: PredHNNData, a: Word, b: Word, h: Word, syls: Seq<Syllable>,
)
    requires hnn_pred_data_valid(data),
    ensures ({
...
proof fn lemma_textbook_psi_p_length(data: PredHNNData, h: Word, syls: Seq<Syllable>)
    requires hnn_pred_data_valid(data),
    ensures ({
        let afp = tower_afp_data(data, 0);
        let rep = crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h);
...
///  ψ(p⁻¹) length: PREPEND gives +1, COLLAPSE gives -1.
proof fn lemma_textbook_psi_p_inv_length(data: PredHNNData, h: Word, syls: Seq<Syllable>)
    requires hnn_pred_data_valid(data),
    ensures ({
        let afp = tower_afp_data(data, 0);
        let rep = crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h);
...
proof fn lemma_textbook_act_single_stable(
    data: PredHNNData, s: Symbol, h: Word, syls: Seq<Syllable>,
)
    requires
        hnn_pred_data_valid(data),
        is_stable(data, s),
...
proof fn lemma_stable_count_zero_no_stable(data: PredHNNData, w: Word, pos: int)
    requires
...
proof fn lemma_textbook_base_case(data: PredHNNData, w: Word)
    requires
        hnn_pred_data_valid(data),
        stable_count(data, w) == 0,
    ensures
...
///  Chain: rep_props → same_b_rcoset → crate::pred_normal_form_amalgamated::in_generated_subgroup_pred → subgroup_to_k_word → witness → decomposition.
proof fn lemma_b_rcoset_h_word_valid(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p2),
        word_valid(g, data.p2.num_generators),
    ensures
        word_valid(crate::pred_normal_form_afp_textbook::b_rcoset_h(data, g),
            crate::pred_normal_form_afp_textbook::k_size(data)),
{
    use crate::pred_normal_form_afp_textbook::*;
    lemma_b_rcoset_rep_props(data, g);
    let rep = b_rcoset_rep(data, g);
    //  same_b_rcoset(data, g, rep) → in_right_subgroup_pred(data, concat(g, inv(rep)))
    //  → crate::pred_normal_form_amalgamated::in_generated_subgroup_pred(p2, b_words, concat(g, inv(rep)))
    ...
    let h_w: Word = choose|hw: Word|
        ...
        && equiv_in_pred_presentation(data.p2, apply_embedding(b_words(data), hw), target);
    assert(b_words(data).len() == k_size(data));
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_decomposition(data, g, h_w);
}

///  Witness-free: a_rcoset_h is word_valid for k_size.
proof fn lemma_a_rcoset_h_word_valid(data: PredAmalgamatedData, g: Word)
    requires
        amalgamated_data_pred_valid(data),
        pred_presentation_valid(data.p1),
        word_valid(g, data.p1.num_generators),
    ensures
        word_valid(crate::pred_normal_form_afp_textbook::a_rcoset_h(data, g),
            crate::pred_normal_form_afp_textbook::k_size(data)),
{
    use crate::pred_normal_form_afp_textbook::*;
    lemma_a_rcoset_rep_props(data, g);
    ...
    let h_w: Word = choose|hw: Word|
        ...
        && equiv_in_pred_presentation(data.p1, apply_embedding(a_words(data), hw), target);
    assert(a_words(data).len() == k_size(data));
    crate::pred_normal_form_afp_textbook::lemma_rcoset_decomposition(data, g, h_w);
}

///  After ψ(p), the new h is in the A-subgroup.
///  h_new = apply_embedding(a_words, b_rcoset_h(h)) ∈ A.
proof fn lemma_psi_p_output_in_A(data: PredHNNData, h: Word)
    requires
        hnn_pred_data_valid(data),
    ...
    ensures ({
        ...
        crate::pred_normal_form_amalgamated::in_left_subgroup_pred(afp, h_new)
    }),
{
    let afp = tower_afp_data(data, 0);
    crate::pred_tower::lemma_tower_afp_data_valid(data, 0);
    crate::pred_tower::lemma_tower_valid(data, 0);
    ...
    assert forall|i: int| 0 <= i < crate::pred_normal_form_afp_textbook::a_words(afp).len()
        implies word_valid(
            #[trigger] crate::pred_normal_form_afp_textbook::a_words(afp)[i],
            afp.p1.num_generators)
    by {
        crate::pred_tower::lemma_tower_num_generators(data, 0);
    }
    crate::pred_normal_form_afp_textbook::lemma_apply_embedding_in_subgroup(
        afp.p1, crate::pred_normal_form_afp_textbook::a_words(afp), h_id);
}

///  After ψ(p⁻¹), the new h is in the B-subgroup.
proof fn lemma_psi_p_inv_output_in_B(data: PredHNNData, h: Word)
    requires
        hnn_pred_data_valid(data),
        word_valid(h, data.base.num_generators),
    ensures ({
        ...
        crate::pred_normal_form_amalgamated::in_right_subgroup_pred(afp, h_new)
    }),
{
    let afp = tower_afp_data(data, 0);
    crate::pred_tower::lemma_tower_afp_data_valid(data, 0);
    ...
    let h_id = crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, h);
    assert forall|i: int| 0 <= i < crate::pred_normal_form_afp_textbook::b_words(afp).len()
        implies word_valid(
            #[trigger] crate::pred_normal_form_afp_textbook::b_words(afp)[i],
            afp.p2.num_generators)
    by {}
    crate::pred_normal_form_afp_textbook::lemma_apply_embedding_in_subgroup_g2(
        afp.p2, crate::pred_normal_form_afp_textbook::b_words(afp), h_id);
}
...
///  A prefix of a p-reduced word is also p-reduced.
proof fn lemma_no_pinch_prefix(data: PredHNNData, w: Word)
...
proof fn lemma_gen_inv_base_not_in_B(
    data: PredHNNData, w: Word, i: int, j: int,
)
    requires
        hnn_pred_data_valid(data),
        !has_pinch(data, w),
    ...
    }),
{
    ...
    let afp = tower_afp_data(data, 0);
    let base_word = w.subrange(i + 1, j);
    if crate::pred_normal_form_amalgamated::in_right_subgroup_pred(afp, base_word) {
        //  Connect in_right_subgroup_pred(afp, ...) to the b_gens in has_pinch_at.
        //  afp.p2 = data.base, and the b_gens sequences are extensionally equal.
        let nk = data.associations.len();
        let b_gens = Seq::new(nk, |k: int| data.associations[k].1);
        assert(afp.p2 == data.base);
        //  in_right_subgroup_pred unfolds with afp's identifications
        ...
        assert(crate::pred_normal_form_amalgamated::in_generated_subgroup_pred(data.base, b_gens, base_word));
        assert(has_pinch_at(data, w, i, j));
        assert(has_pinch(data, w));
    }
}
    ...
    }),
{
    let afp = tower_afp_data(data, 0);
    let base_word = w.subrange(i + 1, j);
    if crate::pred_normal_form_amalgamated::in_left_subgroup_pred(afp, base_word) {
        let nk = data.associations.len();
        let a_gens = Seq::new(nk, |k: int| data.associations[k].0);
        ...
        assert(crate::pred_normal_form_amalgamated::in_generated_subgroup_pred(data.base, a_gens, base_word));
        assert(has_pinch_at(data, w, i, j));
        assert(has_pinch(data, w));
    }
}
...
proof fn lemma_p_reduced_no_collapse(
    ...
    requires
        ...
        word_valid(w_orig, hnn_pred_presentation(data).num_generators),
        word_valid(h, data.base.num_generators),
        w.len() <= w_orig.len(),
        w =~= w_orig.subrange(0, w.len() as int),
        word_valid(h_id, crate::pred_normal_form_afp_textbook::k_size(
        ...
        if s == Symbol::Gen(ng) {
            ...
            if syls.len() == 0 {
                ...
                assert forall|i: int| 0 <= i
                    < crate::pred_normal_form_afp_textbook::b_words(afp).len()
                    implies word_valid(
                        #[trigger] crate::pred_normal_form_afp_textbook::b_words(afp)[i],
                        afp.p2.num_generators)
                by {}
                lemma_not_in_subgroup_concat_embed_b(
                    afp, base_word, h_id);
                //  Now: !in_right_subgroup_pred(afp, concat(base_word, embed_b(h_id)))
                //  = !in_right_subgroup_pred(afp, h) → b_rcoset_rep ≠ ε
                lemma_not_in_right_subgroup_rep_nonempty(afp, h);
            }
            //  ψ(p) PREPENDs (no collapse shown above).
            let (h_new, syls_new) = textbook_psi_p(data, h, syls);
            let new_h_id = crate::pred_normal_form_afp_textbook::b_rcoset_h(afp, h);
            crate::pred_tower::lemma_tower_afp_data_valid(data, 0);
            crate::pred_tower::lemma_tower_valid(data, 0);
            lemma_b_rcoset_h_word_valid(afp, h);
            ...
            assert(word_valid(h_new, ng)) by {
                assert forall|i: int| 0 <= i
                    < crate::pred_normal_form_afp_textbook::a_words(afp).len()
                    implies word_valid(
                        #[trigger] crate::pred_normal_form_afp_textbook::a_words(afp)[i], ng)
                by { crate::pred_tower::lemma_tower_num_generators(data, 0); }
                crate::benign::lemma_apply_embedding_valid(
                    crate::pred_normal_form_afp_textbook::a_words(afp), new_h_id, ng);
            }
            ...
            assert(h_new =~= concat(empty_word(),
                apply_embedding(
                    crate::pred_normal_form_afp_textbook::a_words(afp), new_h_id))) by {
            ...
            let (h_new, syls_new) = textbook_psi_p_inv(data, h, syls);
            let new_h_id = crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, h);
            crate::pred_tower::lemma_tower_afp_data_valid(data, 0);
            crate::pred_tower::lemma_tower_valid(data, 0);
            lemma_a_rcoset_h_word_valid(afp, h);
            assert(word_valid(h_new, ng)) by {
                assert forall|i: int| 0 <= i
                    < crate::pred_normal_form_afp_textbook::b_words(afp).len()
                    implies word_valid(
                        #[trigger] crate::pred_normal_form_afp_textbook::b_words(afp)[i], ng)
                by {}
                crate::benign::lemma_apply_embedding_valid(
                    crate::pred_normal_form_afp_textbook::b_words(afp), new_h_id, ng);
            }
            assert(w.drop_last() =~= w_orig.subrange(0, w.len() as int - 1));
            assert(w_orig.subrange(w.len() as int - 1, w.len() as int - 1)
                =~= empty_word());
            assert(h_new =~= concat(empty_word(),
                apply_embedding(
                    crate::pred_normal_form_afp_textbook::b_words(afp), new_h_id))) by {
                assert(concat(empty_word(), h_new) =~= h_new) by {
                    assert forall|k: int| 0 <= k < h_new.len()
                        implies concat(empty_word(), h_new)[k] == h_new[k] by {}
                }
...
proof fn lemma_psi_p_h_valid(data: PredHNNData, h: Word)
    requires
        hnn_pred_data_valid(data),
        word_valid(h, data.base.num_generators),
    ensures
...
proof fn lemma_psi_p_inv_h_valid(data: PredHNNData, h: Word)
    requires
        hnn_pred_data_valid(data),
        word_valid(h, data.base.num_generators),
    ensures
...
proof fn lemma_p_reduced_initial_no_collapse(data: PredHNNData, w: Word)
    requires
        hnn_pred_data_valid(data),
        !has_pinch(data, w),
        word_valid(w, hnn_pred_presentation(data).num_generators),
...
//  5. PredDerivation steps preserve syllables → w ≡ ε → 0 syllables
...
//    h1 ≡ h2 → concat(h1, inv(h2)) ≡ ε → in_right_subgroup_pred(concat(h1, inv(h2)))
...
proof fn lemma_psi_p_respects_base_equiv(
    data: PredHNNData, h1: Word, h2: Word, syls: Seq<Syllable>,
)
    requires
        hnn_pred_data_valid(data),
        word_valid(h1, data.base.num_generators),
        word_valid(h2, data.base.num_generators),
        equiv_in_pred_presentation(data.base, h1, h2),
    ensures
        textbook_psi_p(data, h1, syls) == textbook_psi_p(data, h2, syls),
{
    ...
    //  Step 1: same B-coset
    crate::pred_normal_form_afp_textbook::lemma_same_b_rcoset_from_equiv(afp, h1, h2);

    //  Step 2: same rep
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_rep_invariant(afp, h1, h2);

    //  Step 3: same h_id — need witnesses
    //  From b_rcoset_rep_props: same_b_rcoset(g, rep) → in_right_subgroup_pred → crate::pred_normal_form_amalgamated::in_generated_subgroup_pred
    //  → subgroup_to_k_word gives the witness
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_rep_props(afp, h1);
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_rep_props(afp, h2);
    crate::word::lemma_inverse_word_valid(
        crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h1), ng);
    crate::word::lemma_inverse_word_valid(
        crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h2), ng);
    crate::word::lemma_concat_word_valid(
        h1, inverse_word(crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h1)), ng);
    crate::word::lemma_concat_word_valid(
        h2, inverse_word(crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h2)), ng);
    crate::pred_normal_form_afp_textbook::lemma_subgroup_to_k_word(
        data.base, crate::pred_normal_form_afp_textbook::b_words(afp),
        concat(h1, inverse_word(crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h1))));
    crate::pred_normal_form_afp_textbook::lemma_subgroup_to_k_word(
        data.base, crate::pred_normal_form_afp_textbook::b_words(afp),
        concat(h2, inverse_word(crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h2))));
    assert(crate::pred_normal_form_afp_textbook::b_words(afp).len()
        == crate::pred_normal_form_afp_textbook::k_size(afp));
    let hw1: Word = choose|hw: Word|
        word_valid(hw, crate::pred_normal_form_afp_textbook::k_size(afp))
        && equiv_in_pred_presentation(data.base,
            apply_embedding(crate::pred_normal_form_afp_textbook::b_words(afp), hw),
            concat(h1, inverse_word(crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h1))));
    let hw2: Word = choose|hw: Word|
        ...
        && equiv_in_pred_presentation(data.base,
            apply_embedding(crate::pred_normal_form_afp_textbook::b_words(afp), hw),
            concat(h2, inverse_word(crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h2))));
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_h_equiv_invariant_general(
        afp, h1, h2, hw1, hw2);
        ...
        textbook_psi_p_inv(data, h1, syls) == textbook_psi_p_inv(data, h2, syls),
{
    ...
    //  Step 1: same A-coset
    crate::pred_normal_form_afp_textbook::lemma_same_a_rcoset_from_equiv(afp, h1, h2);

    //  Step 2: same rep
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_invariant(afp, h1, h2);

    //  Step 3: same h_id — need witnesses via subgroup_to_k_word
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_props(afp, h1);
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_props(afp, h2);
    crate::word::lemma_inverse_word_valid(
        crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h1), ng);
    crate::word::lemma_inverse_word_valid(
        crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h2), ng);
    crate::word::lemma_concat_word_valid(
        h1, inverse_word(crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h1)), ng);
    crate::word::lemma_concat_word_valid(
        h2, inverse_word(crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h2)), ng);
    crate::pred_normal_form_afp_textbook::lemma_subgroup_to_k_word(
        data.base, crate::pred_normal_form_afp_textbook::a_words(afp),
        concat(h1, inverse_word(crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h1))));
    crate::pred_normal_form_afp_textbook::lemma_subgroup_to_k_word(
        data.base, crate::pred_normal_form_afp_textbook::a_words(afp),
        concat(h2, inverse_word(crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h2))));
    assert(crate::pred_normal_form_afp_textbook::a_words(afp).len()
        == crate::pred_normal_form_afp_textbook::k_size(afp));
    let hw1: Word = choose|hw: Word|
        word_valid(hw, crate::pred_normal_form_afp_textbook::k_size(afp))
        && equiv_in_pred_presentation(data.base,
            apply_embedding(crate::pred_normal_form_afp_textbook::a_words(afp), hw),
            concat(h1, inverse_word(crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h1))));
    let hw2: Word = choose|hw: Word|
        word_valid(hw, crate::pred_normal_form_afp_textbook::k_size(afp))
        && equiv_in_pred_presentation(data.base,
            apply_embedding(crate::pred_normal_form_afp_textbook::a_words(afp), hw),
            concat(h2, inverse_word(crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h2))));
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_h_equiv_invariant(
        afp, h1, h2, hw1, hw2);
...
proof fn lemma_act_hnn_respects_base_equiv(
    data: PredHNNData, w: Word, h1: Word, h2: Word, syls: Seq<Syllable>,
)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
    ...
    decreases w.len(),
{
    if w.len() > 0 {
        ...
        if s == Symbol::Gen(ng) {
            ...
            lemma_pred_equiv_refl(data.base, textbook_act_hnn(data, w.drop_last(), h_new, syls_new).0);
        } else if s == Symbol::Inv(ng) {
            ...
            lemma_pred_equiv_concat_right(data.base, s_word, h1, h2);
        ...
        word_valid(textbook_psi_p(data, h, syls).0, data.base.num_generators),
{
    ...
    let ng = data.base.num_generators;
    lemma_tower_afp_data_valid(data, 0);
    let b_ws = crate::pred_normal_form_afp_textbook::b_words(afp);
    let a_ws = crate::pred_normal_form_afp_textbook::a_words(afp);
    let h_id = crate::pred_normal_form_afp_textbook::b_rcoset_h(afp, h);
    let ks = crate::pred_normal_form_afp_textbook::k_size(afp);
    assert(a_ws.len() == ks);
    //  Get h_id word_valid
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_rep_props(afp, h);
    crate::word::lemma_inverse_word_valid(
        crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h), ng);
    crate::word::lemma_concat_word_valid(h,
        inverse_word(crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h)), ng);
    crate::pred_normal_form_afp_textbook::lemma_subgroup_to_k_word(
        data.base, b_ws, concat(h,
            inverse_word(crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h))));
    assert(b_ws.len() == ks);
    let hw: Word = choose|hw: Word| word_valid(hw, ks)
        && equiv_in_pred_presentation(data.base,
            apply_embedding(b_ws, hw), concat(h,
                inverse_word(crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h))));
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_decomposition(afp, h, hw);
    //  word_valid(h_id, ks)
...
proof fn lemma_group_cancel_right(p: PredPresentation, a: Word, b: Word, c: Word)
    requires
        pred_presentation_valid(p),
        word_valid(a, p.num_generators),
        word_valid(b, p.num_generators),
        word_valid(c, p.num_generators),
        equiv_in_pred_presentation(p, concat(a, c), concat(b, c)),
    ensures
        equiv_in_pred_presentation(p, a, b),
{
    ...
    crate::pred_normal_form_afp_textbook::lemma_right_cancel(p, a, c);
    //  concat(concat(b, c), inv(c)) ≡ b
    crate::pred_normal_form_afp_textbook::lemma_right_cancel(p, b, c);
    //  concat(a·c, inv(c)) ≡ concat(b·c, inv(c))  [from premise, concat_left with inv(c)]
    lemma_pred_equiv_concat_left(p, concat(a, c), concat(b, c), inverse_word(c));
    //  Chain: a ≡ concat(a·c, inv(c)) ≡ concat(b·c, inv(c)) ≡ b
    crate::word::lemma_concat_word_valid(concat(a, c), inverse_word(c), p.num_generators);
    lemma_pred_equiv_symmetric(p, concat(concat(a, c), inverse_word(c)), a);
    lemma_pred_equiv_transitive(p, a,
...
/// From identifications_isomorphic_pred: embed_b(k1) ≡ embed_b(k2) → embed_a(k1) ≡ embed_a(k2).
proof fn lemma_iso_transfer_b_to_a(
    data: PredHNNData, k1: Word, k2: Word,
)
    requires
        hnn_pred_data_valid(data),
        hnn_pred_associations_isomorphic(data),
        ({
            ...
            &&& equiv_in_pred_presentation(data.base,
                    apply_embedding(crate::pred_normal_form_afp_textbook::b_words(afp), k1),
                    apply_embedding(crate::pred_normal_form_afp_textbook::b_words(afp), k2))
        }),
    ensures
        equiv_in_pred_presentation(data.base,
            apply_embedding(crate::pred_normal_form_afp_textbook::a_words(tower_afp_data(data, 0)), k1),
            apply_embedding(crate::pred_normal_form_afp_textbook::a_words(tower_afp_data(data, 0)), k2)),
{
    let afp = tower_afp_data(data, 0);
    let ng = data.base.num_generators;
    let a_ws = crate::pred_normal_form_afp_textbook::a_words(afp);
    ...
    lemma_pred_equiv_concat_left(data.base, eb1, eb2, inverse_word(eb2));
    lemma_pred_word_inverse_right(data.base, eb2);
    crate::word::lemma_concat_word_valid(eb1, inverse_word(eb2), ng);
    lemma_pred_equiv_transitive(data.base,
    ...
    lemma_pred_cancel_inverse_right(data.base, ea1, ea2);
}
...
proof fn lemma_iso_transfer_a_to_b(
    ...
    requires
        ...
        ({
            ...
            &&& word_valid(k1, ks) && word_valid(k2, ks)
            &&& equiv_in_pred_presentation(data.base,
                    apply_embedding(crate::pred_normal_form_afp_textbook::a_words(afp), k1),
                    apply_embedding(crate::pred_normal_form_afp_textbook::a_words(afp), k2))
        }),
    ensures
        equiv_in_pred_presentation(data.base,
            apply_embedding(crate::pred_normal_form_afp_textbook::b_words(tower_afp_data(data, 0)), k1),
            apply_embedding(crate::pred_normal_form_afp_textbook::b_words(tower_afp_data(data, 0)), k2)),
{
    let afp = tower_afp_data(data, 0);
    let ng = data.base.num_generators;
    let a_ws = crate::pred_normal_form_afp_textbook::a_words(afp);
    ...
    crate::word::lemma_inverse_word_valid(ea2, ng);
    lemma_pred_equiv_concat_left(data.base, ea1, ea2, inverse_word(ea2));
    lemma_pred_word_inverse_right(data.base, ea2);
    ...
    crate::word::lemma_inverse_word_valid(eb2, ng);
    lemma_pred_cancel_inverse_right(data.base, eb1, eb2);
}
...
proof fn lemma_stable_pair_gen_inv_case_a(
    ...
    requires
        ...
        !({
            let afp = tower_afp_data(data, 0);
            let rep_a = crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h);
    ...
    ensures ({
        ...
        out.1 == syls
        && equiv_in_pred_presentation(data.base, out.0, h)
    }),
{
    let afp = tower_afp_data(data, 0);
    let ng = data.base.num_generators;
    ...
    let rep_a = crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h);
    let h_id_a = crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, h);
    let a_ws = crate::pred_normal_form_afp_textbook::a_words(afp);
    let b_ws = crate::pred_normal_form_afp_textbook::b_words(afp);
    let phi_h = apply_embedding(b_ws, h_id_a);
    assert(a_ws.len() == crate::pred_normal_form_afp_textbook::k_size(afp));
    assert(b_ws.len() == crate::pred_normal_form_afp_textbook::k_size(afp));

    //  A-decomposition witness + properties
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_props(afp, h);
    ...
    crate::pred_normal_form_afp_textbook::lemma_rcoset_decomposition(afp, h, hw_a);
    ...
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_in_subgroup(afp, phi_h);

    //  Round-trip: b_rcoset_h(embed_b(h_id_a)) =~= h_id_a
    lemma_tower_identifications_isomorphic(data, 0);
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_h_b_canonical(afp, h, hw_a);
    ...
    lemma_pred_equiv_symmetric(data.base,
    ...
    }),
{
    ...
    crate::pred_normal_form_afp_textbook::lemma_subgroup_to_k_word(
        data.base, a_ws, concat(h, inverse_word(rep_a)));
    let hw_a: Word = choose|hw: Word|
        word_valid(hw, ks) && equiv_in_pred_presentation(data.base,
            apply_embedding(a_ws, hw), concat(h, inverse_word(rep_a)));
    crate::pred_normal_form_afp_textbook::lemma_rcoset_decomposition(afp, h, hw_a);
    let h_id_a = crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, h);
...
proof fn lemma_stable_pair_gen_inv_case_b(
    ...
    requires
        ...
        word_valid(syls.first().rep, data.base.num_generators),
        !(syls.first().rep =~= empty_word()),
        crate::pred_normal_form_afp_textbook::b_rcoset_rep(
    ...
    }),
{
    ...
    crate::pred_normal_form_afp_textbook::lemma_right_cancel(data.base, phi_h, top_rep);
    ...
    lemma_pred_equiv_symmetric(data.base, concat(h_mid, inverse_word(top_rep)), phi_h);
    //  phi_h ≡ concat(h_mid, inv(top_rep))
    crate::pred_normal_form_afp_textbook::lemma_in_subgroup_equiv(
        data.base, b_ws, phi_h, concat(h_mid, inverse_word(top_rep)));
    //  in_right_subgroup_pred(afp, concat(h_mid, inv(top_rep))) = same_b_rcoset(h_mid, top_rep)

    //  rep_b = b_rcoset_rep(h_mid) =~= b_rcoset_rep(top_rep) =~= top_rep (canonical)
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_rep_invariant(afp, h_mid, top_rep);
    ...
    let rep_b = crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h_mid);
...
proof fn lemma_stable_pair_case_b_h_equiv(
    data: PredHNNData, h: Word, h_mid: Word, h_id_a: Word, top_rep: Word, hw_a: Word,
)
    requires
        hnn_pred_data_valid(data),
        hnn_pred_associations_isomorphic(data),
        ({
            ...
            &&& equiv_in_pred_presentation(data.base,
                    concat(apply_embedding(a_ws, h_id_a),
                        crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h)), h)
            &&& crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h) =~= empty_word()
            //  h_id_a = a_rcoset_h(afp, h)
            &&& h_id_a =~= crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, h)
            //  same_b_rcoset(h_mid, top_rep) already established
            &&& crate::pred_normal_form_amalgamated::in_right_subgroup_pred(afp,
                    concat(h_mid, inverse_word(top_rep)))
            //  B-rcoset_rep(h_mid) =~= top_rep (from invariance + canonicity)
            &&& crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h_mid) =~= top_rep
        }),
    ensures
        equiv_in_pred_presentation(data.base,
            apply_embedding(
                crate::pred_normal_form_afp_textbook::a_words(tower_afp_data(data, 0)),
                crate::pred_normal_form_afp_textbook::b_rcoset_h(tower_afp_data(data, 0), h_mid)),
            h),
{
    let afp = tower_afp_data(data, 0);
    let ng = data.base.num_generators;
    ...
    let b_ws = crate::pred_normal_form_afp_textbook::b_words(afp);
    let ks = crate::pred_normal_form_afp_textbook::k_size(afp);
    let h_id_b = crate::pred_normal_form_afp_textbook::b_rcoset_h(afp, h_mid);
    ...
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_rep_props(afp, h_mid);
    crate::word::lemma_inverse_word_valid(
        crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h_mid), ng);
    crate::word::lemma_concat_word_valid(h_mid,
        inverse_word(crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h_mid)), ng);
    crate::pred_normal_form_afp_textbook::lemma_subgroup_to_k_word(
        data.base, b_ws, concat(h_mid,
            inverse_word(crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h_mid))));
    let hw_b: Word = choose|hw: Word|
        word_valid(hw, ks) && equiv_in_pred_presentation(data.base,
            apply_embedding(b_ws, hw), concat(h_mid,
                inverse_word(crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h_mid))));
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_decomposition(afp, h_mid, hw_b);
    ...
    lemma_pred_equiv_refl(data.base, h_mid);
    //  Since h_mid =~= concat(embed_b_ha, top_rep), Z3 should see equiv(h_mid, concat(embed_b_ha, top_rep))
    lemma_pred_equiv_symmetric(data.base, concat(embed_b_hb, top_rep), h_mid);
    lemma_pred_equiv_transitive(data.base,
    ...
    crate::pred_normal_form_afp_textbook::lemma_right_cancel(data.base, embed_b_hb, top_rep);
    crate::pred_normal_form_afp_textbook::lemma_right_cancel(data.base, embed_b_ha, top_rep);
    //  Build: embed_b_hb ≡ RHS chain ≡ embed_b_ha
    lemma_pred_equiv_concat_left(data.base,
    ...
    lemma_pred_equiv_transitive(data.base, embed_b_hb,
    ...
    lemma_pred_equiv_concat_left(data.base, embed_b_hb, embed_b_ha, inverse_word(embed_b_ha));
    lemma_pred_word_inverse_right(data.base, embed_b_ha);
    ...
    //  By identifications_isomorphic_pred: embed_a(diff) ≡ ε
    lemma_tower_identifications_isomorphic(data, 0);
    ...
    lemma_pred_cancel_inverse_right(data.base, embed_a_hb, embed_a_ha);
    //  embed_a(h_id_b) ≡ embed_a(h_id_a) ✓

    //  Step 5: embed_a(h_id_a) ≡ h (from A-decomposition, rep_a = ε)
    lemma_pred_equiv_symmetric(data.base,
        concat(embed_a_ha, crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h)), h);

    //  Step 6: chain embed_a(h_id_b) ≡ embed_a(h_id_a) ≡ h
    lemma_pred_equiv_transitive(data.base, embed_a_hb, embed_a_ha, h);
}
...
proof fn lemma_stable_pair_inv_gen_case_a(
    ...
    requires
        ...
        !({
            let afp = tower_afp_data(data, 0);
            let rep_b = crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h);
    ...
    }),
{
    ...
    let rep_b = crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h);
    let h_id_b = crate::pred_normal_form_afp_textbook::b_rcoset_h(afp, h);
    let a_ws = crate::pred_normal_form_afp_textbook::a_words(afp);
    let b_ws = crate::pred_normal_form_afp_textbook::b_words(afp);
    ...
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_decomposition(afp, h, hw_b);
    ...
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_in_subgroup(afp, embed_a_hb);
    ...
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_props(afp, embed_a_hb);
    crate::word::lemma_inverse_word_valid(
        crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, embed_a_hb), ng);
    crate::word::lemma_concat_word_valid(embed_a_hb,
        inverse_word(crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, embed_a_hb)), ng);
    crate::pred_normal_form_afp_textbook::lemma_subgroup_to_k_word(
        data.base, a_ws, concat(embed_a_hb,
            inverse_word(crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, embed_a_hb))));
    let hw_a2: Word = choose|hw: Word|
        word_valid(hw, ks) && equiv_in_pred_presentation(data.base,
            apply_embedding(a_ws, hw), concat(embed_a_hb,
                inverse_word(crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, embed_a_hb))));
    crate::pred_normal_form_afp_textbook::lemma_rcoset_decomposition(afp, embed_a_hb, hw_a2);
    //  embed_a(h_id_a') · ε ≡ embed_a_hb = embed_a(h_id_b)
    let h_id_a2 = crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, embed_a_hb);
    ...
    }),
{
    ...
    lemma_tower_identifications_isomorphic(data, 0);
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_h_b_canonical(afp, h, hw_b);
...
proof fn lemma_stable_pair_inv_gen_case_b2_h_equiv(
    data: PredHNNData, h: Word, hw_b: Word,
)
    requires
        hnn_pred_data_valid(data),
        hnn_pred_associations_isomorphic(data),
    ...
    ensures ({
        ...
        let h_id_b = crate::pred_normal_form_afp_textbook::b_rcoset_h(afp, h);
        let embed_a_hb = apply_embedding(a_ws, h_id_b);
        let h_id_a = crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, embed_a_hb);
        equiv_in_pred_presentation(data.base, apply_embedding(b_ws, h_id_a), h)
    }),
{
    let afp = tower_afp_data(data, 0);
    let ng = data.base.num_generators;
    ...
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_in_subgroup(afp, embed_a_hb);
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_props(afp, embed_a_hb);
    let rep_a = crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, embed_a_hb);
    crate::word::lemma_inverse_word_valid(rep_a, ng);
    ...
    crate::pred_normal_form_afp_textbook::lemma_rcoset_decomposition(afp, embed_a_hb, hw_a);
    let h_id_a = crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, embed_a_hb);
...
proof fn lemma_stable_pair_inv_gen_case_b(
    ...
    requires
        ...
        word_valid(syls.first().rep, data.base.num_generators),
        !(syls.first().rep =~= empty_word()),
        crate::pred_normal_form_afp_textbook::a_rcoset_rep(
            tower_afp_data(data, 0), syls.first().rep) =~= syls.first().rep,
    ensures ({
        let mid = textbook_psi_p(data, h, syls);
        let out = textbook_psi_p_inv(data, mid.0, mid.1);
    ...
    }),
{
    ...
    crate::word::lemma_inverse_word_valid(top_rep, ng);
    crate::pred_normal_form_afp_textbook::lemma_right_cancel(data.base, embed_a_hb, top_rep);
    crate::word::lemma_concat_word_valid(h_mid, inverse_word(top_rep), ng);
    lemma_pred_equiv_symmetric(data.base, concat(h_mid, inverse_word(top_rep)), embed_a_hb);
    crate::pred_normal_form_afp_textbook::lemma_in_subgroup_equiv(
    ...
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_invariant(afp, h_mid, top_rep);
    ...
    assert(mid.0 =~= h_mid);
    assert(mid.1 =~= syls.drop_first());
    let rep_a_mid = crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h_mid);
    ...
    //  So embed_b(h_id_a') ≡ h
    let h_id_a_mid = crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, h_mid);

    //  A-decomposition of h_mid
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_props(afp, h_mid);
    ...
    crate::pred_normal_form_afp_textbook::lemma_rcoset_decomposition(afp, h_mid, hw_a_mid);
    ...
    crate::word::lemma_concat_word_valid(embed_a_hb, top_rep, ng);
    crate::pred_normal_form_afp_textbook::lemma_right_cancel(
...
#[verifier::tactus_tactic("first | first | rfl | decide | (simp_all only [Classical.not_forall, Decidable.not_not, Int.add_emod_left, Int.cast_ofNat_Int, Int.natCast_add, Int.neg_add_emod_self, Int.ofNat_eq_coe, Int.ofNat_zero_le, Int.sub_zero, Int.toNat_natCast_add_one, Int.zero_add, Int.zero_sub, Int.mul_add, Int.add_mul, Int.toNat_zero, Int.toNat_one, Int.add_sub_cancel, Nat.add_le_add_iff_right, Nat.add_left_cancel_iff, Nat.add_zero, Nat.le_add_left, Nat.le_add_right, Nat.le_refl, Nat.not_le, Nat.not_lt, Nat.reduceLeDiff, Nat.sub_le_iff_le_add, Nat.zero_add, Nat.zero_le, Nat.mul_add, Nat.add_mul, Nat.add_sub_cancel, and_imp, and_self, and_true, eq_iff_iff, forall_const, forall_eq, ge_iff_le, gt_iff_lt, iff_true, imp_false, imp_self, implies_true, not_and, not_exists, not_false_eq_true, Classical.not_imp, not_or, not_true_eq_false, true_and] <;> omega) | (intro afp rep_b is_collapse hnn hne; have hc := Classical.not_not.mp hnn; have hf := h_req3 ⟨hc.1.2, hc.2⟩; exact ⟨h_req0, h_req1, h_req2, hc, hf.1.1, hf.1.2, hf.2⟩) | (intro afp rep_b is_collapse hnn hnn2; have hf := h_req3 ⟨(Classical.not_not.mp hnn).1.2, (Classical.not_not.mp hnn).2⟩; exact absurd (Classical.not_not.mp hnn2) hf.1.2)")]
...
proof fn lemma_hnn_relator_preserves(
    data: PredHNNData, i: int, h: Word, syls: Seq<Syllable>,
)
    requires
        hnn_pred_data_valid(data),
        hnn_pred_associations_isomorphic(data),
    ...
    }),
{
    ...
    crate::pred_normal_form_afp_textbook::lemma_right_cancel(data.base, inv_bi, h);
    ...
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_rep_invariant(afp, h_prime, h);
    //  same rep → same ψ(p) branch → same syls ✓

    //  Step 2: B-decompositions → embed_b(h_id2) ≡ concat(inv_bi, embed_b(h_id))
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_rep_props(afp, h);
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_rep_props(afp, h_prime);
    let rep = crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h);
    ...
    crate::pred_normal_form_afp_textbook::lemma_subgroup_to_k_word(
        data.base, b_ws, concat(h_prime, inverse_word(
            crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h_prime))));
    ...
    let hw2: Word = choose|hw: Word| word_valid(hw, ks)
        && equiv_in_pred_presentation(data.base,
            apply_embedding(b_ws, hw), concat(h_prime, inverse_word(
                crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h_prime))));
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_decomposition(afp, h, hw1);
    crate::pred_normal_form_afp_textbook::lemma_b_rcoset_decomposition(afp, h_prime, hw2);
    let h_id = crate::pred_normal_form_afp_textbook::b_rcoset_h(afp, h);
    let h_id2 = crate::pred_normal_form_afp_textbook::b_rcoset_h(afp, h_prime);
    ...
    lemma_pred_equiv_concat_right(data.base, inv_bi,
    ...
    lemma_pred_equiv_concat_right(data.base, a_i,
    ...
    lemma_pred_word_inverse_right(data.base, a_i);
    lemma_pred_equiv_concat_left(data.base,
        concat(a_i, inv_ai), empty_word(), apply_embedding(a_ws, h_id));
    lemma_pred_concat_identity_left(data.base, apply_embedding(a_ws, h_id));
    ...
    let psi_rep = crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, h);
...
/// Decomposition helper: shows act(hnn_relator_pred(i), h, syls) == ψ(p⁻¹)(concat(a_i, ψ(p)(h', syls).0), ψ(p)(h', syls).1)
...
proof fn lemma_hnn_relator_decompose(
    ...
    ensures ({
        ...
        textbook_act_hnn(data, hnn_relator_pred(data, i), h, syls)
    ...
    }),
{
    ...
    let inv_bi = inverse_word(b_i);
    let r = hnn_relator_pred(data, i);
    let t_word: Word = Seq::new(1, |_j: int| stable_letter_pred(data));
    let t_inv_word: Word = Seq::new(1, |_j: int| stable_letter_inv_pred(data));
    let h_prime = concat(inv_bi, h);
...
proof fn lemma_hnn_dual_conjugation_chain(
    ...
    ensures ({
        ...
        &&& syls_psi_conj == syls_psi_orig
        &&& equiv_in_pred_presentation(data.base, concat(b_i, h_psi_conj), h_psi_orig)
    }),
{
    ...
    crate::word::lemma_inverse_word_valid(h, ng);
    crate::pred_normal_form_afp_textbook::lemma_right_cancel(data.base, inv_ai, h);
    crate::word::lemma_concat_word_valid(h_prime, inverse_word(h), ng);
    lemma_pred_equiv_symmetric(data.base,
    ...
    crate::pred_normal_form_afp_textbook::lemma_in_subgroup_equiv(
        ...
        concat(h_prime, inverse_word(h)));
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_invariant(afp, h_prime, h);

    //  Step 2: A-decompositions → embed_a(h_id2) ≡ concat(inv_ai, embed_a(h_id))
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_props(afp, h);
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_props(afp, h_prime);
    let rep = crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h);
    crate::word::lemma_inverse_word_valid(rep, ng);
    crate::word::lemma_concat_word_valid(h, inverse_word(rep), ng);
    crate::word::lemma_concat_word_valid(h_prime, inverse_word(rep), ng);
    ...
    crate::pred_normal_form_afp_textbook::lemma_subgroup_to_k_word(
        data.base, a_ws, concat(h_prime, inverse_word(
            crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h_prime))));
    let hw1: Word = choose|hw: Word| word_valid(hw, ks)
        && equiv_in_pred_presentation(data.base,
    ...
    let hw2: Word = choose|hw: Word| word_valid(hw, ks)
        && equiv_in_pred_presentation(data.base,
            apply_embedding(a_ws, hw), concat(h_prime, inverse_word(
                crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h_prime))));
    crate::pred_normal_form_afp_textbook::lemma_rcoset_decomposition(afp, h, hw1);
    crate::pred_normal_form_afp_textbook::lemma_rcoset_decomposition(afp, h_prime, hw2);
    let h_id = crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, h);
    let h_id2 = crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, h_prime);
    crate::benign::lemma_apply_embedding_valid(a_ws, h_id, ng);
    crate::benign::lemma_apply_embedding_valid(a_ws, h_id2, ng);
    crate::benign::lemma_apply_embedding_valid(b_ws, h_id, ng);
    crate::benign::lemma_apply_embedding_valid(b_ws, h_id2, ng);
    ...
    lemma_pred_equiv_concat_right(data.base, inv_ai,
    ...
    lemma_pred_equiv_concat_right(data.base, b_i,
        apply_embedding(b_ws, h_id2),
        concat(inv_bi, apply_embedding(b_ws, h_id)));
    lemma_pred_word_inverse_right(data.base, b_i);
    lemma_pred_equiv_concat_left(data.base,
        concat(b_i, inv_bi), empty_word(), apply_embedding(b_ws, h_id));
    lemma_pred_concat_identity_left(data.base, apply_embedding(b_ws, h_id));
    ...
    let psi_rep = crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, h);
    if !(psi_rep =~= empty_word() && syls.len() > 0 && !syls.first().is_left) {
...
proof fn lemma_hnn_relator_inverse_decompose(
    ...
    ensures ({
        ...
        textbook_act_hnn(data, inverse_word(hnn_relator_pred(data, i)), h, syls)
            == (concat(b_i, h2), syls2)
    }),
{
    let ng = data.base.num_generators;
    ...
    //  Help Z3: inverse_word(hnn_relator_pred) =~= right-associated concat
    let r_inv = inverse_word(hnn_relator_pred(data, i));
    ...
    }),
{
    ...
    assert(syls1.len() > 0 ==> word_valid(syls1.first().rep, ng)) by {
        let rep = crate::pred_normal_form_afp_textbook::b_rcoset_rep(
            tower_afp_data(data, 0), h);
        if !(rep =~= empty_word() && syls.len() > 0 && syls.first().is_left) {
            crate::pred_normal_form_afp_textbook::lemma_b_rcoset_rep_props(
                tower_afp_data(data, 0), h);
        } else {
    ...
    lemma_pred_equiv_transitive(data.base, concat(b_i, h2), rt.0, h);
}

//  ---- Tier 3: Assembly — the final Miller argument ----

/// has_stable_letter implies stable_count ≥ 1.
pub proof fn lemma_has_stable_implies_count(data: PredHNNData, w: Word)
    requires
        has_stable_letter(data, w),
    ensures
...
pub proof fn lemma_no_pinch_action_nontrivial(data: PredHNNData, w: Word)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
        has_stable_letter(data, w),
...
pub open spec fn hnn_canonical_state(data: PredHNNData, h: Word, syls: Seq<Syllable>) -> bool {
    let afp = tower_afp_data(data, 0);
    let ng = data.base.num_generators;
    &&& word_valid(h, ng)
    &&& (forall|j: int| #![trigger syls[j]] 0 <= j < syls.len() ==> ({
        ...
        &&& (syls[j].is_left ==>
                crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, syls[j].rep)
                    =~= syls[j].rep)
        &&& (!syls[j].is_left ==>
                crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, syls[j].rep)
                    =~= syls[j].rep)
    }))
    ...
    decreases w.len(),
{
    if w.len() > 0 {
        ...
        if s == Symbol::Gen(ng) {
            ...
            crate::pred_normal_form_afp_textbook::lemma_subgroup_to_k_word(
                data.base,
                crate::pred_normal_form_afp_textbook::a_words(afp),
            ...
            let hw: Word = choose|hw: Word|
                ...
                && equiv_in_pred_presentation(data.base,
                    apply_embedding(crate::pred_normal_form_afp_textbook::a_words(afp), hw),
                    concat(h, inverse_word(rep)));
            crate::pred_normal_form_afp_textbook::lemma_rcoset_decomposition(afp, h, hw);
            let h_id = crate::pred_normal_form_afp_textbook::a_rcoset_h(afp, h);
            crate::benign::lemma_apply_embedding_valid(
                crate::pred_normal_form_afp_textbook::b_words(afp), h_id, ng);
            if is_collapse {
                crate::word::lemma_concat_word_valid(
                    apply_embedding(crate::pred_normal_form_afp_textbook::b_words(afp), h_id),
                    syls.first().rep, ng);
            }
            ...
            textbook_psi_p(data, h, syls).1),
{
    ...
    if !is_collapse {
        crate::pred_normal_form_afp_textbook::lemma_b_rcoset_rep_props(afp, h);
        crate::pred_normal_form_afp_textbook::lemma_b_rcoset_rep_idempotent(afp, h);
    }
    ...
    assert forall|j: int| #![trigger syls1[j]] 0 <= j < syls1.len()
        implies (syls1[j].is_left ==>
                crate::pred_normal_form_afp_textbook::a_rcoset_rep(afp, syls1[j].rep) =~= syls1[j].rep)
            && (!syls1[j].is_left ==>
                crate::pred_normal_form_afp_textbook::b_rcoset_rep(afp, syls1[j].rep) =~= syls1[j].rep)
    by {
        if !is_collapse {
            if j == 0 {} else { assert(syls1[j] == syls[j - 1]); }
        } else { assert(syls1[j] == syls[j + 1]); }
            ...
            textbook_psi_p_inv(data, h, syls).1),
{
    ...
    if !is_collapse {
        crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_props(afp, h);
        crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_idempotent(afp, h);
    }
    //  h1 word_valid
    crate::pred_normal_form_afp_textbook::lemma_a_rcoset_rep_props(afp, h);
    crate::word::lemma_inverse_word_valid(rep, ng);
...
proof fn lemma_trivial_middle_preserves_syls(
    ...
    requires
        hnn_pred_data_valid(data),
        word_valid(prefix, hnn_pred_presentation(data).num_generators),
        word_valid(suffix, hnn_pred_presentation(data).num_generators),
        ({
            ...
            &&& syls_m == syls_s
            &&& equiv_in_pred_presentation(data.base, h_m, h_s)
...
proof fn lemma_free_expand_base_preserves(
    data: PredHNNData, w: Word, pos: int, s: Symbol,
)
    requires
        hnn_pred_data_valid(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
    ...
    }),
{
    ...
    assert(equiv_in_pred_presentation(data.base, middle, empty_word())) by {
        let s_word: Word = Seq::new(1, |_i: int| s);
        assert(word_valid(s_word, ng)) by {
            assert forall|k: int| 0 <= k < s_word.len()
                implies symbol_valid(#[trigger] s_word[k], ng) by {}
        ...
        lemma_pred_word_inverse_right(data.base, s_word);
    ...
    lemma_pred_equiv_concat_left(data.base, middle, empty_word(), h_s);
    lemma_pred_concat_identity_left(data.base, h_s);
    lemma_pred_equiv_transitive(data.base,
    ...
    }),
{
    ...
    assert(equiv_in_pred_presentation(data.base,
...
proof fn lemma_free_reduce_preserves(
    data: PredHNNData, w: Word, pos: int,
)
    requires
        hnn_pred_data_valid(data),
        hnn_pred_associations_isomorphic(data),
    ...
    }),
{
    ...
    lemma_hnn_step_preserves_word_valid(
        data, w, PredDerivationStep::FreeReduce { position: pos });
    let w_next = reduce_at(w, pos);
...
proof fn lemma_relator_insert_preserves(
    data: PredHNNData, w: Word, pos: int, relator: Word, inverted: bool,
)
    requires
        hnn_pred_data_valid(data),
        hnn_pred_associations_isomorphic(data),
    ...
    }),
{
    ...
    //  word_valid for relator from pred_presentation_valid(hp)
    lemma_hnn_pred_presentation_valid(data);
    reveal(pred_presentation_valid);
    assert(word_valid(relator, ng + 1));
    if inverted {
        crate::word::lemma_inverse_word_valid(relator, ng + 1);
    }
    ...
    if (data.base.relators)(relator) {
        //  Base relator: word_valid from pred_presentation_valid(data.base)
        let base_rel = relator;
        ...
        crate::pred_presentation_lemmas::lemma_pred_relator_is_identity(data.base, relator);
        ...
        if inverted {
            ...
            crate::pred_normal_form_afp_textbook::lemma_equiv_inverse(
                data.base, base_rel, empty_word());
            crate::word::lemma_inverse_empty();
        }
        //  equiv(r, ε) in data.base
        lemma_pred_equiv_concat_left(data.base, r, empty_word(), h_s);
        lemma_pred_concat_identity_left(data.base, h_s);
        lemma_pred_equiv_transitive(data.base, concat(r, h_s), concat(empty_word(), h_s), h_s);
        lemma_trivial_middle_preserves_syls(data, prefix, r, suffix);
    } else {
        //  HNN relator: Tier 2 forward/inverse.  Since !(base.relators)(relator)
        //  and (hp.relators)(relator) = base || hnn_extra, hnn_extra holds.
        assert(hnn_extra_relator_pred(data, relator));
        let j = choose|i: int| 0 <= i < data.associations.len() && relator == hnn_relator_pred(data, i);
        assert(0 <= j < data.associations.len());
        //  Connect r (= relator) to hnn_relator_pred(data, j)
        assert(relator =~= hnn_relator_pred(data, j));
        if !inverted {
            assert(r =~= hnn_relator_pred(data, j));
            lemma_hnn_relator_preserves(data, j, h_s, syls_s);
        } else {
            assert(r =~= inverse_word(hnn_relator_pred(data, j)));
...
proof fn lemma_relator_delete_preserves(
    data: PredHNNData, w: Word, position: int, relator: Word, inverted: bool,
)
    requires
        hnn_pred_data_valid(data),
        hnn_pred_associations_isomorphic(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
        apply_step_pred(hnn_pred_presentation(data), w,
            PredDerivationStep::RelatorDelete { position, relator, inverted }).is_some(),
    ensures ({
        let w_next = apply_step_pred(hnn_pred_presentation(data), w,
    ...
    }),
{
    ...
    let ew = empty_word();
    let es = Seq::<Syllable>::empty();
    let step = PredDerivationStep::RelatorDelete { position, relator, inverted };
    //  apply_step_pred(delete) Some ⟹ (hp.relators)(relator) ∧ 0 ≤ position ∧ position+rlen ≤ w.len()
    assert((hp.relators)(relator));
    lemma_hnn_step_preserves_word_valid(data, w, step);
    let w_next_val = apply_step_pred(hp, w, step).unwrap();
    lemma_relator_insert_preserves(
        data, w_next_val, position, relator, inverted);
...
proof fn lemma_single_step_preserves_syls(
    data: PredHNNData,
    w: Word,
    step: PredDerivationStep,
)
    requires
        hnn_pred_data_valid(data),
        hnn_pred_associations_isomorphic(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
        apply_step_pred(hnn_pred_presentation(data), w, step).is_some(),
    ensures
        textbook_act_hnn(data, w, empty_word(), Seq::<Syllable>::empty()).1
                    ...
                    empty_word(), Seq::<Syllable>::empty()).1,
{
    ...
    match step {
        PredDerivationStep::FreeExpand { position, symbol } => {
            ...
            let w_next_exp = apply_step_pred(hp, w, step).unwrap();
        ...
        PredDerivationStep::RelatorInsert { position, relator, inverted } => {
            assert((hp.relators)(relator));
            lemma_relator_insert_preserves(data, w, position, relator, inverted);
            let prefix = w.subrange(0, position);
            let suffix = w.subrange(position, w.len() as int);
            assert(concat(prefix, suffix) =~= w);
            let r = get_relator_pred(relator, inverted);
            let w_next_ins = apply_step_pred(hp, w, step).unwrap();
            assert(w_next_ins =~= concat(prefix, concat(r, suffix)));
        },
        PredDerivationStep::RelatorDelete { position, relator, inverted } => {
            lemma_relator_delete_preserves(data, w, position, relator, inverted);
        },
    }
}

/// PredDerivation induction: if derivation from w to ε, then act(w, ε, []).1 is empty.
...
pub proof fn lemma_derivation_preserves_syls(
    ...
    requires
        ...
        hnn_pred_associations_isomorphic(data),
        word_valid(w, hnn_pred_presentation(data).num_generators),
        pred_derivation_produces(hnn_pred_presentation(data), steps, w)
            == Some(empty_word()),
    ensures
        textbook_act_hnn(data, w, empty_word(), Seq::<Syllable>::empty()).1
            =~= Seq::<Syllable>::empty(),
    decreases steps.len(),
{
    if steps.len() == 0 {
        ...
        //  Therefore: act(w, ε, []).1 =~= []
        let step = steps.first();
        let w_next = apply_step_pred(hnn_pred_presentation(data), w, step).unwrap();
